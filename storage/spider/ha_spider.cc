...
bool ha_spider::get_error_message(
  ...
  switch (error)
  {
    ...
    case ER_SPIDER_COND_INVALID_NUM:
        if (buf->reserve(ER_SPIDER_COND_INVALID_LEN))
            DBUG_RETURN(TRUE);
...
SPIDER_CONN* ha_spider::spider_get_conn_by_idx(int link_idx)
{
	...
	else
	{
		...
		{/* failed to create conn */
		 /*
		 1. ·ÖÅäÁ¬½ÓÊ§°Ü£¬´Ë´¦ÊÍ·Åshare¶ÔÏó»áµ¼ÖÂÏÂ´Î·ÃÎÊÒì³£
		 2. ·ÖÅäÁ¬½ÓÊ§°Ü£¬share²»Ó¦¸ÃÊÍ·Å(¿ÉÄÜshareÔÚËü´¦ÔÚÊ¹ÓÃÖÐ£©

		 */
		 //			share->init_error = TRUE;
		 //			share->init_error_time = (time_t) time((time_t*) 0); /* ²»´æÔÚ×óÖµ£¬ÎÞÒâÒå */
		 //			share->init = TRUE;
		 //			spider_free_share(share);
