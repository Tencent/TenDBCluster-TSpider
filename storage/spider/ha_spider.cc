...
handler *ha_spider::clone(const char *name, MEM_ROOT *mem_root) {
  ...
  DBUG_PRINT("info", ("spider this=%p", this));
  if (!(spider = (ha_spider *)get_new_handler(table->s, mem_root,
                                              spider_hton_ptr)) ||
      !(spider->ref =
...
int ha_spider::open(const char *name, int mode, uint test_if_locked) {
  THD *thd = ha_thd();
...
int ha_spider::check_access_kind(THD *thd, bool write_request) {
  int error_num, roop_count;
      ...
      for (roop_count = 0; roop_count < (int)share->link_count; roop_count++) {
        if (!write_request) {
          DBUG_PRINT("info", ("spider spider_use_hs_read=%d",
                              spider_param_use_hs_read(
                                  thd, share->use_hs_reads[roop_count])));
          ...
          DBUG_PRINT("info", ("spider spider_use_hs_write=%d",
                              spider_param_use_hs_write(
                                  thd, share->use_hs_writes[roop_count])));
          DBUG_PRINT("info", ("spider use_hs_write[%d]=%ld", roop_count,
                              share->use_hs_writes[roop_count]));
          if (spider_param_use_hs_write(thd,
                                        share->use_hs_writes[roop_count])) {
...
int ha_spider::index_read_map_internal(uchar *buf, const uchar *key,
  ...
  if (sql_kinds & SPIDER_SQL_KIND_HS) {
    if ((error_num = append_limit_hs_part(result_list.internal_offset,
                                          result_list.limit_num,
...
int ha_spider::index_next_same(uchar *buf, const uchar *key, uint keylen) {
  int error_num;
  backup_error_status();
...
#ifdef HA_MRR_USE_DEFAULT_IMPL
#if defined(MARIADB_BASE_VERSION) && MYSQL_VERSION_ID >= 100000
int ha_spider::multi_range_read_next_first(range_id_t *range_info)
#else
...
#if defined(MARIADB_BASE_VERSION) && MYSQL_VERSION_ID >= 100000
    if (!(multi_range_keys = (range_id_t *)spider_malloc(
...
#if defined(MARIADB_BASE_VERSION) && MYSQL_VERSION_ID >= 100000
int ha_spider::multi_range_read_next(range_id_t *range_info)
#else
...
#ifdef HA_MRR_USE_DEFAULT_IMPL
#if defined(MARIADB_BASE_VERSION) && MYSQL_VERSION_ID >= 100000
int ha_spider::multi_range_read_next_next(range_id_t *range_info)
#else
...
#if defined(HS_HAS_SQLCOM) && defined(HAVE_HANDLERSOCKET)
    if (sql_kinds & SPIDER_SQL_KIND_HS) DBUG_RETURN(HA_ERR_WRONG_COMMAND);
#endif
#ifndef WITHOUT_SPIDER_BG_SEARCH
    if ((error_num = spider_set_conn_bg_param(this))) DBUG_RETURN(error_num);
#endif
...
int ha_spider::cmp_ref(const uchar *ref1, const uchar *ref2) {
  int ret = 0;
  ...
  if (table_share->primary_key < MAX_KEY) {
    ...
    for (field = table->field; *field; field++) {
      if ((ret = (*field)->cmp_binary_offset((uint)ptr_diff))) {
        DBUG_PRINT("info",
                   ("spider different at %s", (*field)->field_name.str));
        break;
      }
    }
  }
...
float spider_ft_find_relevance(FT_INFO *handler, uchar *record, uint length) {
...
void spider_ft_close_search(FT_INFO *handler) {
...
int ha_spider::info(uint flag) {
  ...
  */
  ...
  if (flag & (HA_STATUS_TIME | HA_STATUS_CONST | HA_STATUS_VARIABLE |
    ...
    DBUG_PRINT("info", ("spider sts_interval=%f", sts_interval));
...
ulong ha_spider::index_flags(uint idx, uint part, bool all_parts) const {
...
void ha_spider::get_auto_increment(ulonglong offset, ulonglong increment,
...
#else
void ha_spider::start_bulk_insert(ha_rows rows)
#endif
{
...
#ifdef HANDLER_HAS_DIRECT_UPDATE_ROWS
void ha_spider::direct_update_init(THD *thd, bool hs_request) {
#if defined(HS_HAS_SQLCOM) && defined(HAVE_HANDLERSOCKET)
  uint roop_count;
#endif
  ...
  if (hs_request) {
    ...
    for (roop_count = 0; roop_count < share->link_count; roop_count++) {
      if (spider_param_use_hs_write(thd, share->use_hs_writes[roop_count])) {
        DBUG_PRINT("info", ("spider do_hs_direct_update[%d]=TRUE", roop_count));
...
int ha_spider::exec_bulk_update(ha_rows *dup_key_found) {
  int error_num;
  backup_error_status();
...
int ha_spider::bulk_update_row(const uchar *old_data, const uchar *new_data,
...
int ha_spider::direct_update_rows_init(uint mode, KEY_MULTI_RANGE *ranges,
                                       uint range_count, bool sorted,
  ...
  DBUG_PRINT("info",
             ("spider hs_pushed_ret_fields_num=%zu", hs_pushed_ret_fields_num));
#endif
...
#ifdef HANDLER_HAS_DIRECT_UPDATE_ROWS
#ifdef HANDLER_HAS_DIRECT_UPDATE_ROWS_WITH_HS
int ha_spider::direct_delete_rows_init(uint mode, KEY_MULTI_RANGE *ranges,
                                       uint range_count, bool sorted) {
#if defined(HS_HAS_SQLCOM) && defined(HAVE_HANDLERSOCKET)
  int error_num;
#endif
...
double ha_spider::read_time(uint index, uint ranges, ha_rows rows) {
...
bool ha_spider::get_error_message(int error, String *buf) {
  ...
  switch (error) {
    ...
    case ER_SPIDER_COND_INVALID_NUM:
      ...
      buf->q_append(ER_SPIDER_COND_INVALID_STR, ER_SPIDER_COND_INVALID_LEN);
      break;
    ...
    case ER_SPIDER_XA_TIMEOUT_NUM:
      ...
      buf->q_append(ER_SPIDER_XA_TIMEOUT_STR, ER_SPIDER_XA_TIMEOUT_LEN);
...
int ha_spider::rename_table(const char *from, const char *to) {
  ...
  my_hash_value_type from_hash_value =
      my_calc_hash(&spider_open_tables, (uchar *)from, from_len);
  my_hash_value_type to_hash_value =
      my_calc_hash(&spider_open_tables, (uchar *)to, to_len);
#endif
  THD *thd = ha_thd();
  ...
  if (sql_command == SQLCOM_RENAME_TABLE ||
      ...
      !(alter_table_from = (SPIDER_ALTER_TABLE *)my_hash_search(
            &trx->trx_alter_table_hash, (uchar *)from, from_len))) {
    if (!(table_tables = spider_open_sys_table(
              current_thd, SPIDER_SYS_TABLES_TABLE_NAME_STR,
              SPIDER_SYS_TABLES_TABLE_NAME_LEN, TRUE, &open_tables_backup,
              FALSE, &error_num))) {
    ...
    if ((alter_table_to = (SPIDER_ALTER_TABLE *)my_hash_search(
             &trx->trx_alter_table_hash, (uchar *)to, to_len))) {
...
int ha_spider::delete_table(const char *name) {
  int error_num;
  THD *thd = ha_thd();
  SPIDER_TRX *trx;
  TABLE *table_tables = NULL;
...
#ifdef SPIDER_HAS_HASH_VALUE_TYPE
    my_hash_value_type hash_value =
        my_calc_hash(&spider_open_tables, (uchar *)name, name_len);
#endif
    if (sql_command == SQLCOM_ALTER_TABLE &&
#ifdef SPIDER_HAS_HASH_VALUE_TYPE
        (alter_table = (SPIDER_ALTER_TABLE *)my_hash_search_using_hash_value(
             &trx->trx_alter_table_hash, hash_value, (uchar *)name,
             name_len)) &&
#else
...
#else
    lgtm_tblhnd_share =
        spider_get_lgtm_tblhnd_share(name, name_len, TRUE, FALSE, &error_num);
#endif
...
int ha_spider::disable_indexes(uint mode) {
  int error_num;
  backup_error_status();
...
int ha_spider::enable_indexes(uint mode) {
  int error_num;
  backup_error_status();
...
int ha_spider::check(THD *thd, HA_CHECK_OPT *check_opt) {
  int error_num;
  backup_error_status();
...
int ha_spider::repair(THD *thd, HA_CHECK_OPT *check_opt) {
  int error_num;
  backup_error_status();
...
bool ha_spider::check_and_repair(THD *thd) {
...
int ha_spider::analyze(THD *thd, HA_CHECK_OPT *check_opt) {
  int error_num;
  backup_error_status();
...
int ha_spider::optimize(THD *thd, HA_CHECK_OPT *check_opt) {
  int error_num;
  backup_error_status();
...
bool ha_spider::is_fatal_error(int error_num, uint flags) {
  DBUG_ENTER("ha_spider::is_fatal_error");
  DBUG_PRINT("info", ("spider error_num=%d", error_num));
  DBUG_PRINT("info", ("spider flags=%u", flags));
  if (!handler::is_fatal_error(error_num, flags)) {
...
int ha_spider::info_push(uint info_type, void *info) {
  int error_num = 0;
  ...
  switch (info_type) {
    ...
    case INFO_KIND_HS_RET_FIELDS: {
      ...
      if (hs_pushed_ret_fields_size < hs_pushed_ret_fields_num) {
        if (hs_pushed_ret_fields)
          spider_free(spider_current_trx, hs_pushed_ret_fields, MYF(0));
        if (!(hs_pushed_ret_fields = (uint32 *)spider_bulk_malloc(
                  spider_current_trx, 17, MYF(MY_WME), &hs_pushed_ret_fields,
                  sizeof(uint32) * hs_pushed_ret_fields_num, NullS))) {
          DBUG_RETURN(HA_ERR_OUT_OF_MEM);
        }
      ...
      for (roop_count = 0; roop_count < hs_pushed_ret_fields_num;
           roop_count++) {
    ...
    case INFO_KIND_HS_APPEND_STRING_REF: {
      ...
      Field *field;
      if (hs_pushed_ret_fields) {
        field = get_top_table_field(hs_pushed_ret_fields[hs_pushed_strref_num]);
      } else {
    ...
    case INFO_KIND_BULK_ACCESS_BEGIN:
...
#ifdef HA_CAN_BULK_ACCESS
#if defined(HS_HAS_SQLCOM) && defined(HAVE_HANDLERSOCKET)
      memset(bulk_access_link_current->spider->result_list.hs_r_bulk_open_index,
             0, share->link_bitmap_size);
      memset(bulk_access_link_current->spider->result_list.hs_w_bulk_open_index,
             0, share->link_bitmap_size);
#endif
#endif
      /*
      #if defined(HS_HAS_SQLCOM) && defined(HAVE_HANDLERSOCKET)
            if ((error_num =
      bulk_access_link_current->spider->reset_hs_strs_pos(
      ...
      if (bulk_access_link_exec_tgt->spider->pt_clone_source_handler != this) {
        ...
        for (bulk_access_link_exec_tgt = bulk_access_link_first;
          ...
          if (bulk_access_link_exec_tgt->sequence_num >= sequence_num) {
            DBUG_ASSERT(bulk_access_link_exec_tgt->sequence_num ==
                        sequence_num);
            break;
          }
        }
...
bool ha_spider::handler_opened(int link_idx, uint tgt_conn_kind) {
...
void ha_spider::clear_handler_opened(int link_idx, uint tgt_conn_kind) {
...
int ha_spider::close_opened_handler(int link_idx, bool release_conn) {
  ...
  if (spider_bit_is_set(r_handler_opened, link_idx)) {
    ...
    if ((error_num2 = spider_db_close_handler(
             this, hs_r_conns[link_idx], link_idx, SPIDER_CONN_KIND_HS_READ))) {
      if (share->monitoring_kind[link_idx] && need_mons[link_idx]) {
        error_num2 = spider_ping_table_mon_from_table(
            trx, trx->thd, share, link_idx,
            (uint32)share->monitoring_sid[link_idx], share->table_name,
    ...
    spider_clear_bit(r_handler_opened, link_idx);
    if (release_conn) {
      if (!hs_r_conns[link_idx]->opened_handlers &&
  ...
  if (spider_bit_is_set(w_handler_opened, link_idx)) {
    if ((error_num2 =
             spider_db_close_handler(this, hs_w_conns[link_idx], link_idx,
    ...
    spider_clear_bit(w_handler_opened, link_idx);
    if (release_conn) {
      if (!hs_w_conns[link_idx]->opened_handlers &&
...
#endif
#endif
        if ((error_num = spider_db_open_handler(
                 this,
#if defined(HS_HAS_SQLCOM) && defined(HAVE_HANDLERSOCKET)
                 (tmp_conn_kind1 == SPIDER_CONN_KIND_MYSQL
                      ...
                      : tmp_conn_kind1 == SPIDER_CONN_KIND_HS_READ
                            ? hs_r_conns[roop_count]
                            : hs_w_conns[roop_count])
#endif
                     ,
                 roop_count))) {
          if (share->monitoring_kind[roop_count] && need_mons[roop_count]) {
...
#if defined(HS_HAS_SQLCOM) && defined(HAVE_HANDLERSOCKET)
int ha_spider::reset_hs_sql(ulong sql_type) {
  int error_num;
  uint roop_count, dbton_id;
...
int ha_spider::reset_hs_keys(ulong sql_type) {
  int error_num;
  uint roop_count, dbton_id;
...
int ha_spider::reset_hs_upds(ulong sql_type) {
  int error_num;
  uint roop_count, dbton_id;
...
int ha_spider::reset_hs_strs(ulong sql_type) {
  int error_num;
  uint roop_count, dbton_id;
...
int ha_spider::reset_hs_strs_pos(ulong sql_type) {
  int error_num;
  uint roop_count, dbton_id;
...
int ha_spider::push_back_hs_upds(SPIDER_HS_STRING_REF &info) {
  int error_num;
  uint roop_count, dbton_id;
...
#if defined(HS_HAS_SQLCOM) && defined(HAVE_HANDLERSOCKET)
int ha_spider::append_key_where_hs_part(const key_range *start_key,
                                        const key_range *end_key,
                                        ulong sql_type) {
  int error_num;
  uint roop_count, dbton_id;
...
#if defined(HS_HAS_SQLCOM) && defined(HAVE_HANDLERSOCKET)
int ha_spider::append_limit_hs_part(longlong offset, longlong limit,
                                    ulong sql_type) {
  int error_num;
  uint roop_count, dbton_id;
  spider_db_handler *dbton_hdl;
...
#if defined(HS_HAS_SQLCOM) && defined(HAVE_HANDLERSOCKET)
int ha_spider::append_insert_values_hs_part(ulong sql_type) {
  int error_num;
  uint roop_count, dbton_id;
  spider_db_handler *dbton_hdl;
...
bool ha_spider::is_bulk_insert_exec_period(bool bulk_end) {
  ...
  if (sql_kinds & SPIDER_SQL_KIND_SQL) {
#endif
    for (roop_count = 0; roop_count < share->use_sql_dbton_count;
         roop_count++) {
      dbton_id = share->use_sql_dbton_ids[roop_count];
      dbton_hdl = dbton_handler[dbton_id];
      if (dbton_hdl->first_link_idx >= 0 &&
...
SPIDER_CONN *ha_spider::spider_get_conn_by_idx(int link_idx) {
  ...
  if (this && this->conns && this->conns[link_idx]) {
    ...
    if (!this->conns[link_idx]) { /* failed to create conn */
      ...
      //0); 			share->init = TRUE; 			spider_free_share(share);
