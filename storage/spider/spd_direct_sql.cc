...
int spider_udf_direct_sql_create_table_list(SPIDER_DIRECT_SQL *direct_sql,
  ...
  if (!(direct_sql->db_names = (char **)spider_bulk_malloc(
            ...
            sizeof(char) * (table_name_list_length +
                            thd->db_length * table_count + 2 * table_count),
            &direct_sql->iop, sizeof(int) * table_count, NullS)))
#else
  if (!(direct_sql->db_names = (char **)spider_bulk_malloc(
            spider_current_trx, 31, MYF(MY_WME | MY_ZEROFILL),
            &direct_sql->db_names, sizeof(char *) * table_count,
...
int spider_udf_direct_sql_create_conn_key(SPIDER_DIRECT_SQL *direct_sql) {
  ...
  if (direct_sql->access_mode == 0) {
    ...
    direct_sql->conn_key_length =
        1 + direct_sql->tgt_wrapper_length + 1 + direct_sql->tgt_host_length +
        1 + 5 + 1 + direct_sql->tgt_socket_length + 1 +
        ...
        direct_sql->tgt_ssl_cipher_length + 1 + direct_sql->tgt_ssl_key_length +
        1 + 1 + 1 + direct_sql->tgt_default_file_length + 1 +
        direct_sql->tgt_default_group_length;
#if defined(HS_HAS_SQLCOM) && defined(HAVE_HANDLERSOCKET)
  } else {
    direct_sql->conn_key_length = 1 + direct_sql->tgt_wrapper_length + 1 +
                                  direct_sql->tgt_host_length + 1 + 5 + 1 +
...
SPIDER_CONN *spider_udf_direct_sql_create_conn(
  ...
  if (direct_sql->access_mode == 0) {
    ...
    if (!(conn = (SPIDER_CONN *)spider_bulk_malloc(
              spider_current_trx, 33, MYF(MY_WME | MY_ZEROFILL), &conn,
              sizeof(*conn), &tmp_name, direct_sql->conn_key_length + 1,
              &tmp_host, direct_sql->tgt_host_length + 1, &tmp_socket,
              direct_sql->tgt_socket_length + 1, &tmp_wrapper,
              direct_sql->tgt_wrapper_length + 1, &need_mon, sizeof(int),
              NullS))) {
      *error_num = HA_ERR_OUT_OF_MEM;
      goto error_alloc_conn;
    }
...
#if defined(HS_HAS_SQLCOM) && defined(HAVE_HANDLERSOCKET)
  DBUG_PRINT("info",
             ("spider direct_sql->access_mode=%d", direct_sql->access_mode));
#endif
...
#endif
  {
    if (
      ...
      if (!(conn = (SPIDER_CONN *)my_hash_search(&spider_open_connections,
...
#endif
        {
          ...
          if (!(conn =
                    spider_udf_direct_sql_create_conn(direct_sql, error_num)))
            goto error;
        } else {
    ...
    if (direct_sql->access_mode == 0) {
      ...
      if (trx->trx_direct_hs_r_conn_hash.array.max_element > old_elements) {
        spider_alloc_calc_mem(
            spider_current_trx, trx->trx_direct_hs_r_conn_hash,
      ...
      if (trx->trx_direct_hs_w_conn_hash.array.max_element > old_elements) {
        spider_alloc_calc_mem(
            spider_current_trx, trx->trx_direct_hs_w_conn_hash,
...
int spider_udf_direct_sql_get_server(SPIDER_DIRECT_SQL *direct_sql) {
  ...
  if (!(server = get_server_by_name(&mem_root, direct_sql->server_name,
                                    &server_buf))) {
  ...
  if (!direct_sql->tgt_wrapper && server->scheme) {
    ...
    if (!(direct_sql->tgt_wrapper = spider_create_string(
              server->scheme, direct_sql->tgt_wrapper_length))) {
      error_num = HA_ERR_OUT_OF_MEM;
  ...
  if (!direct_sql->tgt_host && server->host) {
    ...
    if (!(direct_sql->tgt_host = spider_create_string(
              server->host, direct_sql->tgt_host_length))) {
      error_num = HA_ERR_OUT_OF_MEM;
      my_error(ER_OUT_OF_RESOURCES, MYF(0), HA_ERR_OUT_OF_MEM);
      goto error;
    }
  ...
  if (!direct_sql->tgt_socket && server->socket) {
    ...
    if (!(direct_sql->tgt_socket = spider_create_string(
              server->socket, direct_sql->tgt_socket_length))) {
      error_num = HA_ERR_OUT_OF_MEM;
      my_error(ER_OUT_OF_RESOURCES, MYF(0), HA_ERR_OUT_OF_MEM);
      goto error;
    }
  ...
  if (!direct_sql->tgt_username && server->username) {
    ...
    if (!(direct_sql->tgt_username = spider_create_string(
              server->username, direct_sql->tgt_username_length))) {
      error_num = HA_ERR_OUT_OF_MEM;
      my_error(ER_OUT_OF_RESOURCES, MYF(0), HA_ERR_OUT_OF_MEM);
      goto error;
    }
  ...
  if (!direct_sql->tgt_password && server->password) {
    ...
    if (!(direct_sql->tgt_password = spider_create_string(
              server->password, direct_sql->tgt_password_length))) {
      error_num = HA_ERR_OUT_OF_MEM;
      my_error(ER_OUT_OF_RESOURCES, MYF(0), HA_ERR_OUT_OF_MEM);
      goto error;
    }
  ...
  while (sprit_ptr[0]) {
    ...
    switch (title_length) {
      ...
      case 17:
        ...
        goto error;
      case 18:
        SPIDER_PARAM_INT_WITH_MAX("connection_channel", connection_channel, 0,
                                  63);
        error_num = param_string_parse.print_param_error();
        goto error;
      case 22:
...
int spider_udf_set_direct_sql_param_default(SPIDER_TRX *trx,
...
#if defined(HS_HAS_SQLCOM) && defined(HAVE_HANDLERSOCKET)
  if (direct_sql->access_mode == -1) direct_sql->access_mode = 0;
#endif
  ...
  if (
      ...
      direct_sql->access_mode == 0 &&
#endif
      !direct_sql->tgt_socket && !strcmp(direct_sql->tgt_host, my_localhost)) {
    DBUG_PRINT("info", ("spider create default tgt_socket"));
    direct_sql->tgt_socket_length = strlen((char *)MYSQL_UNIX_ADDR);
    if (!(direct_sql->tgt_socket = spider_create_string(
              (char *)MYSQL_UNIX_ADDR, direct_sql->tgt_socket_length))) {
      my_error(ER_OUT_OF_RESOURCES, MYF(0), HA_ERR_OUT_OF_MEM);
      DBUG_RETURN(HA_ERR_OUT_OF_MEM);
    }
  }
  ...
  if (direct_sql->connection_channel == -1) direct_sql->connection_channel = 0;
#if MYSQL_VERSION_ID < 50500
#else
