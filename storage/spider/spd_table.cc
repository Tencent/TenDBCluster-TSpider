...
int spider_create_conn_keys(
  ...
  for (roop_count = 0; roop_count < (int) share->all_link_count; roop_count++)
  {
    /* tgt_db not use */
	  ...
	  share->conn_keys_charlen += conn_keys_lengths[roop_count] + 23; /*pre-allocate length for version-num , max length of uint64 is 20*/
#if defined(HS_HAS_SQLCOM) && defined(HAVE_HANDLERSOCKET)
...
int spider_update_conn_keys(
	...
	ulong server_version = get_server_version_by_name(share->server_names[link_idx]);
	pthread_mutex_lock(&spider_tbl_mutex);
	conn_key_version = share->conn_key_version;
	if (server_version != conn_key_version)
	{/* update share conn key with version */
...
SPIDER_SHARE *spider_create_share(
  ...
  if (!(share = (SPIDER_SHARE *)
    spider_bulk_malloc(spider_current_trx, 46, MYF(MY_WME | MY_ZEROFILL),
      &share, sizeof(*share),
      &tmp_name, length + 1,
      &tmp_name_with_version, length_with_version + 1,
...
#ifdef SPIDER_HAS_HASH_VALUE_TYPE
  share->table_name_hash_value = my_calc_hash(&spider_open_tables, (uchar*)share->table_name, share->table_name_length);
#ifdef WITH_PARTITION_STORAGE_ENGINE
...
SPIDER_SHARE *spider_get_share(
  ...
  else
  {
      ...
      strmov(strmov(strmov(table_name_with_version, table_name), "#"), ts_version);
  }
  length = (uint)strlen(table_name_with_version);
#ifdef SPIDER_HAS_HASH_VALUE_TYPE
  my_hash_value_type hash_value = my_calc_hash(&spider_open_tables,
    (uchar*)table_name_with_version, length);
#endif
  pthread_mutex_lock(&spider_tbl_mutex);
#ifdef SPIDER_HAS_HASH_VALUE_TYPE
  if (!(share = (SPIDER_SHARE*) my_hash_search_using_hash_value(
    &spider_open_tables, hash_value, (uchar*)table_name_with_version, length)))
#else
...
#endif
  {
    if (!(share = spider_create_share(
      table_name, table_name_with_version, table_share,
#ifdef WITH_PARTITION_STORAGE_ENGINE
      table->part_info,
#endif
#ifdef SPIDER_HAS_HASH_VALUE_TYPE
...
*/
    ...
    if (
      !(conn = spider_get_conn(
        &tmp_share, 0, trx, NULL, FALSE, FALSE,
    ...
    if (lock && spider_param_use_snapshot_with_flush_tables(thd) == 2)
    {
      ...
      if (
        (!(conn = spider_get_conn(
        &tmp_share, 0, trx, spider, TRUE, FALSE,
        SPIDER_CONN_KIND_MYSQL, &error_num)))
      ) {
...
*/
  ...
  spider_alloc_calc_mem(NULL,
    ...
    spider_lgtm_tblhnd_share_hash.array.size_of_element);
  if(
    my_hash_init(&spider_open_connections, spd_charset_utf8_bin, 256, 0, 0,
  ...
  if (
      my_hash_init(&spider_for_sts_conns, spd_charset_utf8_bin, 32, 0, 0,
      (my_hash_get_key)spider_for_sts_conn_get_key, spider_free_for_sts_conn, 0)
      ) {
      error_num = HA_ERR_OUT_OF_MEM;
  ...
  if (
    ...
    if (direct_order_limit)
    {
      ...
      if (
        ...
        select_lex->having ||
        (!opt_spider_direct_limit_in_group && select_lex->group_list.elements) || 
...
int spider_discover_table_structure(
  ...
  if (!part_info)
  {
      ...
      {
          strmov(strmov(strmov(tmp_name_with_version, table_name), "#"), ts_version);
      }
    if (!(spider_share = spider_create_share(table_name, tmp_name_with_version, share,
#ifdef WITH_PARTITION_STORAGE_ENGINE
      NULL,
#endif
#ifdef SPIDER_HAS_HASH_VALUE_TYPE
    ...
    while ((part_elem = part_it++))
    {
      if ((part_elem)->subpartitions.elements)
      {
        ...
        while ((sub_elem = sub_it++))
        {
          ...
          if (!(spider_share = spider_create_share(tmp_name, tmp_name_with_version, share,
            part_info,
#ifdef SPIDER_HAS_HASH_VALUE_TYPE
            hash_value,
#endif
...
)
{
    ...
    if (
        !(status_table = spider_open_sys_table(
            current_thd, SPIDER_SYS_TABLE_STATUS_NAME_STR,
    ...
    if (
        (error_num = spider_replace_table_status(status_table,
            table_name,
            table_name_length,
            tgt_table_names,
            tgt_dbs,
    ...
    spider_close_sys_table(current_thd, status_table,
        &open_tables_backup, FALSE);
    status_table = NULL;
