...
template <size_t N>
inline bool operator==(const string_ref &x, const char (&y)[N]) {
...
struct string_ref_list_wrap {
  string_ref_list_wrap() {
    if (SPD_INIT_DYNAMIC_ARRAY2(&string_ref_list, sizeof(string_ref), NULL, 16,
                                16, MYF(MY_WME)))
  ...
  size_t size() { return string_ref_list_init ? string_ref_list.elements : 0; }
  bool resize(size_t new_size) {
    if (string_ref_list_init) {
      if (string_ref_list.max_element < new_size &&
          allocate_dynamic(&string_ref_list, new_size))
        return TRUE;
  ...
  bool empty() {
    return string_ref_list_init ? string_ref_list.elements ? FALSE : TRUE
                                : TRUE;
  }
...
inline String *q_append_str(String *str, const char *p) {
  uint32 p_len = strlen(p);
  if (str->reserve(p_len)) return NULL;
  str->q_append(p, p_len);
  return str;
}
