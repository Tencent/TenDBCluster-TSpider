        ...
        if (my_hash_insert(&spider_open_connections, (uchar *)conn)) {
          ...
          if (spider_open_connections.array.max_element > old_elements) {
            spider_alloc_calc_mem(
                spider_current_trx, spider_open_connections,
...
SPIDER_CONN *spider_create_conn(SPIDER_SHARE *share, ha_spider *spider,
  ...
  */
  ...
  SPIDER_CONN *spider_get_conn(SPIDER_SHARE * share, int link_idx,
...
#ifdef SPIDER_HAS_HASH_VALUE_TYPE
    if ((another && !(conn = (SPIDER_CONN *)my_hash_search_using_hash_value(
...
#endif
    {
      if (!trx->thd || ((spider_param_conn_recycle_mode(trx->thd) & 1) ||
        ...
        if (!(conn = (SPIDER_CONN *)my_hash_search_using_hash_value(
                  ...
                  share->conn_keys_lengths[link_idx])))
#else
      if (!(conn = (SPIDER_CONN *)my_hash_search(
...
#endif
        {
          ...
          if (spider_param_max_connections()) { /* enable connection pool */
            ...
            if (!(conn =
                      spider_create_conn(share, spider, link_idx, base_link_idx,
                                         conn_kind, error_num)))
              goto error;
  ...
  void spider_conn_queue_connect_rewrite(SPIDER_SHARE * share,
  ...
  void spider_conn_set_timeout_from_share(SPIDER_CONN * conn, int link_idx,
    ...
    spider_conn_set_timeout(
        ...
        spider_param_net_write_timeout(thd,
  ...
  int spider_set_conn_bg_param(ha_spider * spider) {
    ...
    if (bgs_mode == 0)
      ...
      DBUG_PRINT("info",
                 ("spider bgs_second_read=%lld", share->bgs_second_read));
...
#if defined(HS_HAS_SQLCOM) && defined(HAVE_HANDLERSOCKET)
          if ((error_num =
                   spider_create_conn_thread(spider->hs_r_conns[roop_count])))
            DBUG_RETURN(error_num);
          if ((error_num =
                   spider_create_conn_thread(spider->hs_w_conns[roop_count])))
            DBUG_RETURN(error_num);
#endif
        }
#ifdef SPIDER_HAS_GROUP_BY_HANDLER
...
#else
      if (mysql_mutex_init(spd_key_mutex_bg_job_stack,
                           &conn->bg_job_stack_mutex, MY_MUTEX_INIT_FAST))
#endif
      {
        error_num = HA_ERR_OUT_OF_MEM;
    ...
    if (result_list->bgs_phase > 0) {
      ...
      for (roop_count = roop_start; roop_count < roop_end;
           roop_count = spider_conn_link_idx_next(
               ...
               share->link_count, SPIDER_LINK_STATUS_RECOVERY)) {
        if ((error_num =
                 spider_bg_conn_search(spider, roop_count, roop_start, TRUE,
                                       TRUE, (roop_count != link_ok)))) {
  ...
  int spider_bg_conn_search(ha_spider * spider, int link_idx,
                            int first_link_idx, bool first, bool pre_next,
                            bool discard_result, ulong sql_type) {
    int error_num;
  ...
  void spider_bg_conn_simple_action(SPIDER_CONN * conn, uint simple_action,
                                    bool caller_wait, void *target,
                                    uint link_idx, int *error_num) {
    ...
    while (TRUE) {
      ...
      if (conn->bg_search) {
        ...
        if (result_list->quick_mode == 0 || result_list->bgs_phase == 1 ||
              ...
              if (!(result_list->bgs_error =
                ...
                if (!result_list->bgs_error) {
                  ...
                  if (dbton_handler->execute_sql(
                    ...
                    spider->connection_ids[conn->link_idx] =
                        conn->connection_id;
          ...
          spider->connection_ids[conn->link_idx] = conn->connection_id;
          conn->mta_conn_mutex_unlock_later = TRUE;
          result_list->bgs_error = spider_db_store_result(
              spider, conn->link_idx, result_list->table);
          if ((result_list->bgs_error_with_message = thd->is_error()))
            strmov(result_list->bgs_error_msg, spider_stmt_da_message(thd));
          conn->mta_conn_mutex_unlock_later = FALSE;
  ...
  void *spider_bg_sts_action(void *arg) {
    ...
    while (TRUE) {
      ...
      if (share->bg_sts_kill) {
        ...
        for (roop_count = SPIDER_DBTON_SIZE - 1; roop_count >= 0;
             --roop_count) {
          if (spider_bit_is_set(share->dbton_bitmap, roop_count) &&
              dbton_hdl[roop_count]) {
            delete dbton_hdl[roop_count];
  ...
  int spider_create_mon_threads(SPIDER_TRX * trx, SPIDER_SHARE * share) {
    ...
    if (!share->bg_mon_init) {
      ...
      if (create_bg_mons) {
        ...
        char *buf = (char *)my_alloca(share->table_name_length +
  ...
  bool spider_conn_use_handler(ha_spider * spider, int lock_mode,
                               int link_idx) {
    THD *thd = spider->trx->thd;
    int use_handler =
  ...
  bool spider_conn_need_open_handler(ha_spider * spider, uint idx,
                                     int link_idx) {
#if defined(HS_HAS_SQLCOM) && defined(HAVE_HANDLERSOCKET)
    SPIDER_CONN *conn;
#endif
    ...
    if (spider->handler_opened(link_idx, spider->conn_kind[link_idx])) {
      ...
      if (spider->do_direct_update &&
        ...
        DBUG_PRINT("info", ("spider r_handler_index[%d]=%d", link_idx,
                            spider->r_handler_index[link_idx]));
        DBUG_PRINT("info", ("spider idx=%d", idx));
        DBUG_PRINT("info", ("spider hs_pushed_ret_fields_num=%zu",
                            spider->hs_pushed_ret_fields_num));
        DBUG_PRINT("info", ("spider hs_r_ret_fields_num[%d]=%lu", link_idx,
                            spider->hs_r_ret_fields_num[link_idx]));
        DBUG_PRINT("info", ("spider hs_r_ret_fields[%d]=%p", link_idx,
        ...
        if (spider->hs_pushed_ret_fields_num < MAX_FIELDS &&
          ...
          int roop_count;
          for (roop_count = 0;
               roop_count < (int)spider->hs_pushed_ret_fields_num;
               ++roop_count) {
            DBUG_PRINT("info",
                       ("spider hs_pushed_ret_fields[%d]=%u", roop_count,
                        spider->hs_pushed_ret_fields[roop_count]));
            DBUG_PRINT("info", ("spider hs_r_ret_fields[%d][%d]=%u", link_idx,
                                roop_count,
                                spider->hs_r_ret_fields[link_idx][roop_count]));
          }
        }
#endif
        if (spider->r_handler_index[link_idx] == idx
#ifdef HANDLER_HAS_DIRECT_UPDATE_ROWS
          ...
          DBUG_PRINT("info",
                     ("spider conn->connection_id=%llu", conn->connection_id));
          DBUG_PRINT("info", ("spider conn->server_lost=%s",
                              conn->server_lost ? "TRUE" : "FALSE"));
          DBUG_PRINT("info",
                     ("spider conn->hs_pre_age=%llu", conn->hs_pre_age));
          DBUG_PRINT("info", ("spider hs_w_conn_ages[%d]=%llu", link_idx,
                              spider->hs_w_conn_ages[link_idx]));
          if (!conn->server_lost &&
        ...
        DBUG_PRINT("info", ("spider w_handler_index[%d]=%d", link_idx,
                            spider->w_handler_index[link_idx]));
        DBUG_PRINT("info", ("spider idx=%d", idx));
        DBUG_PRINT("info", ("spider hs_pushed_ret_fields_num=%zu",
                            spider->hs_pushed_ret_fields_num));
        DBUG_PRINT("info", ("spider hs_w_ret_fields_num[%d]=%lu", link_idx,
                            spider->hs_w_ret_fields_num[link_idx]));
        DBUG_PRINT("info", ("spider hs_w_ret_fields[%d]=%p", link_idx,
        ...
        if (spider->hs_pushed_ret_fields_num < MAX_FIELDS &&
          ...
          for (roop_count = 0;
            ...
            DBUG_PRINT("info",
                       ("spider hs_pushed_ret_fields[%d]=%u", roop_count,
                        spider->hs_pushed_ret_fields[roop_count]));
            DBUG_PRINT("info", ("spider hs_w_ret_fields[%d][%d]=%u", link_idx,
                                roop_count,
                                spider->hs_w_ret_fields[link_idx][roop_count]));
          }
        }
#endif
        if (spider->w_handler_index[link_idx] == idx
#ifdef HANDLER_HAS_DIRECT_UPDATE_ROWS
            && ((spider->hs_pushed_ret_fields_num == MAX_FIELDS &&
  ...
  if ((ip_port_conn = (SPIDER_IP_PORT_CONN *)my_hash_search(
    ...
    if (ip_port_conn && ip_port_count >= spider_max_connections &&
      ...
      while (1) {
        ...
        error = pthread_cond_timedwait(&ip_port_conn->cond,
                                       &ip_port_conn->mutex, &abstime);
        --ip_port_conn->waiting_count;
        pthread_mutex_unlock(&ip_port_conn->mutex);
  ...
  static void my_polling_last_visited(uchar * entry, void *data) {
    ...
    if (conn) {
      ...
      my_hash_value_type hash_value = my_calc_hash(
          spider_open_connections, conn->conn_key, conn->conn_key_length);
  ...
  static void *spider_conn_recycle_action(void *arg) {
    ...
    while (conn_rcyc_init) {
      ...
      pthread_mutex_lock(&spider_conn_mutex);
      my_hash_delegate(&spider_open_connections, my_polling_last_visited,
                       &param);
      pthread_mutex_unlock(&spider_conn_mutex);
      ...
      for (size_t i = 0; i < idle_conn_key_hash_value_arr.cur_idx; ++i) {
        ...
        if (get_dynamic_string_array(&idle_conn_key_arr, &conn_key,
                                     &conn_key_len, i)) {
          spider_my_err_logging(
        ...
        SPIDER_CONN *conn = (SPIDER_CONN *)my_hash_search_using_hash_value(
            &spider_open_connections, conn_key_hash_value, (uchar *)conn_key,
            conn_key_len);

#else
      SPIDER_CONN *conn = (SPIDER_CONN *)my_hash_search(
          &spider_open_connections, conn_key, conn_key_len);
#endif
        if (conn) {
          my_hash_delete(&spider_open_connections, (uchar *)conn);
        }
...
#if MYSQL_VERSION_ID < 50500
    if (pthread_create(&conn_rcyc_thread, NULL, spider_conn_recycle_action,
                       NULL))
#else
  ...
  SPIDER_CONN_META_INFO *spider_create_conn_meta(SPIDER_CONN * conn) {
    ...
    if (conn) {
      SPIDER_CONN_META_INFO *ret = (SPIDER_CONN_META_INFO *)my_malloc(
          sizeof(*ret), MY_ZEROFILL | MY_WME);
      if (!ret) {
        goto err_return_direct;
      }
  ...
  */
    ...
    while (get_status_init) {
      ...
      for (ulong i = 0; (i < share_records) && get_status_init;
        ...
        if (difftime(cur_time, modify_time) >= modify_interval &&
          ...
          sts_conn = (SPIDER_FOR_STS_CONN *)my_hash_search(
              &spider_for_sts_conns, (uchar *)key, key_len);
...
#if MYSQL_VERSION_ID < 50500
    if (pthread_create(&get_status_thread, NULL, spider_get_status_action,
                       NULL))
#else
  ...
  uint spider_create_sts_conn_key(char *key, char *host, ulong port, char *user,
    ...
    key_length =
        (uint)(
            strmov(
                strmov(strmov(strmov(key, host) + 1, port_str) + 1, user) + 1,
                passwd) -
            key) +
        1;
  ...
  SPIDER_FOR_STS_CONN *spider_create_sts_conn(char *key, ulong key_len,
    ...
    sts_conn = (SPIDER_FOR_STS_CONN *)my_malloc(sizeof(*sts_conn),
                                                MY_ZEROFILL | MY_WME);
    if (!sts_conn) {
      DBUG_RETURN(NULL);
    }
  ...
  void spider_free_conn_by_servername(char *servername) {
    ...
    while (conn = (SPIDER_CONN *)my_hash_search(
               &spider_open_connections, (uchar *)conn_key, conn_key_len)) {
      my_hash_delete(&spider_open_connections, (uchar *)conn);
      pthread_mutex_unlock(&spider_conn_mutex);
      spider_free_conn(conn);
      pthread_mutex_lock(&spider_conn_mutex);
