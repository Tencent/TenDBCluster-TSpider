...
int spider_xa_lock(XID_STATE *xid_state) {
  ...
  my_hash_value_type hash_value =
      my_calc_hash(&spd_db_att_xid_cache, (uchar *)xid_state->xid.key(),
...
#endif
#else
  if (my_hash_search(&spd_db_att_xid_cache, xid_state->xid.key(),
                     xid_state->xid.key_length()))
#endif
  {
...
int spider_internal_start_trx(ha_spider *spider, SPIDER_CONN *conn,
  ...
  if (!trx->trx_start) {
    ...
    if (!trx->trx_xa && trx->internal_xa &&
      ...
      trx->trx_xa = TRUE;
      trx->xid.formatID = opt_spider_auto_increment_mode_value
      ...
      if (spider_param_internal_xa_id_type(thd) == 0) {
        ...
        trx->xid.gtrid_length =
            my_sprintf(trx->xid.data, (trx->xid.data, "%lx%016llx",
                                       thd_get_thread_id(thd), thd->query_id));
      }
#if defined(MARIADB_BASE_VERSION) && MYSQL_VERSION_ID >= 100002
      trx->xid.bqual_length = my_sprintf(trx->xid.data + trx->xid.gtrid_length,
                                         (trx->xid.data + trx->xid.gtrid_length,
                                          "%lx", thd->variables.server_id));
#else
...
int spider_internal_xa_prepare(THD *thd, SPIDER_TRX *trx, TABLE *table_xa,
  ...
  if (internal_xa) trx->internal_xid_state.xa_state = XA_PREPARED;
  DBUG_RETURN(0);
...
int spider_internal_xa_commit_by_xid(THD *thd, SPIDER_TRX *trx, XID *xid) {
  ...
  */
  ...
  do {
    ...
    if ((error_num = spider_db_xa_commit(conn, xid)) &&
        (force_commit == 0 ||
         (force_commit == 1 && error_num != ER_XAER_NOTA))) {
      SPIDER_CONN_RESTORE_DASTATUS_AND_RESET_ERROR_NUM;
      if (error_num) {
...
int spider_internal_xa_rollback_by_xid(THD *thd, SPIDER_TRX *trx, XID *xid) {
  ...
  */
  ...
  do {
    ...
    conn->error_mode &= spider_param_error_read_mode(thd, 0);
    conn->error_mode &= spider_param_error_write_mode(thd, 0);
    if ((error_num = spider_db_xa_rollback(conn, xid)) &&
        (force_commit == 0 ||
         (force_commit == 1 && error_num != ER_XAER_NOTA))) {
      SPIDER_CONN_RESTORE_DASTATUS_AND_RESET_ERROR_NUM;
      if (error_num) {
...
THD *spider_create_tmp_thd() {
  ...
  if (thd->store_globals()) DBUG_RETURN(NULL);
  lex_start(thd);
  DBUG_RETURN(thd);
}
