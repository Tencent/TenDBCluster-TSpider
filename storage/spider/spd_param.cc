...
#endif
#ifdef SPIDER_HAS_SHOW_SIMPLE_FUNC
    {"Spider_direct_order_limit", (char *)&spider_direct_order_limit,
     SHOW_SIMPLE_FUNC},
    {"Spider_direct_aggregate", (char *)&spider_direct_aggregate,
     SHOW_SIMPLE_FUNC},
    {"Spider_parallel_search", (char *)&spider_parallel_search,
     SHOW_SIMPLE_FUNC},
#else
    {"Spider_direct_order_limit", (char *)&spider_direct_order_limit,
     SHOW_FUNC},
...
static my_bool spider_support_xa = 0;
static MYSQL_SYSVAR_BOOL(support_xa, spider_support_xa,
...
static MYSQL_SYSVAR_BOOL(connect_mutex, spider_connect_mutex,
...
static MYSQL_SYSVAR_UINT(
    connect_error_interval, spider_connect_error_interval,
 ...
 */
static MYSQL_SYSVAR_UINT(
    table_init_error_interval, spider_table_init_error_interval,
    PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_NOSYSVAR,
...
/*
 -1 :use table parameter
  ...
  1 :use table charset
 */
static MYSQL_SYSVAR_INT(use_table_charset, spider_use_table_charset,
...
/*
  ...
  2: recycle in thread
 */
static MYSQL_THDVAR_UINT(conn_recycle_mode,         /* name */
...
/*
  ...
  1: strict
 */
static MYSQL_THDVAR_UINT(conn_recycle_strict,                       /* name */
...
/*
  ...
  TRUE:  sync
 */
static MYSQL_THDVAR_BOOL(sync_trx_isolation,                 /* name */
 ...
 */
static MYSQL_THDVAR_UINT(force_commit,                  /* name */
                         PLUGIN_VAR_RQCMDARG,           /* opt */
...
/*
 -1 :use table parameter
  0-:offset
 */
static MYSQL_THDVAR_LONGLONG(internal_offset,       /* name */
                             PLUGIN_VAR_RQCMDARG,   /* opt */
...
longlong spider_param_internal_offset(THD *thd, longlong internal_offset) {
  ...
  DBUG_RETURN(THDVAR(thd, internal_offset) < 0 ? internal_offset
...
/*
 -1 :use table parameter
  0-:limit
 */
static MYSQL_THDVAR_LONGLONG(internal_limit,        /* name */
                             PLUGIN_VAR_RQCMDARG,   /* opt */
...
longlong spider_param_internal_limit(THD *thd, longlong internal_limit) {
  ...
  DBUG_RETURN(THDVAR(thd, internal_limit) < 0 ? internal_limit
 ...
 */
static MYSQL_THDVAR_LONGLONG(split_read,                   /* name */
                             PLUGIN_VAR_RQCMDARG,          /* opt */
...
longlong spider_param_split_read(THD *thd, longlong split_read) {
  ...
  DBUG_RETURN(THDVAR(thd, split_read) < 0 ? split_read
...
double spider_param_semi_split_read(THD *thd, double semi_split_read) {
  ...
  DBUG_RETURN(THDVAR(thd, semi_split_read) < 0 ? semi_split_read
...
/*
 -1 :use table parameter
  ...
  1-:alloc size
 */
static MYSQL_THDVAR_INT(init_sql_alloc_size,                       /* name */
                        PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_NOSYSVAR, /* opt */
...
/*
 -1 :use table parameter
  ...
  1 :on
 */
static MYSQL_THDVAR_INT(reset_sql_alloc,                           /* name */
                        PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_NOSYSVAR, /* opt */
...
/*
 -1 :use table parameter
  ...
  1 :on
 */
static MYSQL_THDVAR_INT(multi_split_read,                          /* name */
                        PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_NOSYSVAR, /* opt */
...
int spider_param_max_order(THD *thd, int max_order) {
  ...
  DBUG_RETURN(THDVAR(thd, max_order) < 0 ? max_order : THDVAR(thd, max_order));
}
...
static int spider_param_semi_table_lock_check(MYSQL_THD thd,
...
#if MYSQL_VERSION_ID < 50500
  DBUG_RETURN(throw_bounds_warning(
      thd, fixed, FALSE, ((MYSQL_SYSVAR_NAME(thdvar_int_t) *)var)->name,
      (long long)tmp));
#else
  DBUG_RETURN(
...
/*
  ...
  1 :on
 */
static MYSQL_THDVAR_INT(semi_table_lock,                           /* name */
                        PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_NOSYSVAR, /* opt */
...
static int spider_param_semi_table_lock_connection_check(
    MYSQL_THD thd, struct st_mysql_sys_var *var, void *save,
    struct st_mysql_value *value) {
  int error_num;
  SPIDER_TRX *trx;
  my_bool fixed;
...
/*
  0-:block_size
 */
static MYSQL_THDVAR_UINT(block_size,                                /* name */
                         PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_NOSYSVAR, /* opt */
...
/*
 -1 :use table parameter
  ...
  2 :for update
 */
static MYSQL_THDVAR_INT(selupd_lock_mode,                          /* name */
                        PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_NOSYSVAR, /* opt */
...
/*
  ...
  TRUE:  sync
 */
static MYSQL_THDVAR_BOOL(sync_autocommit,     /* name */
                         PLUGIN_VAR_OPCMDARG, /* opt */
...
/*
  ...
  TRUE:  sync
 */
static MYSQL_THDVAR_BOOL(sync_time_zone,      /* name */
                         PLUGIN_VAR_OPCMDARG, /* opt */
...
/*
  ...
  TRUE:  use
 */
static MYSQL_THDVAR_BOOL(use_default_database,                      /* name */
...
/*
 -1 :use table parameter
  0-:bulk insert size
 */
static MYSQL_THDVAR_INT(bulk_size,           /* name */
                        PLUGIN_VAR_RQCMDARG, /* opt */
...
int spider_param_bulk_size(THD *thd, int bulk_size) {
  ...
  DBUG_RETURN(THDVAR(thd, bulk_size) < 0 ? bulk_size : THDVAR(thd, bulk_size));
}
...
/*
 -1 :use table parameter
  0-:bulk update size
 */
static MYSQL_THDVAR_INT(bulk_update_size,    /* name */
                        PLUGIN_VAR_RQCMDARG, /* opt */
...
/*
 -1 :use table parameter
  ...
  1 :on
 */
static MYSQL_THDVAR_INT(internal_optimize,                         /* name */
                        PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_NOSYSVAR, /* opt */
...
int spider_param_internal_optimize_local(THD *thd,
  ...
  DBUG_RETURN(THDVAR(thd, internal_optimize_local) == -1
                  ...
                  : THDVAR(thd, internal_optimize_local));
}
...
/*
  ...
  TRUE:  on
 */
static MYSQL_THDVAR_BOOL(use_flash_logs,                            /* name */
                         PLUGIN_VAR_OPCMDARG | PLUGIN_VAR_NOSYSVAR, /* opt */
...
/*
  ...
  TRUE:  on
 */
static MYSQL_THDVAR_BOOL(lock_exchange,                             /* name */
                         PLUGIN_VAR_OPCMDARG | PLUGIN_VAR_NOSYSVAR, /* opt */
 ...
 */
static MYSQL_THDVAR_INT(quick_mode,          /* name */
                        PLUGIN_VAR_RQCMDARG, /* opt */
...
int spider_param_quick_mode(THD *thd, int quick_mode) {
  ...
  DBUG_RETURN(THDVAR(thd, quick_mode) < 0 ? quick_mode
...
longlong spider_param_quick_page_size(THD *thd, longlong quick_page_size) {
  ...
  DBUG_RETURN(THDVAR(thd, quick_page_size) < 0 ? quick_page_size
...
int spider_param_low_mem_read(THD *thd, int low_mem_read) {
  ...
  DBUG_RETURN(THDVAR(thd, low_mem_read) < 0 ? low_mem_read
 ...
 */
static MYSQL_THDVAR_INT(bgs_mode,                    /* name */
                        PLUGIN_VAR_RQCMDARG,         /* opt */
...
int spider_param_bgs_mode(THD *thd, int bgs_mode) {
  ...
  DBUG_RETURN(THDVAR(thd, bgs_mode) < 0 ? bgs_mode : THDVAR(thd, bgs_mode));
}
...
*/
static MYSQL_THDVAR_INT(bgs_dml,                  /* name */
                        PLUGIN_VAR_RQCMDARG,      /* opt */
...
static MYSQL_THDVAR_BOOL(ignore_xa_log,                             /* name */
                         PLUGIN_VAR_OPCMDARG | PLUGIN_VAR_NOSYSVAR, /* opt */
...
bool spider_param_ignore_xa_log(THD *thd) {
  ...
  DBUG_RETURN(THDVAR(thd, ignore_xa_log));
}
...
longlong spider_param_bgs_first_read(THD *thd, longlong bgs_first_read) {
  ...
  DBUG_RETURN(THDVAR(thd, bgs_first_read) < 0 ? bgs_first_read
...
longlong spider_param_first_read(THD *thd, longlong first_read) {
  ...
  DBUG_RETURN(THDVAR(thd, first_read) < 0 ? first_read
...
longlong spider_param_second_read(THD *thd, longlong second_read) {
  ...
  DBUG_RETURN(THDVAR(thd, second_read) < 0 ? second_read
...
double spider_param_crd_interval(THD *thd, double crd_interval) {
  ...
  DBUG_RETURN(THDVAR(thd, crd_interval) == -1 ? crd_interval
                                              : THDVAR(thd, crd_interval));
}
...
/*
 -1 :use table parameter
  ...
  3 :use explain
 */
static MYSQL_THDVAR_INT(crd_mode,                                  /* name */
                        PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_NOSYSVAR, /* opt */
...
int spider_param_crd_mode(THD *thd, int crd_mode) {
  ...
  DBUG_RETURN(THDVAR(thd, crd_mode) <= 0 ? crd_mode : THDVAR(thd, crd_mode));
}
...
int spider_param_crd_sync(THD *thd, int crd_sync) {
  ...
  DBUG_RETURN(THDVAR(thd, crd_sync) == -1 ? crd_sync : THDVAR(thd, crd_sync));
}
#endif
 ...
 */
static MYSQL_THDVAR_INT(crd_type,                                  /* name */
                        PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_NOSYSVAR, /* opt */
...
int spider_param_crd_type(THD *thd, int crd_type) {
  ...
  DBUG_RETURN(THDVAR(thd, crd_type) == -1 ? crd_type : THDVAR(thd, crd_type));
}
...
/*
 -1 :use table parameter
  0-:weight
 */
static MYSQL_THDVAR_INT(crd_weight,                                /* name */
                        PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_NOSYSVAR, /* opt */
...
double spider_param_crd_weight(THD *thd, double crd_weight) {
  ...
  DBUG_RETURN(THDVAR(thd, crd_weight) == -1 ? crd_weight
...
double spider_param_sts_interval(THD *thd, double sts_interval) {
  ...
  DBUG_RETURN(THDVAR(thd, sts_interval) == -1 ? sts_interval
                                              : THDVAR(thd, sts_interval));
}
...
/*
 -1 :use table parameter
  ...
  2 :use information schema
 */
static MYSQL_THDVAR_INT(sts_mode,                                  /* name */
                        PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_NOSYSVAR, /* opt */
...
int spider_param_sts_mode(THD *thd, int sts_mode) {
  ...
  DBUG_RETURN(THDVAR(thd, sts_mode) <= 0 ? sts_mode : THDVAR(thd, sts_mode));
}
...
int spider_param_sts_sync(THD *thd, int sts_sync) {
  ...
  DBUG_RETURN(THDVAR(thd, sts_sync) == -1 ? sts_sync : THDVAR(thd, sts_sync));
}
#endif
...
/*
  ...
  1-:interval
 */
static MYSQL_THDVAR_INT(ping_interval_at_trx_start,                /* name */
                        PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_NOSYSVAR, /* opt */
...
/*
  ...
  TRUE:  on
 */
static MYSQL_THDVAR_BOOL(same_server_link,                          /* name */
                         PLUGIN_VAR_OPCMDARG | PLUGIN_VAR_NOSYSVAR, /* opt */
...
static MYSQL_SYSVAR_BOOL(get_sts_or_crd, spider_get_sts_or_crd,
                         PLUGIN_VAR_OPCMDARG, "if need to get table status",
                         NULL, NULL, FALSE);
...
/*
  1-: mutex count
 */
static MYSQL_SYSVAR_UINT(udf_table_lock_mutex_count,
...
/*
  1-: mutex count
 */
static MYSQL_SYSVAR_UINT(udf_table_mon_mutex_count,
...
/*
  1-:number of rows
 */
static MYSQL_THDVAR_LONGLONG(udf_ds_bulk_insert_rows, /* name */
                             PLUGIN_VAR_RQCMDARG |
                                 PLUGIN_VAR_NOSYSVAR,             /* opt */
...
longlong spider_param_udf_ds_bulk_insert_rows(
  ...
  DBUG_RETURN(THDVAR(thd, udf_ds_bulk_insert_rows) <= 0
                  ...
                  : THDVAR(thd, udf_ds_bulk_insert_rows));
}
 ...
 */
static MYSQL_THDVAR_INT(udf_ds_table_loop_mode,                    /* name */
                        PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_NOSYSVAR, /* opt */
...
int spider_param_udf_ds_table_loop_mode(THD *thd, int udf_ds_table_loop_mode) {
  ...
  DBUG_RETURN(THDVAR(thd, udf_ds_table_loop_mode) == -1
                  ...
                  : THDVAR(thd, udf_ds_table_loop_mode));
}
...
static MYSQL_SYSVAR_STR(remote_access_charset, spider_remote_access_charset,
...
/*
 -1 :don't set
  ...
  1 :autocommit = 1
 */
static MYSQL_SYSVAR_INT(remote_autocommit, spider_remote_autocommit,
                        PLUGIN_VAR_RQCMDARG,
 ...
 */
#if defined(MARIADB_BASE_VERSION) && MYSQL_VERSION_ID >= 100000
static MYSQL_SYSVAR_STR(remote_time_zone, spider_remote_time_zone,
                        PLUGIN_VAR_MEMALLOC | PLUGIN_VAR_RQCMDARG |
                            PLUGIN_VAR_NOSYSVAR,
...
/*
 -1 :don't set
  ...
  3 :SERIALIZABLE
 */
static MYSQL_SYSVAR_INT(remote_trx_isolation, spider_remote_trx_isolation,
                        PLUGIN_VAR_RQCMDARG,
 ...
 */
#if defined(MARIADB_BASE_VERSION) && MYSQL_VERSION_ID >= 100000
static MYSQL_SYSVAR_STR(remote_default_database, spider_remote_default_database,
                        PLUGIN_VAR_MEMALLOC | PLUGIN_VAR_RQCMDARG,
 ...
 */
static MYSQL_THDVAR_LONGLONG(connect_retry_interval,   /* name */
                             PLUGIN_VAR_RQCMDARG,      /* opt */
...
longlong spider_param_connect_retry_interval(THD *thd) {
  ...
  if (thd) DBUG_RETURN(THDVAR(thd, connect_retry_interval));
  DBUG_RETURN(0);
}
 ...
 */
static MYSQL_THDVAR_INT(connect_retry_count,   /* name */
                        PLUGIN_VAR_RQCMDARG,   /* opt */
...
int spider_param_connect_retry_count(THD *thd) {
  ...
  if (thd) DBUG_RETURN(THDVAR(thd, connect_retry_count));
  DBUG_RETURN(0);
}
 ...
 */
#if defined(MARIADB_BASE_VERSION) && MYSQL_VERSION_ID >= 100000
static MYSQL_THDVAR_STR(bka_engine, /* name */
                        PLUGIN_VAR_MEMALLOC | PLUGIN_VAR_RQCMDARG |
                            PLUGIN_VAR_NOSYSVAR,
...
char *spider_param_bka_engine(THD *thd, char *bka_engine) {
  ...
  DBUG_RETURN(THDVAR(thd, bka_engine) ? THDVAR(thd, bka_engine) : bka_engine);
}
 ...
 */
static MYSQL_THDVAR_INT(bka_mode,                                  /* name */
                        PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_NOSYSVAR, /* opt */
...
int spider_param_bka_mode(THD *thd, int bka_mode) {
  ...
  DBUG_RETURN(THDVAR(thd, bka_mode) == -1 ? bka_mode : THDVAR(thd, bka_mode));
}
...
/*
 -1 :use table parameter
  ...
  1 :use handler
 */
static MYSQL_THDVAR_INT(use_handler,                               /* name */
                        PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_NOSYSVAR, /* opt */
...
int spider_param_use_handler(THD *thd, int use_handler) {
  ...
  DBUG_RETURN(THDVAR(thd, use_handler) == -1 ? use_handler
                                             : THDVAR(thd, use_handler));
}
 ...
 */
static MYSQL_THDVAR_INT(error_read_mode,                           /* name */
                        PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_NOSYSVAR, /* opt */
...
/*
 -1 :use table parameter
  ...
  1 :return 0 record if error
 */
static MYSQL_THDVAR_INT(error_write_mode,                          /* name */
                        PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_NOSYSVAR, /* opt */
...
int spider_param_skip_default_condition(THD *thd, int skip_default_condition) {
  ...
  DBUG_RETURN(THDVAR(thd, skip_default_condition) == -1
                  ...
                  : THDVAR(thd, skip_default_condition));
}
...
int spider_param_skip_parallel_search(THD *thd, int skip_parallel_search) {
  ...
  DBUG_RETURN(THDVAR(thd, skip_parallel_search) == -1
                  ...
                  : THDVAR(thd, skip_parallel_search));
}
...
/*
 -1 :use table parameter
  ...
  1 :read only
 */
static MYSQL_THDVAR_INT(read_only_mode,      /* name */
                        PLUGIN_VAR_RQCMDARG, /* opt */
                        "Read only",         /* comment */
                        NULL,                /* check */
                        NULL,                /* update */
...
/*
 -1 :use UDF parameter
  ...
  1 :can use
 */
static MYSQL_THDVAR_INT(udf_ds_use_real_table,                     /* name */
                        PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_NOSYSVAR, /* opt */
...
int spider_param_udf_ds_use_real_table(THD *thd, int udf_ds_use_real_table) {
  ...
  DBUG_RETURN(THDVAR(thd, udf_ds_use_real_table) == -1
                  ...
                  : THDVAR(thd, udf_ds_use_real_table));
}
#endif

static my_bool spider_general_log;
static MYSQL_SYSVAR_BOOL(general_log, spider_general_log, PLUGIN_VAR_OPCMDARG,
...
static MYSQL_SYSVAR_BOOL(quick_mode_only_select, spider_quick_mode_only_select,
                         PLUGIN_VAR_OPCMDARG,
...
static MYSQL_SYSVAR_BOOL(
    enable_mem_calc, spider_enable_mem_calc,
...
static MYSQL_SYSVAR_BOOL(enable_trx_ha, spider_enable_trx_ha,
                         PLUGIN_VAR_OPCMDARG | PLUGIN_VAR_READONLY |
                             PLUGIN_VAR_NOSYSVAR,
...
TRUE:  on
*/
static MYSQL_THDVAR_BOOL(trans_rollback,      /* name */
                         PLUGIN_VAR_OPCMDARG, /* opt */
...
static MYSQL_SYSVAR_INT(
    idle_conn_recycle_interval, spider_idle_conn_recycle_interval,
    PLUGIN_VAR_OPCMDARG,
...
static MYSQL_SYSVAR_INT(conn_meta_max_invalid_duration,
...
static MYSQL_SYSVAR_UINT(
    conn_wait_timeout, spider_conn_wait_timeout, PLUGIN_VAR_RQCMDARG,
...
static MYSQL_SYSVAR_BOOL(ignore_autocommit, spider_ignore_autocommit,
                         PLUGIN_VAR_OPCMDARG, "spider_ignore_autocommit", NULL,
                         NULL, FALSE);
 ...
 */
static MYSQL_THDVAR_UINT(internal_xa_id_type,          /* name */
                         PLUGIN_VAR_RQCMDARG,          /* opt */
 ...
 */
static MYSQL_THDVAR_INT(casual_read,                               /* name */
                        PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_NOSYSVAR, /* opt */
...
int spider_param_casual_read(THD *thd, int casual_read) {
  ...
  DBUG_RETURN(THDVAR(thd, casual_read) == -1 ? casual_read
                                             : THDVAR(thd, casual_read));
}

static my_bool spider_dry_access;
static MYSQL_SYSVAR_BOOL(dry_access, spider_dry_access,
                         PLUGIN_VAR_OPCMDARG | PLUGIN_VAR_READONLY |
                             PLUGIN_VAR_NOSYSVAR,
 ...
 */
static MYSQL_THDVAR_INT(delete_all_rows_type,                      /* name */
                        PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_NOSYSVAR, /* opt */
...
int spider_param_delete_all_rows_type(THD *thd, int delete_all_rows_type) {
  ...
  DBUG_RETURN(THDVAR(thd, delete_all_rows_type) == -1
                  ...
                  : THDVAR(thd, delete_all_rows_type));
}
...
int spider_param_bka_table_name_type(THD *thd, int bka_table_name_type) {
  ...
  DBUG_RETURN(THDVAR(thd, bka_table_name_type) == -1
                  ? bka_table_name_type
                  : THDVAR(thd, bka_table_name_type));
}
...
/*
 -1 : use table parameter
  ...
  1 : do store
 */
static MYSQL_SYSVAR_INT(store_last_sts, spider_store_last_sts,
                        PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_NOSYSVAR,
                        "Store last sts result into system table", NULL, NULL,
                        0, -1, 1, 0);
...
/*
 -1 : use table parameter
  ...
  1 : do store
 */
static MYSQL_SYSVAR_INT(store_last_crd, spider_store_last_crd,
                        PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_NOSYSVAR,
                        "Store last crd result into system table", NULL, NULL,
                        -1, -1, 1, 0);
...
/*
 -1 : use table parameter
  ...
  1 : do load
 */
static MYSQL_SYSVAR_INT(load_sts_at_startup, spider_load_sts_at_startup,
                        PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_NOSYSVAR,
                        "Load sts from system table at startup", NULL, NULL, 0,
                        -1, 1, 0);
...
/*
 -1 : use table parameter
  ...
  1 : do load
 */
static MYSQL_SYSVAR_INT(load_crd_at_startup, spider_load_crd_at_startup,
                        PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_NOSYSVAR,
                        "Load crd from system table at startup", NULL, NULL, 0,
                        -1, 1, 0);
...
/*
  1-: thread count
 */
static MYSQL_SYSVAR_UINT(table_sts_thread_count, spider_table_sts_thread_count,
                         PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY |
                             PLUGIN_VAR_NOSYSVAR,
...
/*
  1-: thread count
 */
static MYSQL_SYSVAR_UINT(table_crd_thread_count, spider_table_crd_thread_count,
                         PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY |
                             PLUGIN_VAR_NOSYSVAR,
...
mysql_declare_plugin(spider) {
  MYSQL_STORAGE_ENGINE_PLUGIN, &spider_storage_engine, "SPIDER",
      "Kentoku Shiba", "Spider storage engine", PLUGIN_LICENSE_GPL,
      spider_db_init, spider_db_done, SPIDER_HEX_VERSION,
      spider_status_variables, spider_system_variables, NULL,
