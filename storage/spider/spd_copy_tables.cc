...
long long spider_copy_tables_body(UDF_INIT *initid, UDF_ARGS *args,
  ...
  if (thd->open_tables != 0 || thd->handler_tables_hash.records != 0 ||
      thd->derived_tables != 0 || thd->lock != 0 ||
#if MYSQL_VERSION_ID < 50500
      thd->locked_tables != 0 || thd->prelocked_mode != NON_PRELOCKED
#else
  ...
  if (args->arg_count == 4) {
    if (spider_udf_parse_copy_tables_param(
            copy_tables, args->args[3] ? args->args[3] : (char *)"",
            args->args[3] ? args->lengths[3] : 0))
      goto error;
  } else {
    if (spider_udf_parse_copy_tables_param(copy_tables, (char *)"", 0))
      goto error;
  }
