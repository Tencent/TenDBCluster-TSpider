...
int spider_db_mysql_row::store_to_field(Field *field,
...
int spider_db_mysql_result::fetch_table_status(
  ...
  if (mode == 1) {
    ...
    if (mysql_row[0])
      records = (ha_rows)my_strtoll10(mysql_row[0], (char **)NULL, &error_num);
    else
      records = (ha_rows)0;
    DBUG_PRINT("info", ("spider records=%lld", records));
    if (mysql_row[1])
...
int spider_db_mysql_result::fetch_table_records(int mode, ha_rows &records) {
  ...
  if (mode == 1) {
    ...
    if (mysql_row[8]) {
      records = (ha_rows)my_strtoll10(mysql_row[8], (char **)NULL, &error_num);
    } else
      records = 0;
  }
  DBUG_RETURN(0);
  ...
  if (mode == 1) {
    ...
    while (mysql_row) {
      if (mysql_row[4] && mysql_row[6] &&
        ...
        if ((cardinality[field->field_index] = (longlong)my_strtoll10(
                 mysql_row[6], (char **)NULL, &error_num)) <= 0)
    ...
    while (mysql_row) {
      if (mysql_row[0] && mysql_row[1] &&
        ...
        if ((cardinality[field->field_index] = (longlong)my_strtoll10(
                 mysql_row[1], (char **)NULL, &error_num)) <= 0)
          cardinality[field->field_index] = 1;
        spider_set_bit(cardinality_upd, field->field_index);
...
int spider_db_mysql::connect(char *tgt_host, char *tgt_username,
                             char *tgt_password, long tgt_port,
                             char *tgt_socket, char *server_name,
  ...
  *******************************/
  ...
  if ((error_num && log_result_errors >= 1) ||
    ...
    if (log_result_error_with_sql) {
      ...
      if ((log_result_error_with_sql & 2) && query_length) {
        ...
        fprintf(stderr,
                ...
                security_ctx->host_or_ip, (ulong)thd->thread_id,
      ...
      if (log_result_error_with_sql & 1) {
        ...
        fprintf(stderr,
                ...
                l_time->tm_year + 1900, l_time->tm_mon + 1, l_time->tm_mday,
                l_time->tm_hour, l_time->tm_min, l_time->tm_sec, usec,
                (ulong)thd->thread_id, conn->tgt_host,
                (ulong)db_conn->thread_id, tmp_query_str.c_ptr_safe());
      }
    }
    if (log_result_errors >= 2 && db_conn->warning_count > 0) {
      ...
      if (spider_param_log_result_errors() >= 3) print_warnings(l_time);
    } else if (log_result_errors >= 4) {
      ulong usec;
      struct tm *l_time = spider_get_time(usec);
      fprintf(stderr,
              "%04d%02d%02d %02d:%02d:%02d.%ld [INFO SPIDER RESULT] "
              "from [%s] %ld to %ld:  "
              "affected_rows: %llu  id: %llu  status: %u  warning_count: %u\n",
              l_time->tm_year + 1900, l_time->tm_mon + 1, l_time->tm_mday,
              l_time->tm_hour, l_time->tm_min, l_time->tm_sec, usec,
...
void spider_db_mysql::print_warnings(struct tm *l_time) {
        ...
        */
        ...
        while (row) {
          fprintf(stderr,
                  ...
                  hrtime_sec_part(current_time), conn->tgt_host,
...
spider_db_result *spider_db_mysql::store_result(
    spider_db_result_buffer **spider_res_buf,
    st_spider_db_request_key *request_key, int *error_num) {
...
int spider_db_mysql::xa_start(XID *xid, int *need_mon) {
...
int spider_db_mysql::xa_end(XID *xid, int *need_mon) {
...
int spider_db_mysql::xa_prepare(XID *xid, int *need_mon) {
...
int spider_db_mysql::xa_end_and_prepare(XID *xid, int *need_mon) {
  int error_num = 0;
  int error_num1 = 0;
  int error_num2 = 0;
...
int spider_db_mysql::xa_commit(XID *xid, int *need_mon) {
...
int spider_db_mysql::xa_commit_one_phase(XID *xid, int *need_mon) {
  int error_num = 0;
  int error_num1 = 0;
  int error_num2 = 0;
...
int spider_db_mysql::xa_rollback(XID *xid, int *need_mon) {
...
int spider_db_mysql::select_binlog_gtid_pos(
  ...
  length =
      conn->db_conn->escape_string((char *)str->ptr() + str->length(),
...
int spider_db_mysql::append_sql(char *sql, ulong sql_length,
...
int spider_db_mysql::append_open_handler(
    uint handler_id, const char *db_name, const char *table_name,
...
int spider_db_mysql::append_select(uint handler_id, spider_string *sql,
...
int spider_db_mysql::append_insert(uint handler_id,
...
int spider_db_mysql::append_update(uint handler_id, spider_string *sql,
                                   SPIDER_DB_HS_STRING_REF_BUFFER *keys,
                                   SPIDER_DB_HS_STRING_REF_BUFFER *upds,
                                   int limit, int skip, bool increment,
                                   bool decrement,
                                   st_spider_db_request_key *request_key) {
...
int spider_db_mysql::append_delete(uint handler_id, spider_string *sql,
                                   SPIDER_DB_HS_STRING_REF_BUFFER *keys,
                                   int limit, int skip,
                                   st_spider_db_request_key *request_key) {
...
*/
      ...
      */
      ...
      DBUG_PRINT("info", ("spider geo is %f %f %f %f", xmin, xmax, ymin, ymax));
/*
...
int spider_db_mysql_util::append_xa_start(spider_string *str, XID *xid) {
...
int spider_mysql_share::create_column_name_str() {
  ...
  for (field = table_share->field, str = column_name_str; *field;
    ...
    str->set_charset(spider_share->access_charset);
    if ((error_num = spider_db_append_name_with_quote_str(
             str, (char *)(*field)->field_name.str, dbton_id)))
      goto error;
  }
  DBUG_RETURN(0);
...
int spider_mysql_handler::append_index_hint(spider_string *str, int link_idx,
                                            ulong sql_type) {
  ...
  for (field = table->field; *field; field++) {
    ...
    if ((*field)->is_null()) {
      ...
      if (spider_db_mysql_utility.append_column_value(
              spider, insert_sql, *field, NULL, share->access_charset) ||
...
int spider_mysql_handler::append_insert(spider_string *str, int link_idx) {
  SPIDER_SHARE *share = spider->share;
...
int spider_mysql_handler::append_increment_update_set(spider_string *str) {
  ...
  for (roop_count = 0; roop_count < hs_upds.size(); roop_count++) {
    if (value[roop_count].size() == 1 && *(value[roop_count].begin()) == '0')
      continue;

    Field *top_table_field =
        spider->get_top_table_field(spider->hs_pushed_ret_fields[roop_count]);
    if (!(field = spider->field_exchange(top_table_field))) continue;
    field_name_length =
        mysql_share->column_name_str[field->field_index].length();
    ...
    if (spider->hs_increment)
      str->q_append(SPIDER_SQL_HS_INCREMENT_STR, SPIDER_SQL_HS_INCREMENT_LEN);
    else
      str->q_append(SPIDER_SQL_HS_DECREMENT_STR, SPIDER_SQL_HS_DECREMENT_LEN);
...
#endif
        if (spider_db_mysql_utility.append_column_value(
                spider, str, *fields, NULL, share->access_charset) ||
...
#endif
        if (spider_db_mysql_utility.append_column_value(
                spider, str, top_table_field, NULL, share->access_charset) ||
            str->reserve(SPIDER_SQL_COMMA_LEN)) {
#ifndef DBUG_OFF
          dbug_tmp_restore_column_map(table->read_set, tmp_map);
#endif
...
int spider_mysql_handler::append_flush_tables(spider_string *str, int link_idx,
                                              bool lock) {
...
bool spider_mysql_handler::sql_is_filled_up(ulong sql_type) {
...
#if defined(HS_HAS_SQLCOM) && defined(HAVE_HANDLERSOCKET)
int spider_mysql_handler::reset_keys(ulong sql_type) {
...
int spider_mysql_handler::reset_upds(ulong sql_type) {
...
int spider_mysql_handler::reset_strs(ulong sql_type) {
...
int spider_mysql_handler::reset_strs_pos(ulong sql_type) {
...
int spider_mysql_handler::push_back_upds(SPIDER_HS_STRING_REF &info) {
  int error_num;
...
bool spider_mysql_handler::need_lock_before_set_sql_for_exec(ulong sql_type) {
...
ha_rows spider_mysql_handler::explain_select(key_range *start_key,
  ...
  if ((error_num = spider_db_set_names(spider, conn, link_idx)) ||
    ...
    if (error_num == ER_SPIDER_REMOTE_SERVER_GONE_AWAY_NUM &&
      ...
      /* retry */
      if ((error_num = spider_db_ping(spider, conn, link_idx))) {
        if (spider->check_error_mode(error_num)) my_errno = error_num;
        conn->mta_conn_mutex_lock_already = FALSE;
        conn->mta_conn_mutex_unlock_later = FALSE;
        spider_mta_conn_mutex_unlock(conn);
        DBUG_RETURN(HA_POS_ERROR);
...
int spider_mysql_handler::sync_from_clone_source(spider_db_handler *dbton_hdl) {
  DBUG_ENTER("spider_mysql_handler::sync_from_clone_source");
  DBUG_PRINT("info", ("spider this=%p", this));
  DBUG_RETURN(0);
}

bool spider_mysql_handler::support_use_handler(int use_handler) {
...
int spider_mysql_copy_table::copy_key_row(spider_db_copy_table *source_ct,
  ...
  sql.q_append(SPIDER_SQL_NAME_QUOTE_STR, SPIDER_SQL_NAME_QUOTE_LEN);
  if ((error_num = spider_db_append_name_with_quote_str(
           &sql, (char *)field->field_name.str, spider_dbton_mysql.dbton_id)))
    DBUG_RETURN(error_num);
