...
class ha_tcadmin: public handler
{
private:
  THR_LOCK_DATA lock;
  TSD_share *share;
...
public:
  ha_tcadmin(handlerton *hton, TABLE_SHARE *table_arg)
...
TSD_share *ha_tcadmin::get_share()
{
  TSD_share *tmp_share;
  lock_shared_ha_data();
...
int ha_tcadmin::open(const char *name, int mode, uint test_if_locked)
{
  if (!(share= get_share()))
...
static handler *create_handler(handlerton *hton, TABLE_SHARE *table,
                               MEM_ROOT *mem_root)
{
  return new (mem_root) ha_tcadmin(hton, table);
}
...
maria_declare_plugin(test_sql_discovery)
{
  ...
  "TEST_SQL_DISCOVERY",
  "willhan",
  "Minimal engine to for mysql test install",
  PLUGIN_LICENSE_GPL,
  init,
  NULL,
  0x0001,
