...
void best_access_path(JOIN *join, JOIN_TAB *s, table_map remaining_tables,
                      uint idx, bool disable_jbuf, double record_count,
                      POSITION *pos, POSITION *loose_scan_pos);
static void optimize_straight_join(JOIN *join, table_map join_tables);
static bool greedy_search(JOIN *join, table_map remaining_tables, uint depth,
                          uint prune_level, uint use_cond_selectivity);
static bool best_extension_by_limited_search(
    JOIN *join, table_map remaining_tables, uint idx, double record_count,
    double read_time, uint depth, uint prune_level, uint use_cond_selectivity);
static uint determine_search_depth(JOIN *join);
C_MODE_START
static int join_tab_cmp(const void *dummy, const void *ptr1, const void *ptr2);
static int join_tab_cmp_straight(const void *dummy, const void *ptr1,
                                 const void *ptr2);
static int join_tab_cmp_embedded_first(const void *emb, const void *ptr1,
                                       const void *ptr2);
C_MODE_END
...
static Item *make_cond_after_sjm(THD *thd, Item *root_cond, Item *cond,
                                 table_map tables, table_map sjm_tables,
                                 bool inside_or_clause);
static bool make_join_select(JOIN *join, SQL_SELECT *select, COND *item);
static void revise_cache_usage(JOIN_TAB *join_tab);
static bool make_join_readinfo(JOIN *join, ulonglong options,
                               uint no_jbuf_after);
...
static int return_zero_rows(JOIN *join, select_result *res,
                            List<TABLE_LIST> &tables, List<Item> &fields,
                            bool send_row, ulonglong select_options,
                            const char *info, Item *having,
                            List<Item> &all_fields);
static COND *build_equal_items(JOIN *join, COND *cond, COND_EQUAL *inherited,
...
static COND *substitute_for_best_equal_field(THD *thd, JOIN_TAB *context_tab,
                                             COND *cond, COND_EQUAL *cond_equal,
                                             void *table_join_idx);
...
static COND *optimize_cond(JOIN *join, COND *conds, List<TABLE_LIST> *join_list,
                           bool ignore_on_conds, Item::cond_result *cond_value,
                           COND_EQUAL **cond_equal, int flags = 0);
...
static bool test_if_cheaper_ordering(const JOIN_TAB *tab, ORDER *order,
...
static int test_if_order_by_key(JOIN *join, ORDER *order, TABLE *table,
                                uint idx, uint *used_key_parts = NULL);
...
void dbug_serve_apcs(THD *thd, int n_calls) {
  ...
  while (thd->apc_target.n_calls_processed < n_apcs) {
    ...
    if (unlikely(thd->check_killed())) break;
  }
}
...
bool dbug_user_var_equals_int(THD *thd, const char *name, int value) {
  ...
  if ((var = get_variable(&thd->user_vars, &varname, FALSE))) {
    ...
    if (!null_value && var_value == value) return TRUE;
  }
  return FALSE;
}
#endif
...
bool handle_select(THD *thd, LEX *lex, select_result *result,
  ...
  if (select_lex->master_unit()->is_unit_op() ||
    ...
    */
    res = mysql_select(
        thd, select_lex->table_list.first, select_lex->with_wild,
        ...
        select_lex->having, lex->proc_list.first,
        select_lex->options | thd->variables.option_bits |
            setup_tables_done_option,
        result, unit, select_lex);
  }
  DBUG_PRINT("info", ("res: %d  report_error: %d", res, thd->is_error()));
  res |= thd->is_error();
  if (unlikely(res)) result->abort_result_set();
    ...
    */
    ...
    if (ref->in_sum_func) {
      ...
      if (ref->in_sum_func->nest_level > select->nest_level)
        ...
        for (sum_func = ref->in_sum_func;
             sum_func && sum_func->aggr_level >= select->nest_level;
    ...
    new_ref = direct_ref
                  ? new (thd->mem_root) Item_direct_ref(
                        thd, ref->context, item_ref, ref->table_name,
                        &ref->field_name, ref->alias_name_used)
                  : new (thd->mem_root)
                        Item_ref(thd, ref->context, item_ref, ref->table_name,
                                 &ref->field_name, ref->alias_name_used);
    if (!new_ref) return TRUE;
    ref->outer_ref = new_ref;
    ref->ref = &ref->outer_ref;

    if (!ref->fixed && ref->fix_fields(thd, 0)) return TRUE;
...
static void remove_redundant_subquery_clauses(st_select_lex *subq_select_lex) {
  ...
  /*
    ...
    clause
  */
  if (subq_select_lex->group_list.elements && !subq_select_lex->with_sum_func &&
...
*/
static inline int setup_without_group(
    THD *thd, Ref_ptr_array ref_pointer_array, TABLE_LIST *tables,
    ...
    List<Item_window_func> &win_funcs, bool *hidden_group_fields,
    uint *reserved) {
  int res;
  ...
  switch (type) {
    ...
    case SYSTEM_TIME_BETWEEN:
      start.print(str, query_type,
                  STRING_WITH_LEN(" FOR SYSTEM_TIME BETWEEN "));
...
#define newx new (thd->mem_root)
  ...
  if (versioned_tables == 0) DBUG_RETURN(0);
  ...
  for (table = tables; table; table = table->next_local) {
    if (!table->table || !table->table->versioned()) continue;
    ...
    */
    ...
    if (!vers_conditions.is_set()) {
      if (vers_conditions.init_from_sysvar(thd)) DBUG_RETURN(-1);
    }

    if (vers_conditions.is_set()) {
      if (vers_conditions.type == SYSTEM_TIME_ALL) continue;
    ...
    Item *row_start = newx Item_field(thd, &this->context, table->db.str,
                                      table->alias.str, fstart);
    Item *row_end = newx Item_field(thd, &this->context, table->db.str,
                                    table->alias.str, fend);
    ...
    if (vers_conditions.is_set()) {
      ...
      if (vers_conditions.resolve_units(thd)) DBUG_RETURN(-1);
    ...
    if (t->versioned(VERS_TIMESTAMP)) {
      ...
      switch (vers_conditions.type) {
        case SYSTEM_TIME_UNSPECIFIED:
          thd->variables.time_zone->gmt_sec_to_TIME(&max_time,
                                                    TIMESTAMP_MAX_VALUE);
      ...
      switch (vers_conditions.type) {
        ...
        case SYSTEM_TIME_AS_OF:
          trx_id0 = vers_conditions.start.unit == VERS_TIMESTAMP
                        ? newx Item_func_trt_id(thd, point_in_time1,
                                                TR_table::FLD_TRX_ID)
                        : point_in_time1;
        ...
        case SYSTEM_TIME_BETWEEN:
          ...
          trx_id1 = vers_conditions.end.unit == VERS_TIMESTAMP
                        ? newx Item_func_trt_id(thd, point_in_time2,
...
/**
  ...
  @retval
    0   on success
*/
int JOIN::prepare(TABLE_LIST *tables_init, uint wild_num, COND *conds_init,
                  uint og_num, ORDER *order_init, bool skip_order_by,
                  ORDER *group_init, Item *having_init, ORDER *proc_param_init,
                  SELECT_LEX *select_lex_arg, SELECT_LEX_UNIT *unit_arg) {
  DBUG_ENTER("JOIN::prepare");

  // to prevent double initialization on EXPLAIN
  if (optimization_state != JOIN::NOT_OPTIMIZED) DBUG_RETURN(0);
  ...
  if (select_lex->handle_derived(thd->lex, DT_PREPARE)) DBUG_RETURN(-1);
  ...
  */
  ...
  if (select_lex->vers_setup_conds(thd, tables_list) < 0) DBUG_RETURN(-1);
  ...
  */
  ...
  if ((~thd->variables.sql_mode & MODE_ONLY_FULL_GROUP_BY) &&
    ...
    while ((select_el = select_it++)) {
      if (select_el->with_sum_func) found_sum_func_elem = true;
      if (select_el->with_field) found_field_elem = true;
  ...
  while ((tbl = li++)) {
    ...
    */
    if (mixed_implicit_grouping && tbl->table) tbl->table->maybe_null = 1;
#ifdef WITH_PARTITION_STORAGE_ENGINE
    if (tbl->table->part_info) select_lex->partitioned_table_count++;
#endif
  }
  ...
  if (setup_wild(thd, tables_list, fields_list, &all_fields, wild_num,
    ...
    DBUG_RETURN(-1);
  if (select_lex->setup_ref_array(thd, real_og_num)) DBUG_RETURN(-1);
  ...
  if (setup_fields(thd, ref_ptrs, fields_list, MARK_COLUMNS_READ, &all_fields,
                   &select_lex->pre_fix, 1))
    DBUG_RETURN(-1);
  thd->lex->current_select->context_analysis_place = save_place;

  if (setup_without_group(thd, ref_ptrs, tables_list, select_lex->leaf_tables,
                          fields_list, all_fields, &conds, order, group_list,
                          select_lex->window_specs, select_lex->window_funcs,
  ...
  if (skip_order_by &&
    ...
    for (ORDER *order = select_lex->order_list.first; order;
      ...
      if (find_order_in_list(thd, ref_ptrs, tables_list, order, fields_list,
                             all_fields, false, false, false))
        DBUG_RETURN(-1);
    }
    thd->lex->allow_sum_func = save_allow_sum_func;
  ...
  */
  if (select_lex->have_window_funcs()) {
    ...
    Item *item;
    while ((item = it++)) {
      if (item->with_window_func) item->update_used_tables();
    }
  }
  ...
  if (with_elem && select_lex->check_unrestricted_recursive(
      ...
      */
      if (!real_order &&
          ...
          ((item->type() != Item::FIELD_ITEM ||
           ...
           (item->type() != Item::FUNC_ITEM || item->maybe_null ||
            item->result_type() != STRING_RESULT || item->max_length)))
        real_order = TRUE;

      if (item->with_sum_func && item->type() != Item::SUM_FUNC_ITEM)
        item->split_sum_func(thd, ref_ptrs, all_fields, 0);
    }
    if (!real_order) order = NULL;
  }

  if (having && having->with_sum_func)
    having->split_sum_func2(thd, ref_ptrs, all_fields, &having,
                            SPLIT_SUM_SKIP_REGISTERED);
    ...
    */
    for (ORDER *ord = group_list; ord; ord = ord->next) {
      if ((*ord->item)->type() == Item::FIELD_ITEM &&
        ...
        Item_field *field =
            new (thd->mem_root) Item_field(thd, *(Item_field **)ord->item);
  ...
  */
  ...
  if (unlikely(error)) goto err; /* purecov: inspected */
  if (procedure) {
    ...
    if (thd->lex->derived_tables) {
      ...
      */
      my_error(ER_WRONG_USAGE, MYF(0), "PROCEDURE",
               thd->lex->derived_tables & DERIVED_VIEW ? "view" : "subquery");
      goto err;
    }
...
bool JOIN::build_explain() {
  ...
  */
  ...
  bool res = save_explain_data(
      thd->lex->explain, false /* can overwrite */, need_tmp,
  ...
  for (uint i = 0; i < aggr_tables; i++, curr_tab++) {
    if (select_nr == INT_MAX) {
      ...
      curr_tab->tracker =
          thd->lex->explain->get_union(select_nr)->get_tmptable_read_tracker();
    } else {
...
int JOIN::optimize() {
  ...
  if (optimization_state == JOIN::OPTIMIZATION_PHASE_1_DONE)
    ...
    // to prevent double initialization on EXPLAIN
    if (optimization_state != JOIN::NOT_OPTIMIZED) return FALSE;
    optimization_state = JOIN::OPTIMIZATION_IN_PROGRESS;
    res = optimize_inner();
  }
  if (!with_two_phase_optimization ||
      init_state == JOIN::OPTIMIZATION_PHASE_1_DONE) {
    if (!res && have_query_plan != QEP_DELETED) res = build_explain();
...
int JOIN::init_join_caches() {
  ...
  for (tab = first_linear_tab(this, WITH_BUSH_ROOTS, WITHOUT_CONST_TABLES); tab;
    ...
    if (table->file->keyread_enabled()) {
      if (!(table->file->index_flags(table->file->keyread, 0, 1) &
            HA_CLUSTERED_INDEX))
        table->mark_columns_used_by_index(table->file->keyread,
                                          table->read_set);
...
int JOIN::optimize_inner() {
  ...
  */
  ...
  if (select_lex->first_cond_optimization) {
    ...
    if (select_lex->handle_derived(thd->lex, DT_MERGE)) DBUG_RETURN(TRUE);
    table_count = select_lex->leaf_tables.elements;
  }
  ...
  */
  if (select_lex->first_cond_optimization && conds &&
      conds->walk(&Item::exists2in_processor, 0, thd))
    DBUG_RETURN(1);
  /*
    TODO
    make view to decide if it is possible to write to WHERE directly or make
  Semi-Joins able to process ON condition if it is possible for (TABLE_LIST
  *tbl= tables_list; tbl; tbl= tbl->next_local)
  {
    if (tbl->on_expr &&
  ...
  */
  ...
  if (transform_max_min_subquery()) DBUG_RETURN(1); /* purecov: inspected */
  ...
  if (optimize_constant_subqueries()) DBUG_RETURN(1);

  if (setup_jtbm_semi_joins(this, join_list, &conds)) DBUG_RETURN(1);

  if (select_lex->cond_pushed_into_where) {
    conds = and_conds(thd, conds, select_lex->cond_pushed_into_where);
    if (conds && conds->fix_fields(thd, &conds)) DBUG_RETURN(1);
  }
  if (select_lex->cond_pushed_into_having) {
    having = and_conds(thd, having, select_lex->cond_pushed_into_having);
    if (having) {
      select_lex->having_fix_field = 1;
      if (having->fix_fields(thd, &having)) DBUG_RETURN(1);
      select_lex->having_fix_field = 0;
    }
  }

  conds = optimize_cond(this, conds, join_list, FALSE, &cond_value, &cond_equal,
  ...
  if (thd->lex->sql_command == SQLCOM_SELECT &&
      ...
      if (tbl->is_materialized_derived()) {
        ...
        */
        if (!tbl->is_inner_table_of_outer_join()) {
          if (pushdown_cond_for_derived(thd, conds, tbl)) DBUG_RETURN(1);
        }
    ...
    if (select_lex->handle_derived(thd->lex, DT_OPTIMIZE)) DBUG_RETURN(1);
  }
  ...
  {
    having = optimize_cond(this, having, join_list, TRUE, &having_value,
                           &having_equal);
    ...
    if (cond_value == Item::COND_FALSE || having_value == Item::COND_FALSE ||
      ...
      DBUG_PRINT("info",
                 (having_value == Item::COND_FALSE ? "Impossible HAVING"
                                                   : "Impossible WHERE"));
      zero_result_cause = having_value == Item::COND_FALSE ? "Impossible HAVING"
                                                           : "Impossible WHERE";
...
#ifdef WITH_PARTITION_STORAGE_ENGINE
  {
    ...
    if (!(thd->security_ctx->master_access &
          SUPER_ACL)) { /* only support one shard, for not super_acl */
      if (opt_spider_transaction_one_shard &&
          thd_test_options(
              thd, OPTION_NOT_AUTOCOMMIT | OPTION_BEGIN)) { /* transaction */
        if (table_num == 1) {                               /* only one table */
          if (spider_use_mul_partition_count > 0 ||
              (thd->spider_last_partition_num != 0 &&
               (thd->spider_current_partition_num !=
                thd->spider_last_partition_num))) {
            my_error(ER_ACCESS_DENIED_MULPARTITION_IN_TRANSACTION, MYF(0),
                     thd->security_ctx->priv_user,
                     thd->security_ctx->host_or_ip);
            DBUG_RETURN(TRUE);
          }
        } else if (table_num > 1 && (spider_use_mul_partition_count > 0 ||
                                     !is_same_shard)) { /* more than one table*/
          my_error(ER_ACCESS_DENIED_MULPARTITION_IN_TRANSACTION, MYF(0),
                   thd->security_ctx->priv_user, thd->security_ctx->host_or_ip);
          DBUG_RETURN(TRUE);
        }
      } else if (opt_spider_query_one_shard) { /* query */
        if ((table_num == 1 && spider_use_mul_partition_count > 0) ||
            (table_num > 1 && spider_use_mul_partition_count > 1)) {
          /* 1. table_num == 1, use_mul_partition_tables>1 mean multiple
          partition
          2. table_name > 1, must be only one table can multiple partition */
          my_error(ER_ACCESS_DENIED_MULPARTITION_IN_QUERY, MYF(0), "Select",
                   thd->security_ctx->priv_user, thd->security_ctx->host_or_ip,
                   last_tb1->alias.str);
          DBUG_RETURN(TRUE);
        }
  ...
  */
  if (tables_list && implicit_grouping) {
    ...
    */
    if ((res =
             opt_sum_query(thd, select_lex->leaf_tables, all_fields, conds))) {
      DBUG_ASSERT(res >= 0);
      ...
      */
      if (conds && !(thd->lex->describe & DESCRIBE_EXTENDED)) {
        COND *table_independent_conds = make_cond_for_table(
            thd, conds, PSEUDO_TABLE_BITS, 0, -1, FALSE, FALSE);
        DBUG_EXECUTE("where",
  ...
  */
  if (group_list && table_count == 1) {
    group_list =
        remove_const(this, group_list, conds,
  ...
  if (unlikely(make_join_statistics(this, select_lex->leaf_tables, &keyuse)) ||
  ...
  */
  if (fix_all_splittings_in_plan()) DBUG_RETURN(1);
...
setup_subq_exit:
  ...
  if (with_two_phase_optimization)
    ...
    if (optimize_stage2()) DBUG_RETURN(1);
  }
  DBUG_RETURN(0);
}
...
int JOIN::optimize_stage2() {
  ...
  if (subq_exit_fl) goto setup_subq_exit;

  if (unlikely(thd->check_killed())) DBUG_RETURN(1);

  /* Generate an execution plan from the found optimal join order. */
  if (get_best_combination()) DBUG_RETURN(1);
  ...
  if (const_tables && !thd->locked_tables_mode &&
    ...
    */
    mysql_unlock_some_tables(thd, table, const_tables, GET_LOCK_SKIP_SEQUENCES);
  }
  if (!conds && outer_join) {
  ...
  select = make_select(*table, const_table_map, const_table_map, conds,
                       (SORT_INFO *)0, 1, &error);
  if (unlikely(error)) { /* purecov: inspected */
  ...
  for (tab = first_linear_tab(this, WITH_BUSH_ROOTS, WITHOUT_CONST_TABLES); tab;
    ...
    if (*tab->on_expr_ref) {
      *tab->on_expr_ref = substitute_for_best_equal_field(
          thd, NO_PARTICULAR_TAB, *tab->on_expr_ref, tab->cond_equal,
          map2table);
      if (unlikely(thd->is_error())) {
        error = 1;
        DBUG_PRINT("error", ("Error from substitute_for_best_equal"));
  ...
  */
  for (tab = first_linear_tab(this, WITH_BUSH_ROOTS, WITHOUT_CONST_TABLES); tab;
    ...
    for (uint i = 0; i < tab->ref.key_parts; i++) {
      ...
      while (equals) {
        ref_item = substitute_for_best_equal_field(thd, tab, ref_item, equals,
                                                   map2table);
        if (unlikely(thd->is_fatal_error)) DBUG_RETURN(1);
        ...
        *ref_item_ptr = ref_item;
        ...
        if (key_copy->type() == store_key::FIELD_STORE_KEY) {
          if (item->basic_const_item()) {
            ...
            tab->ref.key_copy[key_copy_index] = new store_key_const_item(
                *tab->ref.key_copy[key_copy_index], item);
          } else if (item->const_item()) {
            tab->ref.key_copy[key_copy_index] = new store_key_item(
                *tab->ref.key_copy[key_copy_index], item, TRUE);
          } else {
  ...
  if (setup_semijoin_loosescan(this)) DBUG_RETURN(1);
  ...
  */
  if (table_count - const_tables == 1 && (group || select_distinct) &&
    ...
    if (group && rollup.state == ROLLUP::STATE_NONE &&
      ...
      if (!order || test_if_subpart(group_list, order)) {
        if (skip_sort_order ||
            (select_lex->master_unit()->item &&
             select_limit == HA_POS_ERROR))  // This is a subquery
          order = NULL;
        else
          order = group_list;
      }
    ...
    if (select_distinct && list_contains_unique_index(
                               join_tab[const_tables].table,
                               find_field_in_item_list, (void *)&fields_list)) {
      select_distinct = 0;
    }
  }
  ...
  if (full_join || hash_join) {
    TABLE *stable = (sort_by_table == (TABLE *)1 ? join_tab[const_tables].table
                                                 : sort_by_table);
    /*
  ...
  */
  ...
  if (!(select_options & SELECT_DESCRIBE))
    if (init_ftfuncs(thd, select_lex, MY_TEST(order))) DBUG_RETURN(1);
  ...
  */
  ...
  if (optimize_unflattened_subqueries()) DBUG_RETURN(1);

  int res;
  if ((res = rewrite_to_index_subquery_engine(this)) != -1) DBUG_RETURN(res);
  if (setup_subquery_caches()) DBUG_RETURN(-1);
  ...
  */
  if (need_tmp || select_distinct || group_list || order) {
    for (uint i = 0; i < table_count; i++) {
      if (!(table[i]->map & const_table_map)) table[i]->prepare_for_position();
    }
  }
  ...
  if (!only_const_tables()) {
    ...
    */
    if ((order || group_list) && tab->type != JT_ALL && tab->type != JT_FT &&
  ...
  */
  ...
  if (make_aggr_tables_info()) DBUG_RETURN(1);

  if (init_join_caches()) DBUG_RETURN(1);

  error = 0;

  if (select_options & SELECT_DESCRIBE) goto derived_exit;
...
bool JOIN::add_having_as_table_cond(JOIN_TAB *tab) {
  ...
  if (!need_tmp) used_tables |= const_table_map;

  DBUG_ENTER("JOIN::add_having_as_table_cond");

  Item *sort_table_cond = make_cond_for_table(
      thd, tmp_having, used_tables, (table_map)0, false, false, false);
  if (sort_table_cond) {
    if (!tab->select) {
      if (!(tab->select = new SQL_SELECT)) DBUG_RETURN(true);
      tab->select->head = tab->table;
    }
    if (!tab->select->cond)
      tab->select->cond = sort_table_cond;
    else {
      if (!(tab->select->cond = new (thd->mem_root)
                Item_cond_and(thd, tab->select->cond, sort_table_cond)))
        DBUG_RETURN(true);
    }
    if (tab->pre_idx_push_select_cond) {
      if (sort_table_cond->type() == Item::COND_ITEM)
        sort_table_cond = sort_table_cond->copy_andor_structure(thd);
      if (!(tab->pre_idx_push_select_cond = new (thd->mem_root) Item_cond_and(
                thd, tab->pre_idx_push_select_cond, sort_table_cond)))
        DBUG_RETURN(true);
    }
    ...
    DBUG_EXECUTE("where", print_where(tab->select->cond, "select and having",
                                      QT_ORDINARY););

    having = make_cond_for_table(thd, tmp_having, ~(table_map)0, ~used_tables,
                                 false, false, false);
    DBUG_EXECUTE("where",
...
bool JOIN::add_fields_for_current_rowid(JOIN_TAB *cur,
                                        List<Item> *table_fields) {
  /*
  ...
  */
  for (JOIN_TAB *tab = join_tab; tab < cur; tab++) {
    if (!tab->keep_current_rowid) continue;
...
bool JOIN::make_aggr_tables_info() {
  ...
  if (group_optimized_away) implicit_grouping = true;

  bool implicit_grouping_with_window_funcs =
  ...
  */
  ...
  /*
    ...
    GROUP BY queries.
  */
  if (tables_list && !procedure && opt_spider_group_by_handler) {
    /*
    ...
    */
    ...
    for (tbl = tbl->next_local; ht && tbl; tbl = tbl->next_local) {
      if (!tbl->table || tbl->table->file->partition_ht() != ht) ht = 0;
    }

    if (ht && ht->create_group_by) {
      /* Check if the storage engine can intercept the query */
      Query query = {&all_fields, select_distinct, tables_list,
                     conds,       group_list,      order ? order : group_list,
                     having};
      group_by_handler *gbh = ht->create_group_by(thd, &query);

      if (gbh) {
        if (!(pushdown_query =
                  new (thd->mem_root) Pushdown_query(select_lex, gbh)))
          DBUG_RETURN(1);
        /*
        ...
        */
        ...
        TABLE *table = create_tmp_table(
            thd, curr_tab->tmp_table_param, all_fields, NULL, query.distinct,
            TRUE, select_options, HA_POS_ERROR, &empty_clex_str, !need_tmp,
            query.order_by || query.group_by);
        if (!table) DBUG_RETURN(1);
  ...
  */
  ...
  if (need_tmp) {
    ...
    bzero(curr_tab, sizeof(JOIN_TAB));
    curr_tab->ref.key = -1;
    if (only_const_tables()) first_select = sub_select_postjoin_aggr;
    ...
    */
    ...
    distinct =
        select_distinct && !group_list && !select_lex->have_window_funcs();
    keep_row_order = false;
    bool save_sum_fields =
        (group_list && simple_group) || implicit_grouping_with_window_funcs;
    if (create_postjoin_aggr_table(curr_tab, &all_fields, tmp_group,
                                   save_sum_fields, distinct, keep_row_order))
      DBUG_RETURN(true);
    ...
    */
    if ((group_list &&
        ...
        (select_distinct &&
         tmp_table_param
             .using_outer_summary_function)) { /* Must copy to another table */
      ...
      */
      ...
      if (group_list) {
        ...
        {
          if (add_sorting_to_table(curr_tab - 1, group_list)) DBUG_RETURN(true);
        }
    ...
    if (select_distinct && !group_list) {
      ...
      */
      if (top_join_tab_count) curr_tab->distinct = true;
  ...
  if (group || (implicit_grouping && !implicit_grouping_with_window_funcs) ||
      tmp_table_param.sum_func_count) {
    if (make_group_fields(this, this)) DBUG_RETURN(true);

    DBUG_ASSERT(items3.is_null());

    if (items0.is_null()) init_items_ref_array();
    items3 = ref_ptr_array_slice(4);
    setup_copy_fields(thd, &tmp_table_param, items3, tmp_fields_list3,
                      tmp_all_fields3, curr_fields_list->elements,
                      *curr_all_fields);
    ...
    if (make_sum_func_list(*curr_all_fields, *curr_fields_list, true, true))
      DBUG_RETURN(true);
    if (prepare_sum_aggregators(
            sum_funcs, !join_tab || !join_tab->is_using_agg_loose_index_scan()))
      DBUG_RETURN(true);
  ...
  if (group_list || order) {
    ...
    if (tmp_having && !is_having_added_as_table_cond && !group_list &&
        !sort_and_group) {
      if (add_having_as_table_cond(curr_tab)) DBUG_RETURN(true);
    }
    ...
    */
        ...
        curr_tab->type != JT_EQ_REF)  // Don't sort 1 row
    {
      ...
      if (add_sorting_to_table(sort_tab, order_arg)) DBUG_RETURN(true);
      /*
    ...
    if (!only_const_tables() && !join_tab[const_tables].filesort &&
        !(select_options & SELECT_DESCRIBE)) {
      /*
...
bool JOIN::create_postjoin_aggr_table(JOIN_TAB *tab, List<Item> *table_fields,
                                      ORDER *table_group, bool save_sum_fields,
  ...
  */
  ha_rows table_rows_limit = ((order == NULL || skip_sort_order) &&
                              !table_group && !select_lex->with_sum_func)
                                 ? select_limit
                                 : HA_POS_ERROR;
  ...
  TABLE *table =
      create_tmp_table(thd, tab->tmp_table_param, *table_fields, table_group,
                       distinct, save_sum_fields, select_options,
                       table_rows_limit, &empty_clex_str, true, keep_row_order);
  if (!table) DBUG_RETURN(true);
  tmp_table_param.using_outer_summary_function =
      tab->tmp_table_param->using_outer_summary_function;
  tab->join = this;
  DBUG_ASSERT(tab > tab->join->join_tab || !top_join_tab_count || !tables_list);
  if (tab > join_tab) (tab - 1)->next_select = sub_select_postjoin_aggr;
  if (!(tab->aggr = new (thd->mem_root) AGGR_OP(tab))) goto err;
  tab->table = table;
  ...
  if ((group_list && simple_group) ||
    ...
    if (prepare_sum_aggregators(
            sum_funcs,
            !(tables_list && join_tab->is_using_agg_loose_index_scan())))
      goto err;
...
err:
  if (table != NULL) free_tmp_table(thd, table);
  DBUG_RETURN(true);
}
...
void JOIN::optimize_distinct() {
  for (JOIN_TAB *last_join_tab = join_tab + top_join_tab_count - 1;;) {
    ...
    if (last_join_tab == join_tab) break;
    --last_join_tab;
  }
...
bool JOIN::add_sorting_to_table(JOIN_TAB *tab, ORDER *order) {
  tab->filesort = new (thd->mem_root)
      Filesort(order, HA_POS_ERROR, tab->keep_current_rowid, tab->select);
...
bool JOIN::setup_subquery_caches() {
  ...
  */
  if (select_lex->expr_cache_may_be_used[IN_WHERE] ||
    ...
    JOIN_TAB *tab;
    if (conds && !(conds = conds->transform(
                       thd, &Item::expr_cache_insert_transformer, NULL)))
      DBUG_RETURN(TRUE);
    for (tab = first_linear_tab(this, WITH_BUSH_ROOTS, WITHOUT_CONST_TABLES);
         tab; tab = next_linear_tab(this, tab, WITH_BUSH_ROOTS)) {
      if (tab->select_cond &&
          !(tab->select_cond = tab->select_cond->transform(
                thd, &Item::expr_cache_insert_transformer, NULL)))
        DBUG_RETURN(TRUE);
      if (tab->cache_select && tab->cache_select->cond)
        if (!(tab->cache_select->cond = tab->cache_select->cond->transform(
                  thd, &Item::expr_cache_insert_transformer, NULL)))
          DBUG_RETURN(TRUE);
    }

    if (having && !(having = having->transform(
                        thd, &Item::expr_cache_insert_transformer, NULL)))
      DBUG_RETURN(TRUE);

    if (tmp_having) {
      DBUG_ASSERT(having == NULL);
      if (!(tmp_having = tmp_having->transform(
                thd, &Item::expr_cache_insert_transformer, NULL)))
        DBUG_RETURN(TRUE);
    }
  }
  if (select_lex->expr_cache_may_be_used[SELECT_LIST] ||
    ...
    for (ORDER *tmp_group = group_list; tmp_group;
         tmp_group = tmp_group->next) {
      if (!(*tmp_group->item =
                (*tmp_group->item)
                    ->transform(thd, &Item::expr_cache_insert_transformer,
                                NULL)))
        DBUG_RETURN(TRUE);
    }
  }
...
bool JOIN::shrink_join_buffers(JOIN_TAB *jt, ulonglong curr_space,
                               ulonglong needed_space) {
  JOIN_TAB *tab;
  JOIN_CACHE *cache;
  for (tab = first_linear_tab(this, WITHOUT_BUSH_ROOTS, WITHOUT_CONST_TABLES);
       tab != jt; tab = next_linear_tab(this, tab, WITHOUT_BUSH_ROOTS)) {
    cache = tab->cache;
    if (cache) {
      size_t buff_size;
      if (needed_space < cache->get_min_join_buffer_size()) return TRUE;
...
int JOIN::reinit() {
  ...
  unit->offset_limit_cnt = (ha_rows)(
      select_lex->offset_limit ? select_lex->offset_limit->val_uint() : 0);
...
bool JOIN::prepare_result(List<Item> **columns_list) {
  ...
  if (result->prepare2(this)) goto err;
  ...
  */
  ...
  if (select_lex->select_number != UINT_MAX &&
      select_lex->select_number !=
          INT_MAX /* this is not a UNION's "fake select */
      && have_query_plan != JOIN::QEP_NOT_PRESENT_YET &&
      have_query_plan !=
          JOIN::QEP_DELETED &&  // this happens when there was
    ...
    return save_explain_data_intern(thd->lex->explain, need_tmp_table,
                                    need_order, distinct, message);
  }

  /*
    Can have join_tab==NULL for degenerate cases (e.g. SELECT .. UNION ...
    SELECT LIMIT 0)
  */
...
void JOIN::exec() {
  DBUG_EXECUTE_IF("show_explain_probe_join_exec_start",
                  if (dbug_user_var_equals_int(
                          thd, "show_explain_probe_select_id",
                          select_lex->select_number)) dbug_serve_apcs(thd, 1););
...
void JOIN::exec_inner() {
  ...
  */
  ...
  if (result->prepare2(this)) DBUG_VOID_RETURN;

  if (!tables_list && (table_count || !select_lex->with_sum_func) &&
      !select_lex->have_window_funcs()) {  // Only test of functions
    if (select_options & SELECT_DESCRIBE)
      select_describe(
      ...
      */
      DBUG_ASSERT(error == 0);
      if (cond_value != Item::COND_FALSE && having_value != Item::COND_FALSE &&
          (!conds || conds->val_int()) && (!having || having->val_int())) {
        if (do_send_rows &&
            (procedure ? (procedure->send_row(procedure_fields_list) ||
                          procedure->end_of_records())
                       : result->send_data(fields_list) > 0))
          error = 1;
        else
          send_records = ((select_options & OPTION_FOUND_ROWS)
                              ? 1
  ...
  */
  if (!zero_result_cause && exec_const_cond &&
      !(select_options & SELECT_DESCRIBE) && !exec_const_cond->val_int())
    zero_result_cause = "Impossible WHERE noticed after reading const tables";
    ...
    if (select_lex->have_window_funcs() && send_row_on_empty_set()) {
      ...
      */
      ...
      (void)return_zero_rows(this, result, select_lex->leaf_tables,
                             *columns_list, send_row_on_empty_set(),
                             select_options, zero_result_cause,
  ...
  */
  ...
  if (select_options & SELECT_DESCRIBE) {
    select_describe(this, need_tmp, order != 0 && !skip_sort_order,
                    select_distinct, !table_count ? "No tables used" : NullS);
    DBUG_VOID_RETURN;
  } else {
...
int JOIN::destroy() {
  ...
  if (join_tab) {
    for (JOIN_TAB *tab =
             first_linear_tab(this, WITH_BUSH_ROOTS, WITH_CONST_TABLES);
         tab; tab = next_linear_tab(this, tab, WITH_BUSH_ROOTS)) {
      if (tab->aggr) {
...
bool mysql_select(THD *thd, TABLE_LIST *tables, uint wild_num,
                  List<Item> &fields, COND *conds, uint og_num, ORDER *order,
                  ORDER *group, Item *having, ORDER *proc_param,
  ...
  if (select_lex->join != 0) {
    ...
    if (select_lex->linkage != DERIVED_TABLE_TYPE ||
      ...
      if (select_lex->linkage != GLOBAL_OPTIONS_TYPE) {
        ...
        */
        ...
        if ((err =
                 join->prepare(tables, wild_num, conds, og_num, order, false,
    ...
    */
    if (select_options & SELECT_DESCRIBE) free_join = 0;

    if (!(join = new (thd->mem_root) JOIN(thd, fields, select_options, result)))
      DBUG_RETURN(TRUE);
    THD_STAGE_INFO(thd, stage_init);
    thd->lex->used_tables = 0;
    if ((err = join->prepare(tables, wild_num, conds, og_num, order, false,
                             group, having, proc_param, select_lex, unit))) {
      goto err;
    }
  }
  ...
  if (unlikely(thd->is_error())) goto err;
...
static ha_rows get_quick_record_count(THD *thd, SQL_SELECT *select,
  ...
  if (select) {
    ...
    if (likely((error = select->test_quick_select(
                    thd, *(key_map *)keys, (table_map)0, limit, 0, FALSE,
...
static bool make_join_statistics(JOIN *join, List<TABLE_LIST> &tables_list,
  ...
  */
  ...
  if (!multi_alloc_root(
          join->thd->mem_root, &stat, sizeof(JOIN_TAB) * (table_count),
          &stat_ref, sizeof(JOIN_TAB *) * MAX_TABLES, &stat_vector,
          sizeof(JOIN_TAB *) * (table_count + 1), &table_vector,
          sizeof(TABLE *) * (table_count * 2), &join->positions,
          sizeof(POSITION) * (table_count + 1), &join->best_positions,
          sizeof(POSITION) * (table_count + 1), NullS))
    DBUG_RETURN(1);
  ...
  for (s = stat, i = 0; (tables = ti++); s++, i++) {
    ...
    if (embedding) {
      ...
      if (inside_an_outer_join) continue;
    }
    if (!table->is_filled_at_execution() &&
        (table->s->system ||
  ...
  if (join->outer_join) {
    ...
    */
    ...
    for (s = stat; s < stat_end; s++) {
      ...
      if (outer_join & s->table->map) s->table->maybe_null = 1;
    }
    /* Catch illegal cross references for outer joins */
    for (i = 0, s = stat; i < table_count; i++, s++) {
      if (s->dependent & s->table->map) {
        join->table_count = 0;  // Don't use join->table
        my_message(ER_WRONG_OUTER_JOIN, ER_THD(join->thd, ER_WRONG_OUTER_JOIN),
                   MYF(0));
        goto error;
      }
  ...
  if (join->conds || outer_join) {
    ...
    */
    bool skip_unprefixed_keyparts =
        !(join->is_in_subquery() && ((Item_in_subselect *)join->unit->item)
                                        ->test_strategy(SUBS_IN_TO_EXISTS));
  ...
  for (POSITION *p_pos = join->positions, *p_end = p_pos + const_count;
    ...
    if (!(s->table->map & join->eliminated_tables)) {
      ...
      if ((tmp = join_read_const_table(join->thd, s, p_pos))) {
        if (tmp > 0) goto error;  // Fatal error
      } else {
  ...
  more_const_tables_found:
    ...
    */
    ...
    for (JOIN_TAB **pos = stat_vector + const_count; (s = *pos); pos++) {
      ...
      if (table->is_filled_at_execution()) continue;
      ...
      */
      ...
      if (s->dependent)  // If dependent on some table
      {
        ...
        if (table->file->stats.records <= 1L &&
            ...
            !((outer_join & table->map) &&
              (*s->on_expr_ref)->is_expensive())) {  // system table
          int tmp = 0;
          s->type = JT_SYSTEM;
          join->const_table_map |= table->map;
          set_position(join, const_count++, s, (KEYUSE *)0);
          if ((tmp = join_read_const_table(
                   join->thd, s, join->positions + const_count - 1))) {
            if (tmp > 0) goto error;  // Fatal error
          } else
            found_const_table_map |= table->map;
          continue;
          ...
          do {
            if (keyuse->val->type() != Item::NULL_ITEM && !keyuse->optimize &&
              ...
              if (!((~found_const_table_map) & keyuse->used_tables)) {
                ...
                if (keyuse->val->is_expensive()) has_expensive_keyparts = true;
              } else
          ...
          if (eq_part.is_prefix(key_parts) && !table->fulltext_searched &&
            ...
            if (table->actual_key_flags(keyinfo) & HA_NOSAME) {
              if (base_const_ref == base_eq_part && !has_expensive_keyparts &&
                  !((outer_join & table->map) &&
                    (*s->on_expr_ref)
                        ->is_expensive())) {  // Found everything for ref.
                int tmp;
                ref_changed = 1;
                s->type = JT_CONST;
                join->const_table_map |= table->map;
  ...
  join->sort_by_table =
      get_sort_by_table(join->order, join->group_list,
                        join->select_lex->leaf_tables, join->const_table_map);
  /*
  ...
  */
  if (const_count && sargables) {
    for (; sargables->field; sargables++) {
      ...
      if (is_const) join_tab[0].const_keys.merge(possible_keys);
    }
  }
  ...
  if (join->conds && const_count) {
    ...
    if (conds) {
      if (conds->type() == Item::COND_ITEM &&
        ...
        if (!join->cond_equal) join->cond_equal = new COND_EQUAL;
  ...
  for (s = stat; s < stat_end; s++) {
    ...
    if (s->table->is_splittable()) s->add_keyuses_for_splitting();
    ...
    */
    s->worst_seeks =
        MY_MIN((double)s->found_records / 10, (double)s->read_time * 3);
  ...
  if (pull_out_semijoin_tables(join)) DBUG_RETURN(TRUE);
...

  {
    ...
    if (join->const_tables != join->table_count) {
      if (choose_plan(join, all_table_map & ~join->const_table_map)) goto error;
    } else {
    ...
    if (!(join->select_options & SELECT_DESCRIBE) && unit->derived &&
        unit->derived->is_materialized_derived()) {
      /*
      ...
      */
      for (i = 0; i < join->table_count; i++)
        records *= join->best_positions[i].records_read
                       ? (ha_rows)join->best_positions[i].records_read
                       : 1;
...
static void add_key_field(JOIN *join, KEY_FIELD **key_fields, uint and_level,
                          Item_bool_func *cond, Field *field, bool eq_func,
    ...
    if (!(usable_tables & field->table->map)) {
      ...
      /*
        ...
        Field IS NOT NULL
         ...
         Field IN ...
      */
      if (field->flags & PART_KEY_FLAG) stat[0].key_dependent |= used_tables;
      ...
      for (uint i = 0; i < num_values; i++) {
        ...
        if (row_col_no && value[i]->real_item()->type() == Item::ROW_ITEM) {
          ...
          curr_val = value[i];
        if (!(is_const &= curr_val->const_item())) break;
      }
      if (is_const) {
...
static void add_key_equal_fields(JOIN *join, KEY_FIELD **key_fields,
                                 uint and_level, Item_bool_func *cond,
                                 Item *field_item, bool eq_func, Item **val,
                                 uint num_values, table_map usable_tables,
                                 SARGABLE_PARAM **sargables,
                                 uint row_col_no = 0) {
  Field *field = ((Item_field *)(field_item->real_item()))->field;
  add_key_field(join, key_fields, and_level, cond, field, eq_func, val,
                num_values, usable_tables, sargables, row_col_no);
    ...
    */
    ...
    while (it++) {
      ...
      if (!field->eq(equal_field)) {
        add_key_field(join, key_fields, and_level, cond, equal_field, eq_func,
                      val, num_values, usable_tables, sargables, row_col_no);
      }
    }
  }
}
...
void Item_cond_and::add_key_fields(JOIN *join, KEY_FIELD **key_fields,
...
void Item_cond::add_key_fields(JOIN *join, KEY_FIELD **key_fields,
                               uint *and_level, table_map usable_tables,
                               SARGABLE_PARAM **sargables) {
  List_iterator_fast<Item> li(*argument_list());
  KEY_FIELD *org_key_fields = *key_fields;
...
void Item_func_trig_cond::add_key_fields(JOIN *join, KEY_FIELD **key_fields,
                                         uint *and_level,
...
void Item_func_between::add_key_fields(JOIN *join, KEY_FIELD **key_fields,
                                       uint *and_level, table_map usable_tables,
                                       SARGABLE_PARAM **sargables) {
  /*
  ...
  */
  if (is_local_field(args[0])) {
    ...
    add_key_equal_fields(join, key_fields, *and_level, this, field_item,
                         equal_func, &args[1], num_values, usable_tables,
                         sargables);
  }
  /*
...
void Item_func_in::add_key_fields(JOIN *join, KEY_FIELD **key_fields,
                                  uint *and_level, table_map usable_tables,
                                  SARGABLE_PARAM **sargables) {
  if (is_local_field(args[0]) && !(used_tables() & OUTER_REF_TABLE_BIT)) {
    DBUG_ASSERT(arg_count != 2);
    add_key_equal_fields(join, key_fields, *and_level, this,
                         (Item_field *)(args[0]->real_item()), false, args + 1,
    ...
    for (uint i = 0; i < row_cols; i++, key_col++) {
      if (is_local_field(*key_col)) {
        ...
        add_key_equal_fields(join, key_fields, *and_level, this, field_item,
                             false, args + 1, arg_count - 1, usable_tables,
                             sargables, i + 1);
      }
    }
  }
...
void Item_func_ne::add_key_fields(JOIN *join, KEY_FIELD **key_fields,
                                  uint *and_level, table_map usable_tables,
                                  SARGABLE_PARAM **sargables) {
...
void Item_func_like::add_key_fields(JOIN *join, KEY_FIELD **key_fields,
                                    uint *and_level, table_map usable_tables,
                                    SARGABLE_PARAM **sargables) {
  if (is_local_field(args[0]) && with_sargable_pattern()) {
    ...
    */
    add_key_equal_fields(join, key_fields, *and_level, this,
                         (Item_field *)args[0]->real_item(), false, args + 1, 1,
                         usable_tables, sargables);
  }
}

void Item_bool_func2::add_key_fields_optimize_op(
    JOIN *join, KEY_FIELD **key_fields, uint *and_level,
  ...
  if (is_local_field(args[1])) {
    add_key_equal_fields(join, key_fields, *and_level, this,
                         (Item_field *)args[1]->real_item(), equal_func, args,
                         1, usable_tables, sargables);
  }
}

void Item_func_null_predicate::add_key_fields(JOIN *join,
  ...
  if (is_local_field(args[0]) && !(used_tables() & OUTER_REF_TABLE_BIT)) {
    ...
    add_key_equal_fields(join, key_fields, *and_level, this,
                         ...
                         functype() == Item_func::ISNULL_FUNC, &tmp, 1,
                         usable_tables, sargables);
  }
}

void Item_equal::add_key_fields(JOIN *join, KEY_FIELD **key_fields,
                                uint *and_level, table_map usable_tables,
                                SARGABLE_PARAM **sargables) {
  ...
  if (const_item2) {
    ...
    */
    while (it++) {
      Field *equal_field = it.get_curr_field();
      add_key_field(join, key_fields, *and_level, this, equal_field, TRUE,
                    &const_item2, 1, usable_tables, sargables);
    }
  } else {
    /*
    ...
    */
    ...
    while (fi++) {
      ...
      while ((item = it++)) {
        Field *equal_field = it.get_curr_field();
        if (!field->eq(equal_field)) {
          add_key_field(join, key_fields, *and_level, this, field, TRUE, &item,
                        1, usable_tables, sargables);
        }
      }
      it.rewind();
...
static bool add_keyuse(DYNAMIC_ARRAY *keyuse_array, KEY_FIELD *key_field,
                       uint key, uint part) {
  KEYUSE keyuse;
    ...
    for (uint key = 0; key < form->s->keys; key++) {
      ...
      for (uint part = 0; part < key_parts; part++) {
        if (field->eq(form->key_info[key].key_part[part].field) &&
          ...
          if (add_keyuse(keyuse_array, key_field, key, part)) return TRUE;
        }
      }
    }
...
static bool add_ft_keys(DYNAMIC_ARRAY *keyuse_array, JOIN_TAB *stat, COND *cond,
                        table_map usable_tables) {
  ...
  if (cond->type() == Item::FUNC_ITEM) {
    ...
    if (((Item_cond *)cond)->functype() == Item_func::COND_AND_FUNC) {
      Item *item;
      while ((item = li++)) {
        if (add_ft_keys(keyuse_array, stat, item, usable_tables)) return TRUE;
      }
    }
  }
...
static bool update_ref_and_keys(THD *thd, DYNAMIC_ARRAY *keyuse,
  ...
  sel->cond_count = 0;
  sel->between_count = 0;
  if (cond) cond->walk(&Item::count_sargable_conds, 0, sel);
  for (i = 0; i < tables; i++) {
  ...
  }
  {
    ...
    TABLE_LIST *table;
    while ((table = li++)) {
      if (table->nested_join) count_cond_for_nj(sel, table);
    }
  }
  ...
  *sargables = (SARGABLE_PARAM *)key_fields +
  ...
  for (; field != end; field++) {
    if (add_key_part(keyuse, field)) DBUG_RETURN(TRUE);
  }

  if (select_lex->ftfunc_list->elements) {
    if (add_ft_keys(keyuse, join_tab, cond, normal_tables)) DBUG_RETURN(TRUE);
  }
...
bool sort_and_filter_keyuse(THD *thd, DYNAMIC_ARRAY *keyuse,
  ...
  if (insert_dynamic(keyuse, (uchar *)&key_end)) return TRUE;
  ...
  for (sum_item_ptr = join->sum_funcs; *sum_item_ptr; sum_item_ptr++) {
    ...
    */
    for (uint i = 0; i < sum_item->get_arg_count(); i++) {
      ...
      if (out_args) out_args->push_back(item, join->thd->mem_root);
...
static void add_group_and_distinct_keys(JOIN *join, JOIN_TAB *join_tab) {
  ...
  if (join->group_list ||
      join->simple_group) { /* Collect all query fields referenced in the GROUP
                               clause. */
    for (cur_group = join->group_list; cur_group; cur_group = cur_group->next)
      (*cur_group->item)
          ->walk(&Item::collect_item_field_processor, 0, &indexed_fields);
...
inline double matching_candidates_in_table(JOIN_TAB *s,
  ...
  */
  if (with_found_constraint) records -= records / 4;
...
void best_access_path(JOIN *join, JOIN_TAB *s, table_map remaining_tables,
      ...
      if (is_hash_join_key_no(key)) {
        ...
        */
        if (!(remaining_tables & keyuse->used_tables) &&
          ...
          if (!hj_start_key) hj_start_key = keyuse;
          ...
          */
          if (!(remaining_tables & keyuse->used_tables) &&
            ...
            if (tmp2 < best_prev_record_reads) {
              best_part_found_ref =
                  keyuse->used_tables & ~join->const_table_map;
        ...
        if (found_part == PREV_BITS(uint, key_parts) &&
          ...
          if ((key_flags & (HA_NOSAME | HA_NULL_PART_KEY)) == HA_NOSAME ||
            ...
            if (!found_ref) { /* We found a const key */
              ...
              */
              ...
              if (!(records = keyinfo->actual_rec_per_key(
                        key_parts - 1))) { /* Prefer longer keys */
            ...
            if (table->covering_keys.is_set(key))
              ...
              tmp = table->file->read_time(
                  key, 1, (ha_rows)MY_MIN(tmp, s->worst_seeks));
            tmp *= record_count;
          }
        } else {
          /*
          ...
          */
          if ((found_part & 1) &&
              (!(table->file->index_flags(key, 0, 0) & HA_ONLY_WHOLE_INDEX) ||
               found_part ==
                   PREV_BITS(uint, keyinfo->user_defined_key_parts))) {
            ...
            */
                    ...
                    1 + MY_TEST(ref_or_null_part))  //(C3)
            {
              ...
              if ((records = keyinfo->actual_rec_per_key(max_key_part - 1))) {
                ...
                */
                double rec_per_key;
                if (!(rec_per_key = (double)keyinfo->rec_per_key
  ...
  */
       ...
       join->allowed_outer_join_with_cache))  // (2)
  {
    ...
    best_time =
        tmp + (record_count * join_sel) / TIME_FOR_COMPARE * rnd_records;
    best = tmp;
    records = rnd_records;
  ...
  */
  if ((records >= s->found_records || best > s->read_time) &&        // (1)
      !(s->quick && best_key && s->quick->index == best_key->key &&  // (2)
        best_max_key_part >=
            s->table->quick_key_parts[best_key->key]) &&                // (2)
    ...
    if (s->quick) {
      ...
      */
      ...
      if ((s->table->map & join->outer_join) ||
          disable_jbuf)  // Can't use join cache
      {
        /*
    ...
    */
    if (best == DBL_MAX ||
      ...
      */
      ...
      best_ref_depends_map = 0;
      best_uses_jbuf =
          MY_TEST(!disable_jbuf && !((s->table->map & join->outer_join)));
    }
  }
...
static TABLE_LIST *get_emb_subq(JOIN_TAB *tab) {
  ...
  if (tlist->jtbm_subselect) return tlist;
  TABLE_LIST *embedding = tlist->embedding;
  if (!embedding || !embedding->sj_subq_pred) return NULL;
  return embedding;
}
...
static void choose_initial_table_order(JOIN *join) {
  ...
  for (; tab != tabs_end; tab++) {
    if ((emb_subq = get_emb_subq(*tab))) break;
  }
  uint n_subquery_tabs = (uint)(tabs_end - tab);

  if (!n_subquery_tabs) DBUG_VOID_RETURN;
      ...
      */
      ...
      for (JOIN_TAB **top_level_tab = join->best_ref + join->const_tables;
        ...
        if (!need_tables) {
          ...
          memcpy(top_level_tab + 1, subq_tab,
                 sizeof(JOIN_TAB *) * n_subquery_tables);
          break;
        }
      }
...
static int join_tab_cmp(const void *dummy, const void *ptr1, const void *ptr2) {
  ...
  if ((cmp = compare_embedding_subqueries(jt1, jt2)) != 0) return cmp;
  /*
    After that,
...
static int join_tab_cmp_embedded_first(const void *emb, const void *ptr1,
  ...
  if (jt1->emb_sj_nest == emb_nest && jt2->emb_sj_nest != emb_nest) return -1;
  if (jt1->emb_sj_nest != emb_nest && jt2->emb_sj_nest == emb_nest) return 1;
  ...
  for (JOIN_TAB **pos = join->best_ref + idx; (s = *pos); pos++) {
    ...
    if (use_cond_selectivity > 1)
      pushdown_cond_selectivity =
          table_cond_selectivity(join, idx, s, join_tables);
...
static bool greedy_search(JOIN *join, table_map remaining_tables,
  ...
  n_tables = size_remain = my_count_bits(
      remaining_tables &
      (join->emb_sjm_nest
           ? (join->emb_sjm_nest->sj_inner_tables & ~join->const_table_map)
           : ~(table_map)0));
  ...
  do {
    ...
    if (best_extension_by_limited_search(join, remaining_tables, idx,
                                         ...
                                         prune_level, use_cond_selectivity))
      DBUG_RETURN(TRUE);
    /*
...
void JOIN::get_partial_cost_and_fanout(int end_tab_idx, table_map filter_map,
  ...
  */
  ...
  for (tab = first_depth_first_tab(this), i = const_tables; tab;
       tab = next_depth_first_tab(this, tab), i++) {
    end_tab = tab;
    if (i == end_tab_idx) break;
  }
  ...
  for (tab = first_depth_first_tab(this), i = const_tables;;
    ...
    */
    ...
    if (tab->records_read && (cur_table_map & filter_map)) {
      ...
      if (tab->emb_sj_nest) sj_inner_fanout *= tab->records_read;
    }
...
void JOIN::get_prefix_cost_and_fanout(uint n_tables, double *read_time_arg,
                                      double *record_count_arg) {
  double record_count = 1;
  double read_time = 0.0;
...
static double table_multi_eq_cond_selectivity(JOIN *join, uint idx, JOIN_TAB *s,
                                              table_map rem_tables,
                                              uint keyparts,
  ...
  if (!cond_equal || !cond_equal->current_level.elements) return sel;

  if (!s->keyuse) return sel;
  ...
  while ((item_equal = it++)) {
    ...
    */
    ...
    if (!(used_tables & table_bit)) continue;
    if (item_equal->get_const()) continue;

    bool adjust_sel = FALSE;
    Item_equal_fields_iterator fi(*item_equal);
    while ((fi++) && !adjust_sel) {
      Field *fld = fi.get_curr_field();
      if (fld->table->map != table_bit) continue;
      if (pos->key == 0)
        adjust_sel = TRUE;
      else {
        uint i;
        ...
        for (i = 0; i < keyparts; i++) {
          if (i > 0) keyuse += ref_keyuse_steps[i - 1];
          uint fldno;
          if (is_hash_join_key_no(key))
            fldno = keyuse->keypart;
          else
            fldno = table->key_info[key].key_part[i].fieldnr - 1;
          if (fld->field_index == fldno) break;
        }
        keyuse = pos->key;
    ...
    if (adjust_sel) {
      ...
      */
      ...
      while ((fi++)) {
        ...
        Field *fld = fi.get_curr_field();
        if (!(fld->table->map & ~(table_bit | rem_tables))) continue;
        curr_eq_fld_sel =
            get_column_avg_frequency(fld) / fld->table->stat_records();
        if (curr_eq_fld_sel < 1.0) set_if_bigger(eq_fld_sel, curr_eq_fld_sel);
      }
      sel *= eq_fld_sel;
    }
  }
    ...
    while (keyuse->table == table && keyuse->key == key) {
      if (!(keyuse->used_tables & (rem_tables | table->map))) {
        if (are_tables_local(s, keyuse->val->used_tables())) {
          if (is_hash_join_key_no(key)) {
            if (keyparts == keyuse->keypart) keyparts++;
          } else {
          ...
          if (keyparts > keyuse->keypart) {
            ...
            if (keyparts > 1) {
              ref_keyuse_steps[keyparts - 2] =
                  (uint16)(keyuse - prev_ref_keyuse);
  ...
  if (pos->key != NULL) {
    for (Field **f_ptr = table->field; (field = *f_ptr); f_ptr++) {
      if (!bitmap_is_set(read_set, field->field_index) ||
        ...
        continue;
      for (Field *next_field = field->next_equal_field; next_field != field;
...
static bool best_extension_by_limited_search(
  ...
  DBUG_EXECUTE_IF(
      "show_explain_probe_best_ext_lim_search",
  ...
  DBUG_EXECUTE("opt",
               print_plan(join, idx, read_time, record_count, idx, "SOFAR:"););
  ...
  for (JOIN_TAB **pos = join->best_ref + idx; (s = *pos); pos++) {
    ...
    if ((remaining_tables & real_table_bit) &&
      ...
      if (current_read_time >= join->best_read) {
        DBUG_EXECUTE("opt",
                     print_plan(join, idx + 1, current_record_count, read_time,
                                current_read_time, "prune_by_cost"););
      ...
      */
      if (prune_level == 1) {
        if (best_record_count > current_record_count ||
          ...
          DBUG_EXECUTE(
              "opt", print_plan(join, idx + 1, current_record_count, read_time,
                                current_read_time, "pruned_by_heuristic"););
          restore_prev_nj_state(s);
          restore_prev_sj_state(remaining_tables, s, idx);
          continue;
        }
      ...
      double pushdown_cond_selectivity = 1.0;
      if (use_cond_selectivity > 1)
        pushdown_cond_selectivity = table_cond_selectivity(
            join, idx, s, remaining_tables & ~real_table_bit);
      join->positions[idx].cond_selectivity = pushdown_cond_selectivity;
      double partial_join_cardinality =
          current_record_count * pushdown_cond_selectivity;
      if ((search_depth > 1) && (remaining_tables & ~real_table_bit) &
                                    allowed_tables) { /* Recursively expand the
        ...
        swap_variables(JOIN_TAB *, join->best_ref[idx], *pos);
                 ...
                 relations, or the best complete QEP so far, whichever is
                 smaller.
               */
        if (join->sort_by_table &&
            join->sort_by_table !=
...
void JOIN_TAB::calc_used_field_length(bool max_fl) {
  ...
  if (table->maybe_null) rec_length += sizeof(my_bool);
  ...
  if (max_fl) {
    ...
    set_if_smaller(rec_length,
                   table->file->stats.mean_rec_length + rowid_add_size);
...
int JOIN_TAB::make_scan_filter() {
  ...
  Item *cond = is_inner_table_of_outer_join()
                   ? *get_first_inner_table()->on_expr_ref
                   : join->conds;

  if (cond && (tmp = make_cond_for_table(join->thd, cond,
                                         join->const_table_map | table->map,
                                         table->map, -1, FALSE, TRUE))) {
    DBUG_EXECUTE("where", print_where(tmp, "cache", QT_ORDINARY););
    if (!(cache_select = (SQL_SELECT *)join->thd->memdup((uchar *)select,
                                                         sizeof(SQL_SELECT))))
      DBUG_RETURN(1);
...
bool JOIN_TAB::hash_join_is_possible() {
  if (type != JT_REF && type != JT_EQ_REF) return FALSE;
...
/**
  ...
  already chosen and the JOIN_TAB array for the best join order been already
  set. For a given KEYUSE to access this JOIN_TAB in the best execution plan the
...
bool JOIN_TAB::keyuse_is_valid_for_access_in_chosen_plan(JOIN *join,
  ...
  if (!access_from_tables_is_allowed(keyuse->used_tables,
                                     join->sjm_lookup_tables))
    return false;
  if (join->sjm_scan_tables & table->map) return true;
  table_map keyuse_sjm_scan_tables =
      keyuse->used_tables & join->sjm_scan_tables;
  if (!keyuse_sjm_scan_tables) return true;
  uint sjm_tab_nr = 0;
  ...
  for (uint i = 0; i < sjm_sel->item_list.elements; i++) {
    if (sjm_sel->ref_pointer_array[i] == keyuse->val) return true;
  }
  return false;
}
...
double prev_record_reads(POSITION *positions, uint idx, table_map found_ref) {
  ...
  for (POSITION *pos = positions + idx - 1; pos != pos_end; pos--) {
    if (pos->table->table->map & found_ref) {
      ...
      */
      if (pos->records_read) found *= pos->records_read;
    }
  }
  return found;
}
...
static JOIN_TAB *next_breadth_first_tab(JOIN_TAB *first_top_tab,
  ...
  if (!tab->bush_root_tab) {
    ...
    tab++;
    if (tab < first_top_tab + n_top_tabs_count) return tab;
  ...
  */
  for (; tab < first_top_tab + n_top_tabs_count; tab++) {
    if (tab->bush_children) return tab->bush_children->start;
  }
  return NULL;
}
...
JOIN_TAB *next_explain_order_tab(JOIN *join, JOIN_TAB *tab) {
  ...
  if (tab->last_leaf_in_bush) return tab->bush_root_tab;
...
JOIN_TAB *next_top_level_tab(JOIN *join, JOIN_TAB *tab) {
  ...
  if (tab && tab->bush_root_tab) tab = NULL;
  return tab;
}
...
JOIN_TAB *first_linear_tab(JOIN *join,
  ...
  if (const_tbls == WITHOUT_CONST_TABLES) first += join->const_tables;
...
JOIN_TAB *next_linear_tab(JOIN *join, JOIN_TAB *tab,
  ...
  {
    /* Inside SJM nest */
    if (!tab->last_leaf_in_bush) return tab + 1; /* Return next in nest */
...
bool JOIN::check_two_phase_optimization(THD *thd) {
  if (check_for_splittable_materialized()) return true;
  return false;
}
...
bool JOIN::inject_cond_into_where(Item *injected_cond) {
  ...
  if (conds && conds->type() == Item::COND_ITEM &&
    ...
    if (cond_equal) and_args->disjoin((List<Item> *)&cond_equal->current_level);
  }

  where_item = and_items(thd, conds, where_item);
  if (!where_item->fixed && where_item->fix_fields(thd, 0)) return true;
 ...
 */
  ...
  if (select_lex->window_specs.elements) aggr_tables++;

  if (aggr_tables > 2) aggr_tables = 2;
  if (!(join_tab = (JOIN_TAB *)thd->alloc(sizeof(JOIN_TAB) *
  ...
  if (!(root_range = new (thd->mem_root) JOIN_TAB_RANGE)) DBUG_RETURN(TRUE);
  root_range->start = join_tab;
  /* root_range->end will be set later */
  join_tab_ranges.empty();

  if (join_tab_ranges.push_back(root_range, thd->mem_root)) DBUG_RETURN(TRUE);
  ...
  for (j = join_tab, tablenr = 0; tablenr < table_count; tablenr++, j++) {
    ...
    if (cur_pos->sj_strategy == SJ_OPT_MATERIALIZE ||
      ...
      */
      ...
      j->keys = key_map(
          1); /* The unique index is always in 'possible keys' in EXPLAIN */
    ...
    *j = *best_positions[tablenr].table;
    ...
    if (j->type == JT_CONST) goto loop_end;  // Handled in make_join_stat..
    ...
    if (j->type == JT_SYSTEM) goto loop_end;
  ...
  for (j = join_tab, tablenr = 0; tablenr < table_count; tablenr++, j++) {
    if (j->bush_children) j = j->bush_children->start;
    ...
    if (j->last_leaf_in_bush) j = j->bush_root_tab;
  }

  top_join_tab_count =
      (uint)(join_tab_ranges.head()->end - join_tab_ranges.head()->start);
...
static bool create_hj_key_for_table(JOIN *join, JOIN_TAB *join_tab,
  ...
  if (!keyinfo->rec_per_key) DBUG_RETURN(TRUE);
  keyinfo->key_part = key_part_info;
  ...
  if (jtab->bush_root_tab) {
    ...
    */
    table_map local_tables = jtab->emb_sj_nest->nested_join->used_tables |
                             jtab->join->const_table_map | OUTER_REF_TABLE_BIT;
  ...
  if (ftkey) {
    ...
    for (i = 0; i < keyparts; keyuse++, i++) {
      while (((~used_tables) & keyuse->used_tables) ||
             ...
             !j->keyuse_is_valid_for_access_in_chosen_plan(join, keyuse) ||
             keyuse->keypart == NO_KEYPART ||
             (keyuse->keypart != (is_hash_join_key_no(key)
      ...
      if (keyuse->null_rejecting) j->ref.null_rejecting |= (key_part_map)1 << i;
      ...
      */
      if (!keyuse->val->used_tables() &&
          !thd->lex->describe) {  // Compare against constant
        store_key_item tmp(thd, keyinfo->key_part[i].field,
                           key_buff + maybe_null, maybe_null ? key_buff : 0,
                           keyinfo->key_part[i].length, keyuse->val, FALSE);
        if (unlikely(thd->is_fatal_error)) DBUG_RETURN(TRUE);
        tmp.copy();
        j->ref.const_ref_part_map |= key_part_map(1) << i;
      } else
        *ref_key++ = get_store_key(thd, keyuse, join->const_table_map,
                                   &keyinfo->key_part[i], key_buff, maybe_null);
      /*
  ...
  *ref_key = 0;  // end_marker
  ...
  if (j->type == JT_CONST)
    j->table->const_table = 1;
             ...
             (keyparts > keyinfo->user_defined_key_parts &&  // true only for
                                                             // extended keys
              MY_TEST(key_flags & HA_EXT_NOSAME) &&
  ...
  j->read_record.unlock_row =
      (j->type == JT_EQ_REF) ? join_read_key_unlock_row : rr_unlock_row;
  DBUG_RETURN(0);
}
        ...
        if (tab->ref.null_rejecting & ((key_part_map)1 << keypart)) {
          ...
          */
          ...
          DBUG_EXECUTE(
              "where",
              print_where(notnull,
                          (referred_tab ? referred_tab->table->alias.c_ptr()
                                        : "outer_ref_cond"),
                          QT_ORDINARY););
          if (!tab->first_inner) {
...
static COND *add_found_match_trig_cond(THD *thd, JOIN_TAB *tab, COND *cond,
                                       JOIN_TAB *root_tab) {
  COND *tmp;
  DBUG_ASSERT(cond != 0);
  if (tab == root_tab) return cond;
  ...
  */
  ...
  for (tab = first_linear_tab(join, WITH_BUSH_ROOTS, WITHOUT_CONST_TABLES); tab;
    ...
    if (tab->bush_children) {
      if (setup_sj_materialization_part1(tab)) DBUG_RETURN(TRUE);
  ...
  for (tab = first_linear_tab(join, WITH_BUSH_ROOTS, WITHOUT_CONST_TABLES); tab;
    ...
    for (; embedding; embedding = embedding->embedding) {
      if (embedding->is_active_sjm()) {
        /* We're trying to walk out of an SJ-Materialization nest. Don't do
         * this.  */
        break;
      }
      /* Ignore sj-nests: */
      ...
      if (++nested_join->counter < nested_join->n_tables) break;
  ...
  if (select) {
    ...
    */
    ...
    {         /* there may be a select without a cond. */
      ...
      */
      {  // Check const tables
        join->exec_const_cond = make_cond_for_table(
            thd, cond, join->const_table_map, (table_map)0, -1, FALSE, FALSE);
        ...
        if (join->table_count != join->const_tables) {
          COND *outer_ref_cond = make_cond_for_table(
              thd, cond, join->const_table_map | OUTER_REF_TABLE_BIT,
          ...
          COND *pseudo_bits_cond = make_cond_for_table(
              thd, cond, join->const_table_map | PSEUDO_TABLE_BITS,
    ...
    */
    ...
    for (i = join->top_join_tab_count - 1; i >= join->const_tables; i--) {
      if (!join->join_tab[i].bush_children) break;
    }
    ...
    for (tab = first_depth_first_tab(join); tab;
      ...
      */
      ...
      if (cond) {
        ...
        /* Add conditions added by add_not_null_conds(). */
        if (tab->select_cond) add_cond_and_fix(thd, &tmp, tab->select_cond);
      }
      ...
      if (cond && !tmp && tab->quick) {  // Outer join
        if (tab->type != JT_ALL && !is_hj) {
          ...
          */
          ...
          tmp =
              new (thd->mem_root) Item_int(thd, (longlong)1, 1);  // Always true
        }
      }
      if (tmp || !cond || tab->type == JT_REF || tab->type == JT_REF_OR_NULL ||
        ...
        SQL_SELECT *sel = tab->select =
            ((SQL_SELECT *)thd->memdup((uchar *)select, sizeof(*select)));
        ...
        */
        if (cond && tmp) {
          ...
          */
          COND *tmp_cond;
          if (!(tmp_cond =
                    add_found_match_trig_cond(thd, first_inner_tab, tmp, 0)))
            DBUG_RETURN(1);
          sel->cond = tmp_cond;
          ...
          if (tab->table) {
            ...
            if ((tab->table->file->ha_table_flags() &
              ...
              COND *push_cond = make_cond_for_table(
                  thd, tmp_cond, current_map, current_map, -1, FALSE, FALSE);
              if (push_cond) {
        ...
        uint ref_key =
            sel->head ? (uint)sel->head->reginfo.join_tab->ref.key + 1 : 0;
        if (i == join->const_tables && ref_key) {
          ...
          */
          ...
          if (i != join->const_tables && tab->use_quick != 2 &&
              !tab->first_inner) { /* Read with cache */
            if (tab->make_scan_filter()) DBUG_RETURN(1);
          }
        }
      }
      ...
      */
      ...
      for (JOIN_TAB *join_tab = start_from; join_tab != end_with; join_tab++) {
        if (*join_tab->on_expr_ref) {
          ...
          if (!tmp_cond) continue;
          tmp_cond = new (thd->mem_root)
              Item_func_trig_cond(thd, tmp_cond, &cond_tab->not_null_compl);
          if (!tmp_cond) DBUG_RETURN(1);
          ...
          if (cond_tab->select) cond_tab->select->cond = cond_tab->select_cond;
        }
      }
      ...
      */
      while (first_inner_tab && first_inner_tab->last_inner == last_tab) {
        ...
        */
        ...
        table_map used_tables2 =
            (join->const_table_map | OUTER_REF_TABLE_BIT | RAND_TABLE_BIT);

        start_from = tab->bush_root_tab
                         ? tab->bush_root_tab->bush_children->start
                         : join->join_tab + join->const_tables;
        for (JOIN_TAB *inner_tab = start_from; inner_tab <= last_tab;
             inner_tab++) {
          ...
          */
          COND *tmp_cond =
              make_cond_for_table(thd, on_expr, used_tables2, current_map,
          ...
          if (inner_tab->bush_children) {
            ...
            */
            ...
            if (sjm->is_used && !sjm->is_sj_scan) is_sjm_lookup_tab = TRUE;
          }
          ...
          if (tmp_cond && !is_sjm_lookup_tab) {
            JOIN_TAB *cond_tab =
                (inner_tab < first_inner_tab ? first_inner_tab : inner_tab);
            Item **sel_cond_ref =
                (inner_tab < first_inner_tab ? &first_inner_tab->on_precond
                                             : &inner_tab->select_cond);
            /*
              First add the guards for match variables of
              all embedding outer join operations.
            */
            if (!(tmp_cond = add_found_match_trig_cond(
                      thd, cond_tab->first_inner, tmp_cond, first_inner_tab)))
              DBUG_RETURN(1);
            /*
            ...
            */
            ...
            tmp_cond = new (thd->mem_root) Item_func_trig_cond(
                thd, tmp_cond, &first_inner_tab->not_null_compl);
            ...
            *sel_cond_ref = !(*sel_cond_ref)
                                ? tmp_cond
                                : new (thd->mem_root) Item_cond_and(
                                      thd, *sel_cond_ref, tmp_cond);
            DBUG_PRINT("info", ("Item_cond_and %p", (*sel_cond_ref)));
            if (!(*sel_cond_ref)) DBUG_RETURN(1);
...
static uint get_next_field_for_derived_key(uchar *arg) {
  ...
  if (!keyuse) return (uint)(-1);
  TABLE *table = keyuse->table;
  uint key = keyuse->key;
  uint fldno = keyuse->keypart;
  uint keypart =
      keyuse->keypart_map == (key_part_map)1 ? 0 : (keyuse - 1)->keypart + 1;
  for (;
       keyuse->table == table && keyuse->key == key && keyuse->keypart == fldno;
       keyuse++)
    keyuse->keypart = keypart;
  if (keyuse->key != key) keyuse = 0;
  *((KEYUSE **)arg) = keyuse;
  return fldno;
}
...
static bool generate_derived_keys_for_table(KEYUSE *keyuse, uint count,
                                            uint keys) {
  TABLE *table = keyuse->table;
  if (table->alloc_keys(keys)) return TRUE;
  uint key_count = 0;
...
static bool generate_derived_keys(DYNAMIC_ARRAY *keyuse_array) {
  ...
  for (uint i = 0; i < elements; i++, keyuse++) {
    if (!keyuse->table) break;
    KEYUSE *first_table_keyuse = NULL;
    table_map last_used_tables = 0;
    uint count = 0;
    uint keys = 0;
    TABLE_LIST *derived = NULL;
    if (keyuse->table != prev_table) derived = keyuse->table->pos_in_table_list;
...
void JOIN::drop_unused_derived_keys() {
  ...
  for (tab = first_linear_tab(this, WITH_BUSH_ROOTS, WITHOUT_CONST_TABLES); tab;
    ...
    if (!tmp_tbl) continue;
    if (!tmp_tbl->pos_in_table_list->is_materialized_derived()) continue;
...
static uint make_join_orderinfo(JOIN *join) {
  ...
  */
  ...
  JOIN_TAB *tab;
  if (join->need_tmp) return join->table_count;
...
static void revise_cache_usage(JOIN_TAB *join_tab) {
  ...
  if (join_tab->first_inner) {
    ...
    for (first_inner = join_tab->first_inner; first_inner;
         first_inner = first_inner->first_upper) {
...
/*
  ...
  SYNOPSIS
    end_sj_materialize()
      join            The join
      join_tab        Points to right after the last join_tab in materialization
  bush end_of_records  FALSE <=> This call is made to pass another record
                                combination
                      TRUE  <=> EOF (no action)
...
enum_nested_loop_state end_sj_materialize(JOIN *join, JOIN_TAB *join_tab,
                                          bool end_of_records) {
  int error;
  THD *thd = join->thd;
    ...
    if (unlikely((error = table->file->ha_write_tmp_row(table->record[0])))) {
      ...
      if (table->file->is_fatal_error(error, HA_CHECK_DUP) &&
          create_internal_tmp_table_from_heap(
              thd, table, sjm->sjm_table_param.start_recinfo,
...
static uint check_join_cache_usage(JOIN_TAB *tab, ulonglong options,
                                   uint no_jbuf_after, uint table_index,
                                   JOIN_TAB *prev_tab) {
  Cost_estimate cost;
  uint flags = 0;
  ha_rows rows = 0;
  ...
  */
  if (cache_level == 0 || !prev_tab) return 0;

  if (force_unlinked_cache && (cache_level % 2 == 0)) cache_level--;

  if (options & SELECT_NO_JOIN_CACHE) goto no_join_cache;

  if (tab->use_quick == 2) goto no_join_cache;

  if (tab->table->map & join->complex_firstmatch_tables) goto no_join_cache;
  ...
  */
  if (tab->inside_loosescan_range) goto no_join_cache;
  ...
  */
  if (tab->is_nested_inner()) {
    if (force_unlinked_cache || cache_level == 1) goto no_join_cache;
    if (cache_level & 1) cache_level--;
  }
  ...
  */
  if (table_index > no_jbuf_after) goto no_join_cache;

  /*
    TODO: BNL join buffer should be perfectly ok with tab->bush_children.
  */
  if (tab->loosescan_match_tab || tab->bush_children) goto no_join_cache;
  ...
  if (!prev_tab->use_join_cache) {
    ...
    */
    if (tab->first_inner && tab != tab->first_inner) {
      for (JOIN_TAB *first_inner = tab[-1].first_inner; first_inner;
           first_inner = first_inner->first_upper) {
        if (first_inner == tab->first_inner) goto no_join_cache;
      }
  ...
  switch (tab->type) {
    ...
    case JT_EQ_REF:
      if (cache_level <= 2 || (no_hashed_cache && no_bka_cache))
        goto no_join_cache;
      if (tab->ref.is_access_triggered()) goto no_join_cache;
      ...
      if (!tab->is_ref_for_hash_join() && !no_bka_cache) {
        ...
        rows = tab->table->file->multi_range_read_info(
            tab->ref.key, 10, 20, tab->ref.key_parts, &bufsz, &flags, &cost);
      }
      ...
      if (cache_level > 4 && no_bka_cache) goto no_join_cache;
      ...
      if ((rows != HA_POS_ERROR) && !(flags & HA_MRR_USE_DEFAULT_IMPL)) {
        if (cache_level <= 6 || no_hashed_cache) {
          if (cache_level == 5) prev_cache = 0;
          if ((tab->cache =
                   new (root) JOIN_CACHE_BKA(join, tab, flags, prev_cache)))
            return (6 - MY_TEST(!prev_cache));
          goto no_join_cache;
        } else {
          if (cache_level == 7) prev_cache = 0;
          if ((tab->cache =
                   new (root) JOIN_CACHE_BKAH(join, tab, flags, prev_cache))) {
  ...
  restart:
    ...
    */
    ...
    switch (tab->type) {
      ...
      case JT_REF_OR_NULL:
      case JT_ALL:
        tab->used_join_cache_level =
            check_join_cache_usage(tab, options, no_jbuf_after, idx, prev_tab);
...
void JOIN_TAB::remove_redundant_bnl_scan_conds() {
  ...
  */
  if (select && select->cond != select_cond) return;

  if (is_cond_and(select_cond)) {
    List_iterator<Item> pushed_cond_li(
        *((Item_cond *)select_cond)->argument_list());
    Item *pushed_item;
    Item_cond_and *reduced_select_cond =
        new (join->thd->mem_root) Item_cond_and(join->thd);

    if (is_cond_and(cache_select->cond)) {
      List_iterator<Item> scan_cond_li(
          *((Item_cond *)cache_select->cond)->argument_list());
      Item *scan_item;
...
static bool make_join_readinfo(JOIN *join, ulonglong options,
  ...
  JOIN_TAB *first_tab;
  for (tab = first_tab =
           first_linear_tab(join, WITH_BUSH_ROOTS, WITHOUT_CONST_TABLES);
       tab; tab = next_linear_tab(join, tab, WITH_BUSH_ROOTS)) {
    if (tab->bush_children) {
      if (setup_sj_materialization_part2(tab)) return TRUE;
    }
    ...
    */
    ...
    if (jcl) tab[-1].next_select = sub_select_cache;
    ...
    switch (tab->type) {
      ...
      case JT_CONST:   // Only happens with left join
        ...
        tab->read_first_record =
            tab->type == JT_SYSTEM ? join_read_system : join_read_const;
        ...
        if (tab->use_quick == 2) {
          ...
          if (tab == first_tab) {
            if (tab->select && tab->select->quick) {
              if (statistics) join->thd->inc_status_select_range();
            } else {
            ...
            if (tab->select && tab->select->quick) {
              if (statistics) join->thd->inc_status_select_full_range_join();
            } else {
              join->thd->set_status_no_index_used();
              if (statistics) {
            ...
            if (tab->select && tab->select->quick &&
              ...
              if (tab->loosescan_match_tab)
                ...
                /*
                  ...
                  up for disk-bound loads, slows them down for cases when the
                  data is in disk cache (see BUG#35850): See bug #26447: "Using
                  the clustered index for a table scan is always faster than
    ...
    DBUG_EXECUTE(
        "where", char buff[256];
        String str(buff, sizeof(buff), system_charset_info); str.length(0);
  ...
  */
  for (tab = join->join_tab + join->const_tables;
    ...
    if (tab->use_join_cache) {
      JOIN_TAB *sort_by_tab =
          join->group && join->simple_group && join->group_list
              ? join->join_tab + join->const_tables
      ...
      if (sort_by_tab) {
        ...
        if (sort_by_tab->type == JT_NEXT &&
          ...
          sort_by_tab->read_first_record = join_init_read_record;
                   ...
                   !sort_by_tab->table->covering_keys.is_set(
                       sort_by_tab->index)) {
  ...
  for (JOIN_TAB *tab = first_top_level_tab(join, WITH_CONST_TABLES); tab;
    ...
    if (tab->type == JT_ALL && (!tab->select || !tab->select->quick)) {
      my_message(ER_UPDATE_WITHOUT_KEY_IN_SAFE_MODE,
                 ER_THD(join->thd, ER_UPDATE_WITHOUT_KEY_IN_SAFE_MODE), MYF(0));
      return (1);
    }
  }
  return (0);
...
void JOIN_TAB::cleanup() {
  ...
  DBUG_PRINT("enter",
             ("tab: %p  table %s.%s", this, (table ? table->s->db.str : "?"),
...
double JOIN_TAB::scan_time() {
  ...
  if (table->is_created()) {
    ...
    found_records = records = table->stat_records();
    read_time =
        found_records ? (double)found_records : 10.0;  // TODO:fix this stub
    res = read_time;
  }
  return res;
}
...
ha_rows JOIN_TAB::get_examined_rows() {
  ...
  else if (type == JT_NEXT || type == JT_ALL || type == JT_HASH ||
           type == JT_HASH_NEXT) {
    if (limit) {
      /*
...
bool JOIN_TAB::preread_init() {
  ...
  if (!(derived->get_unit()->uncacheable & UNCACHEABLE_DEPENDENT) ||
    ...
    preread_init_done = TRUE;
  if (select && select->quick) select->quick->replace_handler(table->file);

  DBUG_EXECUTE_IF(
      "show_explain_probe_join_tab_preread",
...
bool TABLE_REF::tmp_table_index_lookup_init(THD *thd, KEY *tmp_key,
                                            Item_iterator &it, bool value,
                                            uint skip) {
  ...
  if (!(key_buff = (uchar *)thd->calloc(ALIGN_SIZE(tmp_key->key_length) * 2)) ||
      !(key_copy = (store_key **)thd->alloc(
            (sizeof(store_key *) * (tmp_key_parts + 1)))) ||
  ...
  for (i = 0; i < tmp_key_parts; i++, cur_key_part++, ref_key++) {
    ...
    *ref_key = new store_key_item(thd, cur_key_part->field,
                                  /* TIMOUR:
                                     ...
                                     cur_ref_buff + MY_TEST(maybe_null), we
                                     could use that information instead.
                                  */
...
void JOIN::join_free() {
  ...
  */
  if (can_unlock && lock && thd->lock && !thd->locked_tables_mode &&
      !(select_options & SELECT_NO_UNLOCK) && !select_lex->subquery_in_having &&
...
void JOIN::cleanup(bool full) {
  ...
  if (full) have_query_plan = QEP_DELETED;
    ...
    if (full) {
      ...
      */
      ...
      for (tab = first_linear_tab(this, WITH_BUSH_ROOTS, WITH_CONST_TABLES);
        ...
        DBUG_PRINT("info",
                   ("close index: %s.%s  alias: %s", tab->table->s->db.str,
                    tab->table->s->table_name.str, tab->table->alias.c_ptr()));
  ...
  if (full) {
    ...
    */
    ...
    if (!join_tab) {
      ...
      while ((table_ref = li++)) {
        if (table_ref->table && table_ref->jtbm_subselect &&
...
static void update_depend_map(JOIN *join) {
  ...
  for (join_tab = first_linear_tab(join, WITH_BUSH_ROOTS, WITH_CONST_TABLES);
       join_tab; join_tab = next_linear_tab(join, join_tab, WITH_BUSH_ROOTS)) {
...
static void update_depend_map_for_order(JOIN *join, ORDER *order) {
  for (; order; order = order->next) {
    ...
    if (!(order->depend_map & (OUTER_REF_TABLE_BIT | RAND_TABLE_BIT)) &&
        !order->item[0]->with_sum_func && join->join_tab) {
      for (JOIN_TAB **tab = join->map2table; depend_map;
           tab++, depend_map >>= 1) {
...
static ORDER *remove_const(JOIN *join, ORDER *first_order, COND *cond,
  ...
  */
  if (join->join_tab) {
    ...
    */
    for (JOIN_TAB *tab = join->join_tab + join->const_tables;
         tab < join->join_tab + join->table_count; tab++)
  ...
  for (order = first_order; order; order = order->next) {
          ...
          */
          if (optimizer_flag(join->thd, OPTIMIZER_SWITCH_ORDERBY_EQ_PROP) &&
            ...
            /*
              ...
              accordingly.
            */
            Item *res = item->propagate_equal_fields(
                join->thd, Value_source::Context_identity(), join->cond_equal);
            Item_equal *item_eq;
            if ((item_eq = res->get_item_equal())) {
              Item *first = item_eq->get_first(NO_PARTICULAR_TAB, NULL);
              if (first->const_item() ||
                  first->used_tables() == first_table_bit) {
    ...
    for (tmp_order = first_order; tmp_order != order;
         tmp_order = tmp_order->next) {
      if (tmp_order->item[0]->eq(order->item[0], 1)) break;
    }
    if (tmp_order != order) continue;  // Duplicate order by. Remove
...
static int return_zero_rows(JOIN *join, select_result *result,
  ...
  if (!(result->send_result_set_metadata(
    ...
    if (send_row) send_error = result->send_data(fields) > 0;
    if (likely(!send_error)) result->send_eof();  // Should be safe
  }
  DBUG_RETURN(0);
}
...
static bool check_simple_equality(THD *thd, const Item::Context &ctx,
  ...
  if (left_item->type() == Item::REF_ITEM &&
    ...
    if (((Item_ref *)left_item)->get_depended_from()) return FALSE;
    left_item = left_item->real_item();
  }
  if (right_item->type() == Item::REF_ITEM &&
      ((Item_ref *)right_item)->ref_type() == Item_ref::VIEW_REF) {
    if (((Item_ref *)right_item)->get_depended_from()) return FALSE;
    ...
    if (left_item_equal) {
      ...
      if (right_item_equal)
        ...
        Type_handler_hybrid_field_type tmp(
            orig_left_item->type_handler_for_comparison());
        if (tmp.aggregate_for_comparison(
                orig_right_item->type_handler_for_comparison()))
          return false;
        Item_equal *item_equal = new (thd->mem_root) Item_equal(
            thd, tmp.type_handler(), orig_left_item, orig_right_item, false);
...

  {
    ...
    if (const_item &&
      ...
      */
      ...
      Item *const_item2 =
          field_item->field->get_equal_const_item(thd, ctx, const_item);
      if (!const_item2) return false;
      ...
      if (item_equal) {
        ...
        */
        ...
        if (tmp.aggregate_for_comparison(
                orig_right_item->type_handler_for_comparison()))
          return false;
        item_equal = new (thd->mem_root) Item_equal(
            thd, tmp.type_handler(), const_item2, orig_field_item, true);
        item_equal->set_context_field(field_item);
        cond_equal->current_level.push_back(item_equal, thd->mem_root);
      }
      return TRUE;
  ...
  for (uint i = 0; i < n; i++) {
    ...
    if (left_item->type() == Item::ROW_ITEM &&
      ...
      */
      ...
      is_converted = check_row_equality(
          thd, comparators[i].subcomparators(), (Item_row *)left_item,
          (Item_row *)right_item, cond_equal, eq_list);
    } else {
      const Arg_comparator *tmp = &comparators[i];
      is_converted = check_simple_equality(
          thd,
          Item::Context(Item::ANY_SUBST, tmp->compare_type_handler(),
    ...
    if (!is_converted) {
      ...
      if (!(eq_item =
                new (thd->mem_root) Item_func_eq(thd, left_item, right_item)) ||
  ...
  if (left_item->type() == Item::ROW_ITEM &&
    ...
    */
    if (left_item->get_item_splocal() || right_item->get_item_splocal())
      return false;
    return check_row_equality(thd, cmp.subcomparators(), (Item_row *)left_item,
                              (Item_row *)right_item, cond_equal, eq_list);
  }
  return check_simple_equality(
      thd, Context(ANY_SUBST, compare_type_handler(), compare_collation()),
      left_item, right_item, cond_equal);
}
...
COND *Item_cond_and::build_equal_items(THD *thd, COND_EQUAL *inherited,
 ...
 */
  while ((item = li++)) {
    ...
    */
    if (item->check_equality(thd, &cond_equal, &eq_list)) li.remove();
  }
  ...
  /*
    ...
    (a=a AND b=b AND a=a).
  */
  if (!cond_args->elements && !cond_equal.current_level.elements &&
      !eq_list.elements)
  ...
  */
  ...
  update_used_tables();
  if (cond_equal_ref) *cond_equal_ref = &m_cond_equal;
  return this;
}

COND *Item_cond::build_equal_items(THD *thd, COND_EQUAL *inherited,
                                   bool link_item_fields,
                                   COND_EQUAL **cond_equal_ref) {
  List<Item> *cond_args = argument_list();
...
COND *Item_func_eq::build_equal_items(THD *thd, COND_EQUAL *inherited,
                                      bool link_item_fields,
                                      COND_EQUAL **cond_equal_ref) {
  COND_EQUAL cond_equal;
  cond_equal.upper_levels = inherited;
  ...
  if (Item_func_eq::check_equality(thd, &cond_equal, &eq_list)) {
    ...
    if (n == 0)
      ...
      if ((item_equal = cond_equal.current_level.pop())) {
        ...
        if (cond_equal_ref)
          *cond_equal_ref =
              new (thd->mem_root) COND_EQUAL(item_equal, thd->mem_root);
        return item_equal;
      }
      ...
      */
      ...
      if (cond_equal_ref) *cond_equal_ref = &and_cond->m_cond_equal;
      return and_cond;
    }
  }
  ...
  if (join_list && !ignore_on_conds) {
    ...
    while ((table = li++)) {
      if (table->on_expr) {
        ...
        */
        table->on_expr =
            build_equal_items(join, table->on_expr, inherited, nested_join_list,
                              ignore_on_conds, &table->cond_equal);
      }
    }
  }
...
static int compare_fields_by_table_order(Item *field1, Item *field2,
                                         void *table_join_idx) {
  int cmp = 0;
  bool outer_ref = 0;
  Item_field *f1 = (Item_field *)(field1->real_item());
  Item_field *f2 = (Item_field *)(field2->real_item());
  if (field1->const_item() || f1->const_item()) return -1;
  if (field2->const_item() || f2->const_item()) return 1;
  ...
  */
  if (tab1->bush_root_tab != tab2->bush_root_tab) {
    if (tab1->bush_root_tab) tab1 = tab1->bush_root_tab;
  ...
  if (!cmp) {
    ...
    if (keyno != MAX_KEY) {
      if (f1->field->part_of_key.is_set(keyno)) cmp = -1;
      if (f2->field->part_of_key.is_set(keyno)) cmp++;
      /*
        Here:
    ...
    if (!cmp) cmp = f1->field->field_index - f2->field->field_index;
  }
...
Item *eliminate_item_equal(THD *thd, COND *cond, COND_EQUAL *upper_levels,
  ...
  DBUG_ASSERT(!cond || cond->type() == Item::INT_ITEM ||
    ...
    */
    if (upper) {
      ...
      if (item_const && upper_const) {
        ...
        */
        Item_func_eq *func =
            new (thd->mem_root) Item_func_eq(thd, item_const, upper_const);
        func->set_cmp_func();
        func->quick_fix_field();
        if (func->val_int()) item = 0;
      } else {
    ...
    if (!item_const && field_sjm && field_sjm != current_sjm) {
      ...
      if (!field_sjm->sj_mat_info->is_sj_scan) produce_equality = FALSE;
    }

    if (produce_equality) {
      if (eq_item && eq_list.push_back(eq_item, thd->mem_root)) return 0;
      ...
      */
      Item *head_item =
          (!item_const && current_sjm && current_sjm_head != field_item)
              ? current_sjm_head
              : head;
      ...
      eq_item = new (thd->mem_root)
          Item_func_eq(thd, field_item->real_item(), head_item);

      if (!eq_item || eq_item->set_cmp_func()) return 0;
      eq_item->quick_fix_field();
    }
  ...
  */
  ...
  switch (eq_list.elements) {
    case 0:
      ...
      break;
    case 1:
      if (!cond || cond->type() == Item::INT_ITEM) res = eq_item;
      break;
    default:
      break;
  }
  ...
  if (!res) res = new (thd->mem_root) Item_cond_and(thd, eq_list);
  if (res) {
    res->quick_fix_field();
    res->update_used_tables();
  }
...
static COND *substitute_for_best_equal_field(THD *thd, JOIN_TAB *context_tab,
  ...
  if (cond->type() == Item::COND_ITEM) {
    ...
    if (and_level) {
      ...
      cond_list->disjoin(
          (List<Item> *)&cond_equal
              ->current_level); /* remove Item_equal objects from the AND. */
    ...
    Item *item;
    while ((item = li++)) {
      Item *new_item = substitute_for_best_equal_field(
          thd, context_tab, item, cond_equal, table_join_idx);
      /*
        This works OK with PS/SP re-execution as changes are made to
        the arguments of AND/OR items only
      */
      if (new_item && new_item != item) li.replace(new_item);
    }
    ...
    if (and_level) {
      ...
      if (eq_cond && !false_eq_cond) {
        ...
        if (eq_cond->type() == Item::COND_ITEM)
          ((Item_cond *)cond)
              ->add_at_head(((Item_cond *)eq_cond)->argument_list());
        else {
...
static void update_const_equal_items(THD *thd, COND *cond, JOIN_TAB *tab,
                                     bool const_key) {
  if (!(cond->used_tables() & tab->table->map)) return;
        ...
        */
        if (!possible_keys.is_clear_all()) {
          ...
          for (use = stat->keyuse; use && use->table == field_tab; use++)
            if (const_key && !use->is_for_hash_join() &&
...
*/
static bool can_change_cond_ref_to_const(Item_bool_func2 *target,
                                         Item *target_expr, Item *target_value,
                                         Item_bool_func2 *source,
                                         Item *source_expr,
                                         Item *source_const) {
  return target_expr->eq(source_expr, 0) && target_value != source_const &&
         target->compare_type_handler()->can_change_cond_ref_to_const(
             target, target_expr, target_value, source, source_expr,
             source_const);
}
      ...
      while ((cond_cmp = cond_itr++)) {
        ...
        if (!args[0]->const_item())
          change_cond_ref_to_const(thd, &save, cond_cmp->and_level,
                                   cond_cmp->and_level, cond_cmp->cmp_func,
                                   args[0], args[1]);
      }
    }
    ...
    if (cond->type() == Item::FUNC_ITEM &&
      ...
      if (!(left_const && right_const) &&
        ...
        if (right_const) {
          ...
          func->update_used_tables();
          change_cond_ref_to_const(thd, save_list, and_father, and_father, func,
                                   args[0], args[1]);
        } else if (left_const) {
  ...
  while ((table = li++)) {
    ...
    if (table->sj_on_expr && !in_sj) {
      ...
      */
      ...
      while ((tbl = it++)) {
        if (!tbl->on_expr && tbl->table) tbl->table->maybe_null = FALSE;
      }
...
static uint reset_nj_counters(JOIN *join, List<TABLE_LIST> *join_list) {
  ...
  while ((table = li++)) {
    ...
    if ((nested_join = table->nested_join)) {
      ...
      if (!nested_join->n_tables) is_eliminated_nest = TRUE;
    }
...
static void restore_prev_nj_state(JOIN_TAB *last) {
  ...
  for (; last_emb != NULL && last_emb != join->emb_sjm_nest;
    ...
    if (!last_emb->sj_on_expr) {
      ...
      if (--nest->counter == 0) join->cur_embedding_map &= ~nest->nj_map;
...
void optimize_wo_join_buffering(JOIN *join, uint first_tab, uint last_tab,
                                table_map last_remaining_tables, bool first_alt,
                                uint no_jbuf_before, double *outer_rec_count,
                                double *reopt_cost) {
  ...
  */
  ...
  for (i = first_tab; i <= last_tab; i++) {
    ...
    if ((i == first_tab && first_alt) || join->positions[i].use_join_buffer) {
      ...
      best_access_path(join, rs, reopt_remaining_tables, i, TRUE, rec_count,
                       &pos, &loose_scan_pos);
    } else
      pos = join->positions[i];

    if ((i == first_tab && first_alt)) pos = loose_scan_pos;

    reopt_remaining_tables &= ~rs->table->map;
    rec_count *= pos.records_read;
    cost += pos.read_time;

    if (!rs->emb_sj_nest) *outer_rec_count *= pos.records_read;
  }
...
static COND *optimize_cond(JOIN *join, COND *conds, List<TABLE_LIST> *join_list,
                           bool ignore_on_conds, Item::cond_result *cond_value,
                           COND_EQUAL **cond_equal, int flags) {
  THD *thd = join->thd;
  ...
  if (!conds) {
    ...
    */
    ...
    conds =
        build_equal_items(join, conds, NULL, join_list, ignore_on_conds,
                          cond_equal, MY_TEST(flags & OPT_LINK_EQUAL_FIELDS));
    DBUG_EXECUTE("where",
                 print_where(conds, "after equal_items", QT_ORDINARY););
    ...
    */
    DBUG_EXECUTE("where",
                 print_where(conds, "after const change", QT_ORDINARY););
...
void propagate_new_equalities(THD *thd, Item *cond,
  ...
  if (cond->type() == Item::COND_ITEM) {
    ...
    cond =
        cond->propagate_equal_fields(thd, Item::Context_boolean(), inherited);
    cond->update_used_tables();
  }
}
...
bool cond_has_datetime_is_null(Item *cond) {
  if (cond_is_datetime_is_null(cond)) return true;
  ...
  if (cond->type() == Item::COND_ITEM) {
    ...
    Item *item;
    while ((item = li++)) {
      if (cond_has_datetime_is_null(item)) return true;
    }
  }
  return false;
}
...
COND *Item_cond::remove_eq_conds(THD *thd, Item::cond_result *cond_value,
                                 bool top_level_arg) {
  ...
  if (and_level) {
    ...
    */
    ...
    Item_equal *eq_item;
    while ((eq_item = it++)) {
      if (eq_item->const_item() && eq_item->val_int()) it.remove();
    }
  ...
  if (!new_equalities.is_empty()) {
    ...
    */
    if (is_simplifiable_cond) {
      if (!(cond = cond->remove_eq_conds(thd, cond_value, false))) return cond;
    }
    should_fix_fields = 1;
  }
...
COND *Item::remove_eq_conds(THD *thd, Item::cond_result *cond_value,
                            bool top_level_arg) {
...
COND *Item_bool_func2::remove_eq_conds(THD *thd, Item::cond_result *cond_value,
                                       bool top_level_arg) {
  if (const_item() && !is_expensive()) {
    *cond_value = eval_const_cond(this) ? Item::COND_TRUE : Item::COND_FALSE;
    return (COND *)0;
...
COND *Item_func_isnull::remove_eq_conds(THD *thd, Item::cond_result *cond_value,
                                        bool top_level_arg) {
    ...
    */
    ...
    if (top_level_arg)  // "auto_increment_column IS NULL" is the only condition
    {
      if (field->flags & AUTO_INCREMENT_FLAG && !field->table->maybe_null &&
        ...
        if ((new_cond = new (thd->mem_root) Item_func_eq(
                 thd, args[0],
...
*/
bool const_expression_in_where(COND *cond, Item *comp_item, Field *comp_field,
                               Item **const_item) {
...
Field *Item::create_tmp_field_int(TABLE *table, uint convert_int_length) {
  ...
  if (max_char_length() > convert_int_length) h = &type_handler_longlong;
  return h->make_and_init_table_field(&name, Record_addr(maybe_null), *this,
                                      table);
}
...
static void create_tmp_field_from_item_finalize(THD *thd, Field *new_field,
                                                Item *item, Item ***copy_func,
                                                bool modify_item) {
  if (copy_func &&
      (item->is_result_field() || (item->real_item()->is_result_field())))
...
static Field *create_tmp_field_from_item(THD *thd, Item *item, TABLE *table,
  ...
  if ((new_field = item->create_tmp_field(false, table)))
    create_tmp_field_from_item_finalize(thd, new_field, item, copy_func,
                                        modify_item);
  return new_field;
}
  ...
  if (field_type() == MYSQL_TYPE_VARCHAR) {
    ...
    if (max_length > MAX_FIELD_VARCHARLENGTH)
      ...
      field = new (thd->mem_root) Field_varstring(
          max_length, maybe_null, &name, table->s, collation.collation);
    else
  ...
  switch (type) {
      ...
      */
      if (((field->maybe_null && field->in_rollup) ||
        ...
        */
        ...
        result =
            create_tmp_field_from_item(thd, item, table, NULL, modify_item);
        *from_field = field->field;
        if (result && modify_item) field->result_field = result;
        if (orig_item) item->maybe_null = save_maybe_null;
        ...
        *from_field = field->field;
        if ((result = handler->make_and_init_table_field(
                 &item->name, Record_addr(item->maybe_null), *item, table)))
          create_tmp_field_from_item_finalize(thd, result, item, copy_func,
                                              modify_item);
        if (result && modify_item) field->result_field = result;
      } else {
        LEX_CSTRING *tmp = orig_item ? &orig_item->name : &item->name;
        result =
            create_tmp_field_from_field(thd, (*from_field = field->field), tmp,
    ...
    case Item::FUNC_ITEM:
      if (((Item_func *)item)->functype() == Item_func::FUNC_SP) {
        ...
        Field *result_field = create_tmp_field_from_field(
            thd, sp_result_field, &item_func_sp->name, table, NULL);

        if (modify_item) item->set_result_field(result_field);
...
void setup_tmp_table_column_bitmaps(TABLE *table, uchar *bitmaps,
  ...
  my_bitmap_init(&table->def_read_set, (my_bitmap_map *)bitmaps, field_count,
                 FALSE);
  bitmaps += bitmap_size;
  my_bitmap_init(&table->tmp_set, (my_bitmap_map *)bitmaps, field_count, FALSE);
  bitmaps += bitmap_size;
...
TABLE *create_tmp_table(THD *thd, TMP_TABLE_PARAM *param, List<Item> &fields,
  ...
  */
  ...
  DBUG_PRINT("enter", ("table_alias: '%s'  distinct: %d  save_sum_fields: %d  "
                       "rows_limit: %lu  group: %d",
                       table_alias->str, (int)distinct, (int)save_sum_fields,
    ...
    if (!param->quick_group)
      ...
      for (ORDER *tmp = group; tmp; tmp = tmp->next) {
        ...
        */
        ...
        if ((*tmp->item)->too_big_for_varchar()) using_unique_constraint = true;
      }
  ...
  */
  ...
  if (!multi_alloc_root(
          ...
          sizeof(Field *) * (field_count + 1), &default_field,
          sizeof(Field *) * (field_count), &blob_field,
          sizeof(uint) * (field_count + 1), &from_field,
          sizeof(Field *) * field_count, &copy_func,
          sizeof(*copy_func) * (copy_func_count + 1), &param->keyinfo,
    ...
    if (type == Item::SUM_FUNC_ITEM && !group &&
      ...
      for (i = 0; i < sum_item->get_arg_count(); i++) {
        ...
        if (!arg->const_item()) {
          ...
          DBUG_ASSERT(!new_field->field_name.str ||
                      strlen(new_field->field_name.str) ==
          ...
          *(reg_field++) = new_field;
          ...
          if (!(tmp_item =
                    new (thd->mem_root) Item_temptable_field(thd, new_field)))
            goto err;
      ...
      */
      ...
      if (unlikely(!new_field)) {
        if (unlikely(thd->is_fatal_error)) goto err;  // Got OOM
  ...
  *blob_field = 0;  // End marker
  ...
  share->reclength = reclength;
  {
    ...
    if (!(table->record[0] =
              (uchar *)alloc_root(&table->mem_root, alloc_length * 3)))
      goto err;
  ...
  if (thd->variables.tmp_memory_table_size == ~(ulonglong)0)  // No limit
    ...
    share->max_rows =
        (ha_rows)(((share->db_type() == heap_hton)
  ...
  */
  ...
  if (group) {
    ...
    for (; cur_group; cur_group = cur_group->next, key_part_info++) {
      ...
      if (cur_group == group) field->key_start.set_bit(0);
      ...
      if (!using_unique_constraint) {
        ...
        if (!(cur_group->field = field->new_key_field(
                  thd->mem_root, table, group_buff + MY_TEST(maybe_null),
                  key_part_info->length, field->null_ptr, field->null_bit)))
          goto err; /* purecov: inspected */
  ...
  if (distinct && field_count != param->hidden_field_count) {
    ...
    */
    ...
    keyinfo->key_part = key_part_info;
    keyinfo->flags =
        HA_NOSAME | HA_NULL_ARE_EQUAL | HA_BINARY_PACK_KEY | HA_PACK_KEY;
    keyinfo->ext_key_flags = keyinfo->flags;
    ...
    */
    size_t rpk_size =
        keyinfo->user_defined_key_parts * sizeof(keyinfo->rec_per_key[0]);
    if (!(keyinfo->rec_per_key =
              (ulong *)alloc_root(&table->mem_root, rpk_size)))
      goto err;
    ...
    */
    if (null_pack_length && share->uniques) {
      ...
      key_part_info->field = new Field_string(
          table->record[0], (uint32)key_part_info->length, (uchar *)0, (uint)0,
          Field::NONE, &null_clex_str, &my_charset_bin);
      if (!key_part_info->field) goto err;
    ...
    for (i = param->hidden_field_count, reg_field = table->field + i;
      ...
      key_part_info->null_offset =
          (uint)((*reg_field)->null_ptr - (uchar *)table->record[0]);
  ...
  table->used_for_duplicate_elimination =
      (param->sum_func_count == 0 && (table->group || table->distinct));
...
bool Virtual_tmp_table::add(List<Spvar_definition> &field_list) {
  ...
  while ((cdef = it++)) {
    Field *tmp;
    if (!(tmp = cdef->make_field(
              s, in_use->mem_root, 0,
              (uchar *)(f_maybe_null(cdef->pack_flag) ? "" : 0),
              f_maybe_null(cdef->pack_flag) ? 1 : 0, &cdef->field_name)))
      DBUG_RETURN(true);
    add(tmp);
  }
  DBUG_RETURN(false);
...
bool Virtual_tmp_table::open() {
  ...
  if (!(record[0] = (uchar *)in_use->alloc(s->rec_buff_length))) return true;
  if (null_pack_length) {
...
bool Virtual_tmp_table::sp_find_field_by_name(uint *idx,
  ...
  for (uint i = 0; (f = field[i]); i++) {
    ...
    if (!my_strnncoll(system_charset_info, (const uchar *)f->field_name.str,
                      f->field_name.length, (const uchar *)name.str,
                      name.length)) {
      *idx = i;
      return false;
    }
...
bool Virtual_tmp_table::sp_find_field_by_name_or_error(
...
bool Virtual_tmp_table::sp_set_all_fields_from_item_list(THD *thd,
  ...
  for (uint i = 0; (item = it++); i++) {
    if (field[i]->sp_prepare_and_store_item(thd, &item)) return true;
  }
  return false;
}
...
bool open_tmp_table(TABLE *table) {
  int error;
  if (unlikely((error = table->file->ha_open(
                    table, table->s->table_name.str, O_RDWR,
...
bool create_internal_tmp_table(TABLE *table, KEY *keyinfo,
  ...
  if (share->keys) {  // Get keys for ni_create
    ...
    HA_KEYSEG *seg = (HA_KEYSEG *)alloc_root(
        &table->mem_root, sizeof(*seg) * keyinfo->user_defined_key_parts);
    if (!seg) goto err;
  ...
  */
  {
    enum data_file_type file_type =
        table->no_rows
            ? NO_RECORD
            : (share->reclength < 64 && !share->blob_fields
                   ? STATIC_RECORD
    ...
    if (unlikely(
            (error = maria_create(share->table_name.str, file_type, share->keys,
...
bool create_internal_tmp_table(TABLE *table, KEY *keyinfo,
  ...
  if (unlikely((error = mi_create(
                    share->table_name.str, share->keys, &keydef,
...
bool create_internal_tmp_table_from_heap(THD *thd, TABLE *table,
  ...
  if (create_internal_tmp_table(
          &new_table, table->key_info, start_recinfo, recinfo,
  ...
  table->file->ha_index_or_rnd_end();
  if (table->file->ha_rnd_init_with_error(1)) DBUG_RETURN(1);
  ...
  */
  while (!table->file->ha_rnd_next(new_table.record[1])) {
    ...
    if (write_err) goto err;
    if (unlikely(thd->check_killed())) {
      thd->send_kill_message();
      goto err_killed;
    }
  }
  if (!new_table.no_rows && new_table.file->ha_end_bulk_insert()) goto err;
  /* copy row that filled HEAP table */
  if (unlikely(
          (write_err = new_table.file->ha_write_tmp_row(table->record[0])))) {
  ...
  if (table->group && tmp_tbl->sum_func_count &&
    ...
    */
    ...
    if (tmp_tbl->precomputed_group_by) {
      ...
      */
      memcpy(tmp_tbl->items_to_copy + tmp_tbl->func_count, join->sum_funcs,
...
static int do_select(JOIN *join, Procedure *procedure) {
  ...
  if (join->pushdown_query) {
    ...
    if (join->pushdown_query->store_data_in_temp_table) {
      JOIN_TAB *last_tab =
          join->join_tab + join->table_count - join->exec_join_tab_cnt();
      last_tab->next_select = end_send;

      enum_nested_loop_state state = last_tab->aggr->end_send();
      if (state >= NESTED_LOOP_OK) state = sub_select(join, last_tab, true);

      if (state < NESTED_LOOP_OK) res = 1;
  ...
  if (join->only_const_tables() && !join->need_tmp) {
    ...
    if (!join->pseudo_bits_cond || join->pseudo_bits_cond->val_int()) {
      ...
      */
      ...
      if (!join->having || join->having->val_int()) {
        List<Item> *columns_list =
            (procedure ? &join->procedure_fields_list : join->fields);
    ...
    */
    if (unlikely(join->thd->is_error())) error = NESTED_LOOP_ERROR;
  } else {
    DBUG_EXECUTE_IF(
    ...
    JOIN_TAB *join_tab =
        join->join_tab + (join->tables_list ? join->const_tables : 0);
  ...
  */
  if (join->order) {
    ...
    if (sort_tab->filesort && join->select_options & OPTION_FOUND_ROWS &&
  ...
  if (error == NESTED_LOOP_OK) {
    ...
    */
    if (unlikely(join->result->send_eof())) rc = 1;  // Don't send error
...
int rr_sequential_and_unpack(READ_RECORD *info) {
  int error;
  if (unlikely((error = rr_sequential(info)))) return error;
...
enum_nested_loop_state sub_select_postjoin_aggr(JOIN *join, JOIN_TAB *join_tab,
                                                bool end_of_records) {
  ...
  if (end_of_records) {
    rc = aggr->end_send();
    if (rc >= NESTED_LOOP_OK) rc = sub_select(join, join_tab, end_of_records);
    DBUG_RETURN(rc);
  }
...
enum_nested_loop_state sub_select_cache(JOIN *join, JOIN_TAB *join_tab,
  ...
  */
  ...
  if (!test_if_use_dynamic_range_scan(join_tab)) {
    if (!cache->put_record()) DBUG_RETURN(NESTED_LOOP_OK);
    /*
...
static enum_nested_loop_state evaluate_join_record(JOIN *join,
  ...
  if (error > 0 || unlikely(join->thd->is_error()))  // Fatal error
    DBUG_RETURN(NESTED_LOOP_ERROR);
  if (error < 0) DBUG_RETURN(NESTED_LOOP_NO_MORE_ROWS);
  ...
  if (select_cond) {
    ...
    if (unlikely(join->thd->is_error())) DBUG_RETURN(NESTED_LOOP_ERROR);
  }
  ...
  if (!select_cond || select_cond_result) {
      ...
      for (JOIN_TAB *tab = first_unmatched; tab <= join_tab; tab++) {
        ...
        */
        ...
        for (JOIN_TAB *first_upper = first_unmatched->first_upper;
          ...
          if (!first_upper->found) not_exists_opt_is_applicable = false;
        }
    ...
    */
    ...
    if (found) {
      ...
      if (rc != NESTED_LOOP_OK && rc != NESTED_LOOP_NO_MORE_ROWS)
        DBUG_RETURN(rc);
      if (return_tab < join->return_tab) join->return_tab = return_tab;

      /* check for errors evaluating the condition */
      if (unlikely(join->thd->is_error())) DBUG_RETURN(NESTED_LOOP_ERROR);

      if (join->return_tab < join_tab) DBUG_RETURN(NESTED_LOOP_OK);
      /*
...
static int join_read_const_table(THD *thd, JOIN_TAB *tab, POSITION *pos) {
  ...
  if (tab->table->pos_in_table_list->is_materialized_derived() &&
    ...
    if (unlikely((error = join_read_system(tab)))) {  // Info for DESCRIBE
  ...
  */
...

  {
    ...
    if (join->conds) update_const_equal_items(thd, join->conds, tab, TRUE);
    while ((tbl = ti++)) {
      TABLE_LIST *embedded;
...
static int join_read_system(JOIN_TAB *tab) {
  ...
  {
    if (unlikely((error = table->file->ha_read_first_row(
                      table->record[0], table->s->primary_key)))) {
...
*/
int join_read_key2(THD *thd, JOIN_TAB *tab, TABLE *table,
                   TABLE_REF *table_ref) {
  int error;
...
static int join_read_always_key(JOIN_TAB *tab) {
  ...
  if (unlikely(cp_buffer_from_ref(tab->join->thd, table, &tab->ref))) return -1;
  if (unlikely((
          error = table->file->prepare_index_key_scan_map(
...
static int join_read_last_key(JOIN_TAB *tab) {
  ...
  if (!table->file->inited && unlikely((error = table->file->ha_index_init(
                                            tab->ref.key, tab->sorted)))) {
    (void)report_error(table, error);
    return 1;
  }
  ...
  if (unlikely((error = table->file->ha_index_read_map(
                    table->record[0], tab->ref.key_buff,
...
static int test_if_quick_select(JOIN_TAB *tab) {
  DBUG_EXECUTE_IF("show_explain_probe_test_if_quick_select",
                  if (dbug_user_var_equals_int(
                          tab->join->thd, "show_explain_probe_select_id",
...
int join_init_read_record(JOIN_TAB *tab) {
  ...
  */
  ...
  if (tab->select && tab->select->quick && tab->select->quick->reset()) {
    ...
    report_error(tab->table, tab->join->thd->killed ? HA_ERR_QUERY_INTERRUPTED
                                                    : HA_ERR_OUT_OF_MEM);
    return 1;
  }
  ...
  if (!tab->preread_init_done && tab->preread_init()) return 1;
...
bool JOIN_TAB::sort_table() {
  ...
  DBUG_ASSERT(join->ordered_index_usage !=
              (filesort->order == join->order ? JOIN::ordered_index_order_by
...
static int join_read_first(JOIN_TAB *tab) {
  ...
  DBUG_ASSERT(table->no_keyread || !table->covering_keys.is_set(tab->index) ||
  ...
  if (likely(!error)) error = table->file->prepare_index_scan();
  if (unlikely(error) || unlikely(error = tab->table->file->ha_index_first(
                                      tab->table->record[0]))) {
    if (error != HA_ERR_KEY_NOT_FOUND && error != HA_ERR_END_OF_FILE)
      report_error(table, error);
    DBUG_RETURN(-1);
...
static int join_read_last(JOIN_TAB *tab) {
  ...
  tab->read_record.index = tab->index;
  tab->read_record.record = table->record[0];
  if (!table->file->inited) error = table->file->ha_index_init(tab->index, 1);
  if (likely(!error)) error = table->file->prepare_index_scan();
  if (unlikely(error) ||
...
int join_read_next_same_or_null(READ_RECORD *info) {
  int error;
  if (unlikely((error = join_read_next_same(info)) >= 0)) return error;
  ...
  if (!end_of_records) {
    ...
    if (join->do_send_rows) {
      ...
      if (unlikely((error = join->result->send_data(*fields)))) {
        if (error > 0) DBUG_RETURN(NESTED_LOOP_ERROR);
    ...
    if (join->send_records >= join->unit->select_limit_cnt &&
      ...
      /*
        ...
        join_tab.
      */
      if (join->order && join->select_options & OPTION_FOUND_ROWS &&
          join_tab > join->join_tab && (join_tab - 1)->filesort &&
          (join_tab - 1)->filesort->using_pq) {
    ...
    if (likely(!join_tab->having || join_tab->having->val_int())) {
      ...
      if ((error = table->file->ha_write_tmp_row(table->record[0]))) {
        ...
        if (create_internal_tmp_table_from_heap(
                join->thd, table, join_tab->tmp_table_param->start_recinfo,
                &join_tab->tmp_table_param->recinfo, error, 1, &is_duplicate))
          DBUG_RETURN(NESTED_LOOP_ERROR);  // Not a table_is_full error
        if (is_duplicate) goto end;
...
static enum_nested_loop_state end_update(JOIN *join,
  ...
  for (group = table->group; group; group = group->next) {
    ...
    if (group->fast_field_copier_setup != group->field) {
      DBUG_PRINT("info", ("new setup %p -> %p", group->fast_field_copier_setup,
                          group->field));
    ...
    if (item->maybe_null) group->buff[-1] = (char)group->field->is_null();
  }
  if (!table->file->ha_index_read_map(
          table->record[1], join_tab->tmp_table_param->group_buff, HA_WHOLE_KEY,
  ...
  if (!join->first_record || end_of_records ||
    ...
    if (join->first_record || (end_of_records && !join->group)) {
      ...
      if (idx < send_group_parts) {
        ...
        if (unlikely(join->rollup.state != ROLLUP::STATE_NONE)) {
          if (unlikely(join->rollup_write_data(
                  (uint)(idx + 1), join_tab->tmp_table_param, table))) {
            DBUG_RETURN(NESTED_LOOP_ERROR);
          }
        }
        if (end_of_records) goto end;
      }
    } else {
      if (end_of_records) goto end;
      join->first_record = 1;
    ...
    if (idx < (int)join->send_group_parts) {
      ...
      if (unlikely(init_sum_functions(join->sum_funcs,
        ...
        DBUG_RETURN(NESTED_LOOP_ERROR);
      if (unlikely(join->procedure)) join->procedure->add();
      goto end;
    }
  }
...
bool test_if_ref(Item *root_cond, Item_field *left_item, Item *right_item) {
  ...
  if (!field->table->const_table && join_tab &&
    ...
    */
    if (join_tab->ref.is_access_triggered()) return FALSE;
...
static Item *make_cond_for_table(THD *thd, Item *cond, table_map tables,
  ...
  return make_cond_for_table_from_pred(thd, cond, cond, tables, used_table,
                                       join_tab_idx_arg, exclude_expensive_cond,
                                       retain_ref_cond);
}

static Item *make_cond_for_table_from_pred(
    THD *thd, Item *root_cond, Item *cond, table_map tables,
    table_map used_table, int join_tab_idx_arg,
    ...
    if (((Item_cond *)cond)->functype() == Item_func::COND_AND_FUNC) {
      ...
      while ((item = li++)) {
        Item *fix = make_cond_for_table_from_pred(
            thd, root_cond, item, tables, used_table, join_tab_idx_arg,
            exclude_expensive_cond, retain_ref_cond);
        if (fix) new_cond->argument_list()->push_back(fix, thd->mem_root);
      }
      ...
      while ((item = li++)) {
        Item *fix = make_cond_for_table_from_pred(
            thd, root_cond, item, tables, 0L, join_tab_idx_arg,
            exclude_expensive_cond, retain_ref_cond);
...
*/
static COND *make_cond_after_sjm(THD *thd, Item *root_cond, Item *cond,
                                 table_map tables, table_map sjm_tables,
                                 bool inside_or_clause) {
  /*
  ...
  */
  ...
  if (cond->type() == Item::COND_ITEM) {
    if (((Item_cond *)cond)->functype() == Item_func::COND_AND_FUNC) {
      ...
      Item *item;
      while ((item = li++)) {
        Item *fix = make_cond_after_sjm(thd, root_cond, item, tables,
                                        sjm_tables, inside_or_clause);
        if (fix) new_cond->argument_list()->push_back(fix, thd->mem_root);
      }
      switch (new_cond->argument_list()->elements) {
        case 0:
      ...
      while ((item = li++)) {
        Item *fix =
            make_cond_after_sjm(thd, root_cond, item, tables, sjm_tables,
...
static Item *part_of_refkey(TABLE *table, Field *field) {
  ...
  if (!join_tab) return (Item *)0;  // field from outer non-select (UPDATE,...)
  ...
  for (; order; order = order->next, const_key_parts >>= 1) {
    ...
    */
    ...
    if (have_pk_suffix && reverse == 0 &&  // all were =const so far
        ...
        table->const_key_parts[pk] ==
            PREV_BITS(uint, table->key_info[pk].user_defined_key_parts)) {
      key_parts = 0;
...
void compute_part_of_sort_key_for_equals(JOIN *join, TABLE *table,
  ...
  if (!optimizer_flag(join->thd, OPTIMIZER_SWITCH_ORDERBY_EQ_PROP)) return;
    ...
    while ((cur_item_eq = li++)) {
      if ((cur_item_eq->used_tables() & needed_tbl_map) &&
        ...
        item_field->item_equal =
            item_eq;  // Save the pointer to our Item_equal.
        break;
      }
    }
  }
  ...
  */
  ...
  for (ORDER *tmp_order = order; tmp_order; tmp_order = tmp_order->next) {
    ...
    */
    ...
    if (usable_keys.is_clear_all()) goto use_filesort;  // No usable keys
  }
  ...
  if (tab->ref.key >= 0 && tab->ref.key_parts) {
    ...
    */
    if (tab->type == JT_REF_OR_NULL || tab->type == JT_FT) goto use_filesort;
  ...
  if (ref_key >= 0 && ref_key != MAX_KEY) {
        ...
        if ((table->quick_keys.is_set(new_ref_key) &&
          ...
          */
          ...
          while (keyuse->key != new_ref_key && keyuse->table == tab->table)
            keyuse++;
          if (create_ref_for_key(
                  tab->join, tab, keyuse, FALSE,
                  (tab->join->const_table_map | OUTER_REF_TABLE_BIT)))
            goto use_filesort;
    ...
    if (usable_keys.is_set(ref_key) &&
        (order_direction = test_if_order_by_key(tab->join, order, table,
                                                ref_key, &used_key_parts)))
  ...
  }
  {
    ...
    test_if_cheaper_ordering(tab, order, table, usable_keys, ref_key,
    ...
    */
    ...
    /*
      ...
      test_if_order_by_key function. It could differ from
      keyinfo->user_defined_key_parts, thus we have to restore it in case of
      desc order as it affects QUICK_SELECT_DESC behaviour.
    */
    used_key_parts =
...
check_reverse_order:
  ...
  if (order_direction == -1)  // If ORDER BY ... DESC
  {
    ...
    if (select && select->quick) {
      ...
      */
      if (select->quick->reverse_sorted()) goto skipped_filesort;
    ...
    if (best_key >= 0) {
      ...
      */
      ...
      if (!quick_created) {
        ...
        tab->index = best_key;
        tab->read_first_record =
            order_direction > 0 ? join_read_first : join_read_last;
        ...
        if (tab->join->select_options & SELECT_DESCRIBE) {
          tab->ref.key = -1;
          tab->ref.key_parts = 0;
          if (select_limit < table->stat_records()) tab->limit = select_limit;
          table->file->ha_end_keyread();
        }
...
skipped_filesort:
  ...
  if (orig_cond_saved && !changed_key) tab->set_cond(orig_cond);
...
use_filesort:
  ...
  if (orig_cond_saved) tab->set_cond(orig_cond);
...
int create_sort_index(THD *thd, JOIN *join, JOIN_TAB *tab, Filesort *fsort) {
  ...
  if (fsort == NULL) fsort = tab->filesort;
  ...
  if (!tab->preread_init_done && tab->preread_init()) goto err;
...

{
  ...
  */
  thd->lex->limit_rows_examined_cnt = ULONGLONG_MAX;
  if (thd->killed == ABORT_QUERY) thd->reset_killed();
...
static int remove_dup_with_compare(THD *thd, TABLE *table, Field **first_field,
  ...
  if (unlikely(file->ha_rnd_init_with_error(1))) DBUG_RETURN(1);
  ...
  for (;;) {
    ...
    if (having && !having->val_int()) {
      if (unlikely((error = file->ha_delete_row(record)))) goto err;
      error = file->ha_rnd_next(record);
      continue;
    }
    ...
    for (;;) {
      ...
      if (compare_record(table, first_field) == 0) {
        ...
        found = 1;
        if (unlikely((error = file->remember_rnd_pos()))) goto err;
      }
    }
    if (!found) break;  // End of file
    /* Restart search on saved row */
    if (unlikely((error = file->restart_rnd_next(record)))) goto err;
  }
...
static int remove_dup_with_hash_index(THD *thd, TABLE *table, uint field_count,
  ...
  if (unlikely(!my_multi_malloc(
          MYF(MY_WME), &key_buffer,
          (uint)((key_length + extra_length) * (long)file->stats.records),
  ...
  if (unlikely(my_hash_init(&hash, &my_charset_bin, (uint)file->stats.records,
                            0, key_length, (my_hash_get_key)0, 0, 0))) {
    my_free(key_buffer);
    DBUG_RETURN(1);
  }

  if (unlikely((error = file->ha_rnd_init(1)))) goto err;
  ...
  for (;;) {
    ...
    if (my_hash_search(&hash, org_key_pos, key_length)) {
      ...
      if (my_hash_insert(&hash, org_key_pos)) goto err;
    }
...
err:
  ...
  file->extra(HA_EXTRA_NO_CACHE);
  (void)file->ha_rnd_end();
  if (unlikely(error)) file->print_error(error, MYF(0));
  DBUG_RETURN(1);
}
...
static bool find_order_in_list(THD *thd, Ref_ptr_array ref_pointer_array,
  ...
  */
  if (order_item->type() == Item::INT_ITEM && order_item->basic_const_item() &&
      !from_window_spec) { /* Order by position */
    uint count;
    ...
    thd->change_item_tree((Item **)&order->item,
                          (Item *)&ref_pointer_array[count - 1]);
  ...
  if (select_item != not_found_item) {
    ...
    */
    ...
    if (from_field == not_found_field ||
      ...
      */
      push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN,
                          ER_NON_UNIQ_ERROR, ER_THD(thd, ER_NON_UNIQ_ERROR),
                          ((Item_ident *)order_item)->field_name.str,
                          thd->where);
    }
  } else if (from_window_spec) {
    Item **found_item = find_item_in_list(
        order_item, all_fields, &counter, REPORT_EXCEPT_NOT_FOUND, &resolution,
  ...
  */
  if (!order_item->fixed &&
      (order_item->fix_fields(thd, order->item) ||
       (order_item = *order->item)->check_cols(1) || thd->is_error()))
    return TRUE; /* Wrong field. */

  if (!add_to_all_fields) return FALSE;
  ...
  *hidden_group_fields = 0;
  ...
  if (thd->variables.sql_mode & MODE_ONLY_FULL_GROUP_BY) {
    ...
    */
    ...
    List_iterator<Item> li(fields);
    List_iterator<Item_field> naf_it(
        thd->lex->current_select->join->non_agg_fields);
    ...
    while (field && (item = li++)) {
      if (item->type() != Item::SUM_FUNC_ITEM && item->marker >= 0 &&
        ...
        while (field) {
          ...
          if (field->marker < cur_pos_in_select_list) goto next_field;
          /* Found a field from the next expression. */
          if (field->marker > cur_pos_in_select_list) break;
          /*
            Check whether the field occur in the GROUP BY list.
            Throw the error later if the field isn't found.
          */
          for (ord = order; ord; ord = ord->next)
            if ((*ord->item)->eq((Item *)field, 0)) goto next_field;
          /*
    ...
    if (!item->const_item() && !item->with_sum_func && !item->marker) {
      ...
      */
      ...
      if (item->type() == Item::FIELD_ITEM &&
        ...
        */
        Item_field *new_item =
            new (thd->mem_root) Item_field(thd, (Item_field *)item);
...
static TABLE *get_sort_by_table(ORDER *a, ORDER *b, List<TABLE_LIST> &tables,
  ...
  while ((table = ti++) && !(map & table->table->map))
    ;
  if (map != table->table->map) DBUG_RETURN(0);  // More than one table
...
bool setup_copy_fields(THD *thd, TMP_TABLE_PARAM *param,
  ...
  if (param->field_count && !(copy = param->copy_field = new (thd->mem_root)
    ...
    */
    if (real_pos->type() == Item::FIELD_ITEM &&
      ...
      if (item->field->flags & BLOB_FLAG) {
        if (!(pos = new (thd->mem_root) Item_copy_string(thd, pos))) goto err;
        /*
        ...
        */
        if (param->copy_funcs.push_front(pos, thd->mem_root)) goto err;
      } else {
        /*
        ...
        */
        field = item->field;
        item->result_field =
            field->make_new_field(thd->mem_root, field->table, 1);
        /*
          We need to allocate one extra byte for null handling and
          another extra byte to not get warnings from purify in
          Field_string::val_int
        */
        if (!(tmp = (uchar *)thd->alloc(field->pack_length() + 2))) goto err;
        if (copy) {
          ...
          copy++;
               ...
               !real_pos->with_sum_func) {  // Save for send fields
      pos = real_pos;
      /* TODO:
         In most cases this result will be sent to the user.
         This should be changed to use copy_int or copy_real depending
      ...
      */
      ...
      {
        if (extra_funcs.push_back(pos, thd->mem_root)) goto err;
      } else if (param->copy_funcs.push_back(pos, thd->mem_root))
        goto err;
    }
    res_all_fields.push_back(pos, thd->mem_root);
    ref_pointer_array[((i < border) ? all_fields.elements - i - 1
                                    : i - border)] = pos;
  }
  param->copy_field_end = copy;
...
static bool change_to_use_tmp_fields(THD *thd, Ref_ptr_array ref_pointer_array,
  ...
  for (uint i = 0; (item = it++); i++) {
    ...
    if (item->with_sum_func && item->type() != Item::SUM_FUNC_ITEM)
      ...
      if (!(item_field = item->get_tmp_table_item(thd))) DBUG_RETURN(true);
      ...
      if (field != NULL) {
        ...
        Item_func_set_user_var *suv = new (thd->mem_root)
            Item_func_set_user_var(thd, (Item_func_set_user_var *)item);
        Item_field *new_field =
            new (thd->mem_root) Item_temptable_field(thd, field);
      ...
      if (item->type() == Item::SUM_FUNC_ITEM && field->table->group)
        ...
        item_field =
            (Item *)new (thd->mem_root) Item_temptable_field(thd, field);
...
static bool change_refs_to_tmp_fields(THD *thd, Ref_ptr_array ref_pointer_array,
  ...
  for (i = 0; (item = it++); i++) {
    if (item->type() == Item::SUM_FUNC_ITEM && item->const_item())
      ...
      if (!(new_item = item->get_tmp_table_item(thd))) return 1;
    }

    if (res_all_fields.push_back(new_item, thd->mem_root)) return 1;
    ref_pointer_array[((i < border) ? all_fields.elements - i - 1
                                    : i - border)] = new_item;
  }
...
bool copy_funcs(Item **func_ptr, const THD *thd) {
  ...
  for (; (func = *func_ptr); func_ptr++) {
    ...
    */
    if (unlikely(thd->is_error())) return TRUE;
  }
  return FALSE;
}
...
static bool add_ref_to_table_cond(THD *thd, JOIN_TAB *join_tab) {
  ...
  for (uint i = 0; i < join_tab->ref.key_parts; i++) {
    Field *field =
        table
            ->field[table->key_info[join_tab->ref.key].key_part[i].fieldnr - 1];
    Item *value = join_tab->ref.items[i];
    cond->add(new (thd->mem_root) Item_func_equal(
                  thd, new (thd->mem_root) Item_field(thd, field), value),
              thd->mem_root);
  }
  if (unlikely(thd->is_fatal_error)) DBUG_RETURN(TRUE);
  if (!cond->fixed) {
  ...
  if (join_tab->select) {
    ...
    if (join_tab->select->pre_idx_push_select_cond) {
      Item *new_cond =
          and_conds(thd, cond_copy, join_tab->select->pre_idx_push_select_cond);
      if (!new_cond->fixed && new_cond->fix_fields(thd, &new_cond)) error = 1;
      ...
      if (item->type() == Item::FIELD_ITEM || item->type() == Item::REF_ITEM) {
        ...
        for (group_tmp = group_list; group_tmp; group_tmp = group_tmp->next) {
          if (item->eq(*group_tmp->item, 0)) {
            Item *new_item;
            if (!(new_item = new (thd->mem_root)
                      Item_ref(thd, context, group_tmp->item, 0, &item->name)))
...
bool JOIN::rollup_init() {
  ...
  */
  ...
  rollup.ref_pointer_arrays = static_cast<Ref_ptr_array *>(thd->alloc(
      (sizeof(Ref_ptr_array) + all_fields.elements * sizeof(Item *)) *
      send_group_parts));
  rollup.fields = static_cast<List<Item> *>(
      thd->alloc(sizeof(List<Item>) * send_group_parts));

  if (!null_items || !rollup.ref_pointer_arrays || !rollup.fields) return true;
  ...
  */
  for (i = 0; i < send_group_parts; i++) {
    ...
    rollup_fields->empty();
    rollup.ref_pointer_arrays[i] =
        Ref_ptr_array(ref_array, all_fields.elements);
          ...
          if (*group_tmp->item == item) {
            ...
            */
            Item_null_result *null_item =
                new (thd->mem_root) Item_null_result(thd);
...
int JOIN::rollup_write_data(uint idx, TMP_TABLE_PARAM *tmp_table_param_arg,
  ...
  for (i = send_group_parts; i-- > idx;) {
    ...
    if ((!having || having->val_int())) {
      ...
      if (unlikely((write_error = table_arg->file->ha_write_tmp_row(
                        table_arg->record[0])))) {
        if (create_internal_tmp_table_from_heap(
                thd, table_arg, tmp_table_param_arg->start_recinfo,
...
int print_explain_message_line(select_result_sink *result, uint8 options,
                               bool is_analyze, uint select_number,
                               const char *select_type, ha_rows *rows,
                               const char *message) {
  THD *thd = result->thd;
  ...
  /* `partitions` */
  if (options & DESCRIBE_PARTITIONS) item_list.push_back(item_null, mem_root);
  ...
  /* `r_rows` */
  if (is_analyze) item_list.push_back(item_null, mem_root);
...
int append_possible_keys(MEM_ROOT *alloc, String_list &list, TABLE *table,
  ...
  for (j = 0; j < table->s->keys; j++) {
    if (possible_keys.is_set(j))
      if (!(list.append_str(alloc, table->key_info[j].name.str))) return 1;
  }
  return 0;
}

bool JOIN_TAB::save_explain_data(Explain_table_access *eta,
                                 table_map prefix_tables, bool distinct_arg,
  ...
  */
  ...
  if (filesort) {
    if (!(eta->pre_join_sort = new (thd->mem_root) Explain_aggr_filesort(
              thd->mem_root, thd->lex->analyze_stmt, filesort)))
      return 1;
  }
  ...
  ****/
  ...
  /* "partitions" column */
  {
    ...
    if (!table->derived_select_number && (part_info = table->part_info) &&
        (join && join->thd->lex->describe)) {  // TODO: all thd->mem_root here
                                               // should be fixed
  ...
  if ((type == JT_ALL || type == JT_HASH) && tab_select && tab_select->quick &&
      use_quick != 2) {
  ...
  */
    ...
    if (ref.key_parts && tab_type != JT_FT) {
      ...
      for (uint kp = 0; kp < ref.key_parts; kp++) {
        if ((key_part_map(1) << kp) & ref.const_ref_part_map) {
          if (!(eta->ref_list.append_str(thd->mem_root, "const"))) return 1;
        } else {
  ...
  {
    eta->hash_next_key.set(thd->mem_root, &table->key_info[index],
  ...
  if (info) {
    ...
    if (packed_info & TAB_INFO_USING_INDEX) eta->push_extra(ET_USING_INDEX);
    if (packed_info & TAB_INFO_USING_WHERE) eta->push_extra(ET_USING_WHERE);
    ...
    if (tab_select) {
      if (use_quick == 2) {
        ...
        if (eta->range_checked_fer)
          eta->range_checked_fer->append_possible_keys_stat(thd->mem_root,
                                                            table, keys);
    ...
    if (table->reginfo.not_exists_optimize) eta->push_extra(ET_NOT_EXISTS);

    if (quick_type == QUICK_SELECT_I::QS_TYPE_RANGE) {
      explain_append_mrr_info((QUICK_RANGE_SELECT *)(tab_select->quick),
                              &eta->mrr_type);
      if (eta->mrr_type.length() > 0) eta->push_extra(ET_USING_MRR);
    }

    if (shortcut_for_distinct) eta->push_extra(ET_DISTINCT);
    ...
    if (check_weed_out_table) {
      ...
      if (do_firstmatch == /*join->join_tab*/ first_top_tab - 1)
        ...
        if (prev_table->derived_select_number) {
          ...
          /* Derived table name generation */
          size_t len = my_snprintf(namebuf, sizeof(namebuf) - 1, "<derived%u>",
                                   prev_table->derived_select_number);
          eta->firstmatch_table_name.append(namebuf, len);
        } else
          eta->firstmatch_table_name.append(
              &prev_table->pos_in_table_list->alias);
      }
    }
    ...
    if (cache) {
      ...
      if (cache->save_explain_data(&eta->bka_type)) return 1;
    }
  }
...
bool save_agg_explain_data(JOIN *join, Explain_select *xpl_sel) {
  ...
  for (uint i = 0; i < join->aggr_tables; i++, join_tab++) {
    ...
    prev_node = node;
    if (!(node = new (thd->mem_root) Explain_aggr_tmp_table)) return 1;
    ...
    if (join_tab->distinct) {
      prev_node = node;
      if (!(node = new (thd->mem_root) Explain_aggr_remove_dups)) return 1;
      node->child = prev_node;
    }

    if (join_tab->filesort) {
      Explain_aggr_filesort *eaf = new (thd->mem_root)
          Explain_aggr_filesort(thd->mem_root, is_analyze, join_tab->filesort);
      if (!eaf) return 1;
      prev_node = node;
      node = eaf;
      node->child = prev_node;
...
int JOIN::save_explain_data_intern(Explain_query *output,
  ...
  DBUG_PRINT("info", ("Select %p, type %s, message %s", join->select_lex,
  ...
  if (message) {
    if (!(explain = new (output->mem_root)
              Explain_select(output->mem_root, thd->lex->analyze_stmt)))
      DBUG_RETURN(1);
#ifndef DBUG_OFF
    ...
    if (save_agg_explain_data(this, explain)) DBUG_RETURN(1);
    ...
    explain = xpl_sel = new (output->mem_root)
        Explain_select(output->mem_root, thd->lex->analyze_stmt);
    if (!explain) DBUG_RETURN(1);
    ...
    if (save_agg_explain_data(this, xpl_sel)) DBUG_RETURN(1);
    ...
    for (JOIN_TAB *tab = first_explain_order_tab(join); tab;
      ...
      Explain_table_access *eta =
          (new (output->mem_root) Explain_table_access(output->mem_root));

      if (!eta) DBUG_RETURN(1);
      if (tab->bush_root_tab != prev_bush_root_tab) {
        if (tab->bush_root_tab) {
          /*
            We've entered an SJ-Materialization nest. Create an object for it.
          */
          if (!(cur_parent = new (output->mem_root)
                    Explain_basic_join(output->mem_root)))
            DBUG_RETURN(1);
  ...
  for (SELECT_LEX_UNIT *tmp_unit = join->select_lex->first_inner_unit();
       tmp_unit; tmp_unit = tmp_unit->next_unit()) {
    /*
  ...
  if (select_lex->is_top_level_node()) output->query_plan_ready();
  ...
  for (SELECT_LEX_UNIT *unit = join->select_lex->first_inner_unit(); unit;
    ...
    */
    if (unit->item && !unit->item->fixed) {
      Item *ref = unit->item;
      if (unit->item->fix_fields(thd, &ref)) DBUG_VOID_RETURN;
...
bool mysql_explain_union(THD *thd, SELECT_LEX_UNIT *unit,
  ...
  if (unit->is_unit_op()) {
    if (unit->union_needs_tmp_table() && unit->fake_select_lex) {
      unit->fake_select_lex->select_number =
          FAKE_SELECT_LEX_ID;  // just for initialization
    ...
    */
    ...
    unit->set_limit(unit->global_parameters());
    res = mysql_select(
        thd, first->table_list.first, first->with_wild, first->item_list,
        first->where, first->order_list.elements + first->group_list.elements,
        first->order_list.first, first->group_list.first, first->having,
        thd->lex->proc_list.first,
        first->options | thd->variables.option_bits | SELECT_DESCRIBE, result,
        unit, first);
  }
  DBUG_RETURN(res || thd->is_error());
}

static void print_table_array(THD *thd, table_map eliminated_tables,
                              String *str, TABLE_LIST **table, TABLE_LIST **end,
    ...
    */
    if (eliminated_tables &&
        ((curr->table && (curr->table->map & eliminated_tables)) ||
         (curr->nested_join &&
          !(curr->nested_join->used_tables & ~eliminated_tables)))) {
...
static bool is_eliminated_table(table_map eliminated_tables, TABLE_LIST *tbl) {
  return eliminated_tables &&
         ((tbl->table && (tbl->table->map & eliminated_tables)) ||
          (tbl->nested_join &&
           !(tbl->nested_join->used_tables & ~eliminated_tables)));
}
...
static void print_join(THD *thd, table_map eliminated_tables, String *str,
                       List<TABLE_LIST> *tables, enum_query_type query_type) {
  ...
  */
  ...
  for (TABLE_LIST *t = ti++; t; t = ti++) {
    ...
    if (print_const_tables || !t->optimized_away)
      if (!is_eliminated_table(eliminated_tables, t)) tables_to_print++;
  }
  ...
  if (!(table = static_cast<TABLE_LIST **>(
            thd->alloc(sizeof(TABLE_LIST *) * tables_to_print))))
    DBUG_VOID_RETURN;  // out of memory

  TABLE_LIST *tmp, **t = table + (tables_to_print - 1);
  while ((tmp = ti++)) {
    if (tmp->optimized_away && !print_const_tables) continue;
    if (is_eliminated_table(eliminated_tables, tmp)) continue;
    *t-- = tmp;
  }
  ...
  */
  DBUG_ASSERT(
      ...
      !(((*table)->table && ((*table)->table->map & eliminated_tables)) ||
        ((*table)->nested_join &&
         !((*table)->nested_join->used_tables & ~eliminated_tables))));
  /*
  ...
  */
  ...
  print_table_array(thd, eliminated_tables, str, table, table + tables_to_print,
                    query_type);
  DBUG_VOID_RETURN;
}
...
void TABLE_LIST::print(THD *thd, table_map eliminated_tables, String *str,
  ...
  if (nested_join) {
    str->append('(');
    print_join(thd, eliminated_tables, str, &nested_join->join_list,
               query_type);
    str->append(')');
    ...
    if (table && table->versioned()) vers_conditions.print(str, query_type);
...
void st_select_lex::print(THD *thd, String *str, enum_query_type query_type) {
  ...
  if ((query_type & QT_SHOW_SELECT_NUMBER) && thd->lex->all_selects_list &&
      thd->lex->all_selects_list->link_next && select_number != UINT_MAX &&
  ...
  if (options & SELECT_DISTINCT) str->append(STRING_WITH_LEN("distinct "));
...
JOIN::enum_reopt_result JOIN::reoptimize(Item *added_where,
  ...
  if (!keyuse.buffer && my_init_dynamic_array(&keyuse, sizeof(KEYUSE), 20, 64,
                                              MYF(MY_THREAD_SPECIFIC))) {
    delete_dynamic(&added_keyuse);
    return REOPT_ERROR;
  }
  ...
  if (save_to) {
    DBUG_ASSERT(!keyuse.elements);
    memcpy(keyuse.buffer, save_to->keyuse.buffer,
  ...
  if (sort_and_filter_keyuse(thd, &keyuse, true)) return REOPT_ERROR;
  optimize_keyuse(this, &keyuse);

  if (optimize_semijoin_nests(this, join_tables)) return REOPT_ERROR;

  /* Re-run the join optimizer to compute a new query plan. */
  if (choose_plan(this, join_tables)) return REOPT_ERROR;
...
void JOIN::cache_const_exprs() {
  ...
  if (const_tables == table_count) return;

  if (conds)
    conds->compile(thd, &Item::cache_const_expr_analyzer,
                   (uchar **)&analyzer_arg, &Item::cache_const_expr_transformer,
                   (uchar *)&cache_flag);
  cache_flag = FALSE;
  if (having)
  ...
  for (JOIN_TAB *tab = first_depth_first_tab(this); tab;
    ...
    if (*tab->on_expr_ref) {
      cache_flag = FALSE;
      (*tab->on_expr_ref)
          ->compile(thd, &Item::cache_const_expr_analyzer,
                    (uchar **)&analyzer_arg,
                    &Item::cache_const_expr_transformer, (uchar *)&cache_flag);
    }
  }
...
static bool get_range_limit_read_cost(const JOIN_TAB *tab, const TABLE *table,
                                      uint keynr, ha_rows rows_limit,
                                      double *read_time) {
  bool res = false;
  /*
    ...
    */
    if (tab) {
      ...
      for (kp = 0; kp < MAX_REF_PARTS; map = map << 1, kp++) {
        if (!(table->const_key_parts[keynr] & map)) break;
        const_parts |= map;
      }
      ...
      if (kp > 0) {
        ...
        */
        ...
        if (ref_rows > 0) {
          ...
          set_if_smaller(tmp,
                         (double)tab->join->thd->variables.max_seeks_for_key);
          if (table->covering_keys.is_set(keynr))
            tmp = table->file->keyread_time(keynr, 1, (ha_rows)tmp);
          else
            tmp = table->file->read_time(
                keynr, 1, (ha_rows)MY_MIN(tmp, tab->worst_seeks));
          if (tmp < best_cost) {
            best_cost = tmp;
...
static bool test_if_cheaper_ordering(const JOIN_TAB *tab, ORDER *order,
                                     TABLE *table, key_map usable_keys,
                                     int ref_key, ha_rows select_limit_arg,
  ...
  */
  ...
  for (nr = 0; nr < table->s->keys; nr++) {
    ...
    if (keys.is_set(nr) && (direction = test_if_order_by_key(
                                join, order, table, nr, &used_key_parts))) {
      /*
      ...
      */
      ...
      bool is_covering =
          (table->covering_keys.is_set(nr) ||
           (table->file->index_flags(nr, 0, 1) & HA_CLUSTERED_INDEX));
      /*
      ...
      */
      if (is_covering || select_limit != HA_POS_ERROR ||
        ...
        if (group) {
          ...
          */
          ...
          rec_per_key = used_key_parts
                            ? keyinfo->actual_rec_per_key(used_key_parts - 1)
                            : 1;
        ...
        */
        ...
        rec_per_key =
            keyinfo->actual_rec_per_key(keyinfo->user_defined_key_parts - 1);
        set_if_bigger(rec_per_key, 1);
        /*
        ...
        */
        ...
        if ((ref_key < 0 && (group || table->force_index || is_covering)) ||
          ...
          ha_rows refkey_select_limit =
              (ref_key >= 0 && !is_hash_join_key_no(ref_key) &&
...
uint get_index_for_order(ORDER *order, TABLE *table, SQL_SELECT *select,
                         ha_rows limit, ha_rows *scanned_limit, bool *need_sort,
                         bool *reverse) {
  if (!order) {
    *need_sort = FALSE;
    if (select && select->quick)
...
ulong check_selectivity(THD *thd, ulong rows_to_read, TABLE *table,
  ...
  if (unlikely(file->ha_rnd_init_with_error(1))) DBUG_RETURN(0);
  do {
    error = file->ha_rnd_next(record);
    ...
    count++;
    while ((cond = it++)) {
      if (cond->cond->val_bool()) cond->positive++;
    }
    it.rewind();
...
enum_nested_loop_state AGGR_OP::put_record(bool end_of_records) {
  ...
  if (!join_tab->table->file->inited)
    if (prepare_tmp_table()) return NESTED_LOOP_ERROR;
  enum_nested_loop_state rc =
      (*write_func)(join_tab->join, join_tab, end_of_records);
  return rc;
}
...
enum_nested_loop_state AGGR_OP::end_send() {
  ...
  if (join_tab->window_funcs_step) {
    if (join_tab->window_funcs_step->exec(join)) return NESTED_LOOP_ERROR;
  }
    ...
    else if (unlikely(join->thd->killed))  // Aborted by user
    {
      ...
      */
      if (join_tab->window_funcs_step) {
        ...
        Item *func;
        for (; (func = *func_ptr); func_ptr++) {
          if (func->with_window_func) func->save_in_result_field(true);
        }
      }
  ...
  if (join_tab->table->file->inited) join_tab->table->file->ha_rnd_end();
