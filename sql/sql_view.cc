...
static bool fill_defined_view_parts(THD *thd, TABLE_LIST *view) {
  ...
  if (tdc_open_view(thd, &decoy, OPEN_VIEW_NO_PARSE)) return TRUE;
...
bool create_view_precheck(THD *thd, TABLE_LIST *tables, TABLE_LIST *view,
  ...
  */
  if ((check_access(thd, CREATE_VIEW_ACL, view->db.str, &view->grant.privilege,
                    &view->grant.m_internal, 0, 0) ||
       check_grant(thd, CREATE_VIEW_ACL, view, FALSE, 1, FALSE)) ||
      (mode != VIEW_CREATE_NEW &&
       (check_access(thd, DROP_ACL, view->db.str, &view->grant.privilege,
                     &view->grant.m_internal, 0, 0) ||
    ...
    for (tbl = sl->get_table_list(); tbl; tbl = tbl->next_local) {
      ...
      */
      ...
      /*
        ...
        not opened yet.
      */
      fill_effective_table_privileges(thd, &tbl->grant, tbl->db.str,
                                      tbl->table_name.str);
    }
  }
  ...
  if (&lex->select_lex != lex->all_selects_list) {
    ...
    for (tbl = tables; tbl; tbl = tbl->next_global) {
      if (!tbl->table_in_first_from_clause) {
        if (check_access(thd, SELECT_ACL, tbl->db.str, &tbl->grant.privilege,
                         &tbl->grant.m_internal, 0, 0) ||
...
bool mysql_create_view(THD *thd, TABLE_LIST *views,
  ...
  */
  ...
  sp_cache_invalidate();
  if (sp_process_definer(thd)) goto err;
  ...
  */
  for (tbl = lex->query_tables; tbl; tbl = tbl->next_global) {
    ...
    if (tbl->view && cmp(&tbl->view_db, &view->db) == 0 &&
   ...
   */
  {
    ...
    if (!final_priv && report_item) {
      my_error(ER_COLUMNACCESS_DENIED_ERROR, MYF(0), "create view",
               thd->security_ctx->priv_user, thd->security_ctx->priv_host,
               report_item->name.str, view->table_name.str);
      res = TRUE;
      goto err;
    }
  }
  ...
  */
  ...
  if (!res && mysql_bin_log.is_open()) {
    ...
    */
    ...
    if (thd->binlog_query(THD::STMT_QUERY_TYPE, buff.ptr(), buff.length(),
                          FALSE, FALSE, FALSE, errcode))
      res = TRUE;
  }

  if (mode != VIEW_CREATE_NEW) query_cache_invalidate3(thd, view, 0);
  if (res) goto err;
...
static void make_view_filename(LEX_CSTRING *dir, char *dir_buff,
                               size_t dir_buff_len, LEX_CSTRING *path,
                               char *path_buff, size_t path_buff_len,
                               LEX_CSTRING *file, TABLE_LIST *view) {
  /* print file name */
  dir->length =
      build_table_filename(dir_buff, dir_buff_len - 1, view->db.str, "", "", 0);
  dir->str = dir_buff;

  path->length =
      build_table_filename(path_buff, path_buff_len - 1, view->db.str,
...
int mariadb_fix_view(THD *thd, TABLE_LIST *view, bool wrong_checksum,
  ...
  if (!wrong_checksum && view->mariadb_version) DBUG_RETURN(HA_ADMIN_OK);

  make_view_filename(&dir, dir_buff, sizeof(dir_buff), &path, path_buff,
                     sizeof(path_buff), &file, view);
  /* init timestamp */
  ...
  if (sql_create_definition_file(&dir, &file, view_file_type, (uchar *)view,
                                 view_parameters)) {
  ...
  sql_print_information(
      "View %`s.%`s: the version is set to %llu%s%s", view->db.str,
      view->table_name.str, view->mariadb_version,
      (wrong_checksum ? ", checksum corrected" : ""),
      (swap_alg ? ((view->algorithm == VIEW_ALGORITHM_MERGE)
...
loop_out:
  ...
  /* check old .frm */
  {
    ...
    if (ha_table_exists(thd, &view->db, &view->table_name)) {
      if (lex->create_info.if_not_exists()) {
        push_warning_printf(
            thd, Sql_condition::WARN_LEVEL_NOTE, ER_TABLE_EXISTS_ERROR,
  ...
  */
  ...
  /*
    ...
    UNION
  */
  if (view->updatable_view && !lex->select_lex.master_unit()->is_unit_op() &&
...
bool mysql_make_view(THD *thd, TABLE_SHARE *share, TABLE_LIST *table,
  ...
  */
  ...
  if ((result = share->view_def->parse(
           (uchar *)table, thd->mem_root, view_parameters,
    ...
    */
    thd->variables.sql_mode &=
        ~(MODE_PIPES_AS_CONCAT | MODE_ANSI_QUOTES | MODE_IGNORE_SPACE |
          MODE_NO_BACKSLASH_ESCAPES | MODE_ORACLE);
    ...
    if (dbchanged && mysql_change_db(thd, &old_db, TRUE)) goto err;
  }
  if (!parse_status) {
    ...
    */
    if (!table->prelocking_placeholder &&
        (old_lex->describe || old_lex->analyze_stmt)) {
      /*
    ...
    */
    ...
    view_is_mergeable =
        (table->algorithm != VIEW_ALGORITHM_TMPTABLE && lex->can_be_merged());
    ...
    if (view_is_mergeable) {
      ...
      */
      for (tbl = view_main_select_tables; tbl; tbl = tbl->next_local) {
        ...
        if (!tbl->sequence) tbl->lock_type = table->lock_type;
    ...
    */
    ...
    if (table->view_suid) {
      /*
        ...
        objects of the view.
      */
      if (!(table->view_sctx = (Security_context *)thd->stmt_arena->calloc(
...
bool mysql_drop_view(THD *thd, TABLE_LIST *views, enum_drop_mode drop_mode) {
  ...
  */
  ...
  if (unlikely(
          lock_table_names(thd, views, 0, thd->variables.lock_wait_timeout, 0)))
    DBUG_RETURN(TRUE);

  for (view = views; view; view = view->next_local) {
    bool not_exist;
    build_table_filename(path, sizeof(path) - 1, view->db.str,
                         view->table_name.str, reg_ext, 0);
    ...
    if ((not_exist = my_access(path, F_OK)) || !dd_frm_is_view(thd, path)) {
      ...
      if (thd->lex->if_exists()) {
        push_warning_printf(thd, Sql_condition::WARN_LEVEL_NOTE,
                            ER_UNKNOWN_VIEW, ER_THD(thd, ER_UNKNOWN_VIEW),
                            name);
        continue;
      }
      if (not_exist) {
...
int view_check(THD *thd, TABLE_LIST *view, HA_CHECK_OPT *check_opt) {
  ...
  if (res != HA_ADMIN_OK) DBUG_RETURN(res);
...
bool mysql_rename_view(THD *thd, const LEX_CSTRING *new_db,
                       const LEX_CSTRING *new_name, TABLE_LIST *view) {
  LEX_CSTRING pathstr;
  File_parser *parser;
  ...
  pathstr.length =
      build_table_filename(path_buff, sizeof(path_buff) - 1, view->db.str,
                           view->table_name.str, reg_ext, 0);
  ...
  if ((parser = sql_parse_prepare(&pathstr, thd->mem_root, 1)) &&
    ...
    */
    ...
    pathstr.length =
        build_table_filename(path_buff, sizeof(path_buff) - 1, new_db->str,
                             new_name->str, reg_ext, 0);
