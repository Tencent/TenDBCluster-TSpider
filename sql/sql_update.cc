...
bool compare_record(const TABLE *table) {
  ...
  */
  if (table->s->can_cmp_whole_record) return cmp_record(table, record[1]);
  /* Compare null bits */
  ...
  if (thd->variables.sql_mode & MODE_SIMULTANEOUS_ASSIGNMENT) {
    ...
    while ((item = it++)) {
      ...
      if (f->has_explicit_value()) {
        my_error(ER_UPDATED_COLUMN_ONLY_ONCE, MYF(0), *(f->table_name),
                 f->field_name.str);
        return TRUE;
      }
...
static bool check_has_vers_fields(TABLE *table, List<Item> &items) {
  List_iterator<Item> it(items);
  if (!table->versioned()) return false;
...
static void prepare_record_for_error_message(int error, TABLE *table) {
  ...
  */
  if (bitmap_is_clear_all(&unique_map)) DBUG_VOID_RETURN;
...
int mysql_update(THD *thd, TABLE_LIST *table_list, List<Item> &fields,
  ...
  if (mysql_handle_derived(thd->lex, DT_INIT)) DBUG_RETURN(1);
  ...
  if (table_list->handle_derived(thd->lex, DT_MERGE_FOR_INSERT)) DBUG_RETURN(1);
  if (table_list->handle_derived(thd->lex, DT_PREPARE)) DBUG_RETURN(1);
  ...
  if (check_unique_table(thd, table_list)) DBUG_RETURN(TRUE);
...
#endif
  if (lock_tables(thd, table_list, table_count, 0)) DBUG_RETURN(1);

  /* Apply the IN=>EXISTS transformation to all subqueries and optimize them */
  if (select_lex->optimize_unflattened_subqueries(false)) DBUG_RETURN(TRUE);

  if (select_lex->inner_refs_list.elements &&
      fix_inner_refs(thd, all_fields, select_lex,
  ...
  if (!(thd->security_ctx->master_access & SUPER_ACL)) {
    if (opt_spider_transaction_one_shard &&
      ...
      if (table->sql_use_partition_count > 1 && !is_config_table(table)) {
        my_error(ER_ACCESS_DENIED_MULPARTITION_IN_QUERY, MYF(0), "Update",
                 thd->security_ctx->priv_user, thd->security_ctx->host_or_ip,
  ...
  */
  ...
  if (unlikely(init_ftfuncs(thd, select_lex, 1))) goto err;

  table->mark_columns_needed_for_update();

  table->update_const_key_parts(conds);
  order = simple_remove_const(order, conds);
  query_plan.scanned_rows =
      select ? select->records : table->file->stats.records;
  ...
  if (select && select->quick && select->quick->unique_key_range()) {
    ...
    query_plan.index = get_index_for_order(
        order, table, select, limit, &scanned_limit, &need_sort, &reverse);
    if (!need_sort) query_plan.scanned_rows = scanned_limit;
    ...
    if (select && select->quick) {
      ...
      if (query_plan.index != MAX_KEY) {
        ...
        used_key_is_modified =
            is_key_used(table, query_plan.index, table->write_set);
      }
    }
  }
  ...
  */
  if (thd->lex->describe) goto produce_explain_and_leave;
  if (!(explain =
  ...
  */
  if ((table->file->ha_table_flags() & HA_CAN_DIRECT_UPDATE_AND_DELETE) &&
      !has_triggers && !binlog_is_row && !query_plan.using_io_buffer &&
      !ignore && !table->check_virtual_columns_marked_for_read() &&
    ...
    if (!use_direct_update &&
      ...
      /*
        Stored function items add RAND_TABLE_BIT to used_tables_cache, we
        should ignore this bit since we want to push stored function
    ...
    */
    if (query_plan.using_filesort) {
      ...
      */
      ...
      if (!(file_sort = filesort(thd, table, &fsort, fs_tracker))) goto err;
      ...
      */
      ...
      if (query_plan.index < MAX_KEY &&
          old_covering_keys.is_set(query_plan.index))
      ...
      */
      ...
      if (unlikely(thd->killed) && !error) error = 1;  // Aborted
      limit = tmp_limit;
      ...
      if (select) {
        ...
        if (!(select = new SQL_SELECT)) goto err;
        select->head = table;
      }
...
update_begin:
  ...
  */
  ...
  if (do_direct_update) {
    ...
    updated = update_rows;
    found = found_rows;
    if (found < updated) found = updated;
    goto update_end;
  }
      ...
      if (!can_compare_record || compare_record(table)) {
        ...
        if (unlikely(error == HA_ERR_RECORD_IS_THE_SAME)) {
          ...
          if (has_vers_fields && table->versioned()) {
            ...
            if (likely(!error)) updated_sys_ver++;
          }
          if (likely(!error)) updated++;
        }
...
update_end:
  ...
  *updated_return = updated;
  ...
  if (unlikely(thd->lex->analyze_stmt)) goto emit_explain_and_leave;
...
err:
  ...
  free_underlaid_joins(thd, select_lex);
  table->file->ha_end_keyread();
  if (table->file->pushed_cond) table->file->cond_pop();
  thd->abort_on_warning = 0;
  DBUG_RETURN(1);
...
bool mysql_prepare_update(THD *thd, TABLE_LIST *table_list, Item **conds,
  ...
  /*
    ...
    (not multi-) update
  */
  if (mysql_handle_derived(thd->lex, DT_PREPARE)) DBUG_RETURN(TRUE);

  if (setup_tables_and_check_access(
          thd, &select_lex->context, &select_lex->top_join_list, table_list,
...
/**
  ...
  @return
    ...
    false  otherwise.
*/
static bool unsafe_key_update(List<TABLE_LIST> leaves,
  ...
  while ((tl = it++)) {
    if (!tl->is_jtbm() && (tl->table->map & tables_for_update)) {
      ...
      it2.rewind();
      while ((tl2 = it2++)) {
        if (tl2->is_jtbm()) continue;
        /*
          Look at "next" tables only since all previous tables have
          already been checked
        */
        TABLE *table2 = tl2->table;
        if (tl2 != tl && table2->map & tables_for_update &&
            table1->s == table2->s) {
          ...
          if (table_partitioned &&
            ...
            my_error(ER_MULTI_UPDATE_KEY_CONFLICT, MYF(0),
                     tl->top_table()->alias.str, tl2->top_table()->alias.str);
            return true;
          }
  ...
  if (table->view) {
    ...
    */
    DBUG_ASSERT(
        ...
        (!table->updatable && !(table->table->map & tables_for_update)));
    ...
    if (check_table_access(thd, updated ? UPDATE_ACL : SELECT_ACL, table, FALSE,
                           1, FALSE))
      return true;
    *updated_arg |= updated;
    ...
    *updated_arg |= updated;
    ...
    if (!table->derived) {
      table->grant.want_privilege = SELECT_ACL & ~table->grant.privilege;
      table->table->grant.want_privilege =
          (SELECT_ACL & ~table->table->grant.privilege);
    }
  }
  return false;
}
...
int mysql_multi_update_prepare(THD *thd) {
  ...
  */
  ...
  if (mysql_handle_derived(lex, DT_MERGE_FOR_INSERT)) DBUG_RETURN(TRUE);

  if (mysql_handle_derived(lex, DT_PREPARE)) DBUG_RETURN(TRUE);
  ...
  if (lex->select_lex.handle_derived(thd->lex, DT_MERGE)) DBUG_RETURN(TRUE);
  ...
  */
  ...
  while ((tl = ti++)) {
    ...
    if (tl->is_jtbm()) continue;

    /* if table will be updated then check that it is unique */
    if (table->map & tables_for_update) {
      if (!tl->single_table_updatable() || check_key_in_view(thd, tl)) {
        my_error(ER_NON_UPDATABLE_TABLE, MYF(0), tl->top_table()->alias.str,
                 "UPDATE");
        DBUG_RETURN(TRUE);
      }

      DBUG_PRINT("info",
                 ("setting table `%s` for update", tl->top_table()->alias.str));
      /*
  ...
  */
  ...
  if (lex->save_prep_leaf_tables()) DBUG_RETURN(TRUE);
...
bool mysql_multi_update(THD *thd, TABLE_LIST *table_list, List<Item> *fields,
  ...
  if (!(*result = new (thd->mem_root)
            multi_update(thd, table_list, &thd->lex->select_lex.leaf_tables,
  ...
  res = mysql_select(thd, table_list, select_lex->with_wild, total_list, conds,
                     select_lex->order_list.elements,
                     select_lex->order_list.first, (ORDER *)NULL, (Item *)NULL,
                     (ORDER *)NULL,
...

{
  ...
  */
  ...
  int error = setup_fields(thd, Ref_ptr_array(), *values, MARK_COLUMNS_READ, 0,
                           NULL, 0);
  ...
  */
  ...
  for (i = 0; i < table_count; i++) {
    ...
    if (fields_for_table[i]->elements) {
      TABLE *table =
          ((Item_field *)(fields_for_table[i]->head()))->field->table;
...
static bool safe_update_on_fly(THD *thd, JOIN_TAB *join_tab,
  ...
  if (unique_table(thd, table_ref, all_tables, 0)) return 0;
  ...
  */
  ...
  for (table_ref = update_tables; table_ref;
    ...
    */
    if (table_ref->check_option && !join->select_lex->uncacheable) {
      ...
      SELECT_LEX *sl;
      for (tmp_unit = join->select_lex->first_inner_unit(); tmp_unit;
  ...
  loop_end:
    ...
    if (table == first_table_for_update && table_ref->check_option) {
      table_map unupdated_tables =
          table_ref->check_option->used_tables() & ~first_table_for_update->map;
      List_iterator<TABLE_LIST> ti(*leaves);
      TABLE_LIST *tbl_ref;
    ...
    */
    ...
    do {
      ...
      */
      ...
      if (!item) DBUG_RETURN(1);
      item->fix_fields(thd, 0);
      if (temp_fields.push_back(item, thd->mem_root)) DBUG_RETURN(1);
    } while ((tbl = tbl_it++));
    ...
    tmp_tables[cnt] =
        create_tmp_table(thd, tmp_param, temp_fields, (ORDER *)&group, 0, 0,
...
static TABLE *item_rowid_table(Item *item) {
  if (item->type() != Item::FUNC_ITEM) return NULL;
  Item_func *func = (Item_func *)item;
  if (func->functype() != Item_func::TEMPTABLE_ROWID) return NULL;
...
int multi_update::prepare2(JOIN *join) {
  if (!join->need_tmp || !join->tmp_table_keep_current_rowid) return 0;
  ...
  for (Item **it = tmptab->tmp_table_param->items_to_copy; *it; it++) {
    ...
    for (uint i = 0; i < table_count; i++) {
      for (Item **it2 = tmp_table_param[i].items_to_copy; *it2; it2++) {
        if (item_rowid_table(*it2) != tbl) continue;
        Item *fld =
            new (thd->mem_root) Item_field(thd, (*it)->get_tmp_table_field());
        if (!fld) return 1;
...
multi_update::~multi_update() {
  ...
  for (table = update_tables; table; table = table->next_local) {
    ...
    if (ignore) table->table->file->extra(HA_EXTRA_NO_IGNORE_DUP_KEY);
  }
    ...
    if (table == table_to_update) {
      ...
      */
      ...
      if (fill_record_n_invoke_before_triggers(
              thd, table, *fields_for_table[offset], *values_for_table[offset],
              0, TRG_EVENT_UPDATE))
        DBUG_RETURN(1);
      /*
...
void multi_update::abort_result_set() {
  ...
  if (updated) query_cache_invalidate3(thd, update_tables, 1);
  /*
...
int multi_update::do_updates() {
  ...
  */
  ...
  for (cur_table = update_tables; cur_table;
    ...
    */
    if (table->vfield) empty_record(table);
    ...
    */
    ...
    for (;;) {
      ...
      uint field_num = 0;
      do {
        if (unlikely((local_error = tbl->file->ha_rnd_pos(
                          tbl->record[0],
      ...
      if (table->vfield && unlikely(table->update_virtual_fields(
                               table->file, VCOL_UPDATE_INDEXED_FOR_UPDATE)))
        goto err2;
      ...
      if (!can_compare_record || compare_record(table)) {
        ...
        if (has_vers_fields && table->versioned()) table->vers_update_fields();

        if (unlikely((local_error = table->file->ha_update_row(
...
bool multi_update::send_eof() {
  ...
  */
  ...
  if (likely(!local_error)) local_error = (table_count) ? do_updates() : 0;
  /*
  ...
  */
  ...
  if (!thd->lex->analyze_stmt) {
    id = thd->arg_of_last_insert_id_function
             ...
             : 0;
    my_snprintf(buff, sizeof(buff), ER_THD(thd, ER_UPDATE_INFO), (ulong)found,
                (ulong)updated, (ulong)thd->cuted_fields);
    ::my_ok(thd,
