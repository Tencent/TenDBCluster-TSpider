...
Explain_delete *Delete_plan::save_explain_delete_data(MEM_ROOT *mem_root,
                                                      THD *thd) {
...
Explain_update *Update_plan::save_explain_update_data(MEM_ROOT *mem_root,
                                                      THD *thd) {
  Explain_query *query = thd->lex->explain;
...
bool Update_plan::save_explain_data_intern(MEM_ROOT *mem_root,
  ...
  *****/
  ...
  if (using_filesort)
    if (!(explain->filesort_tracker =
              new (mem_root) Filesort_tracker(is_analyze)))
      return 1;
...
inline int TABLE::delete_row() {
  ...
  vers_update_end();
  int res;
  if ((res = file->extra(HA_EXTRA_REMEMBER_POS))) return res;
  if ((res = file->ha_update_row(record[1], record[0]))) return res;
...
bool mysql_delete(THD *thd, TABLE_LIST *table_list, COND *conds,
  ...
  bool has_triggers;
  ORDER *order =
      (ORDER *)((order_list && order_list->elements) ? order_list->first
                                                     : NULL);
  ...
  if (with_select) (void)result->prepare(select_lex->item_list, NULL);
...
#endif
  if (lock_tables(thd, table_list, thd->lex->table_count, 0)) DBUG_RETURN(true);
  ...
  */
  ...
  if (!with_select && !using_limit && const_cond_result &&
    ...
    if (likely(!(error = table->file->ha_delete_all_rows()))) {
      ...
      */
      ...
      if (!query_plan.save_explain_delete_data(thd->mem_root, thd)) error = 1;
      goto cleanup;
    }
  ...
  if (!(thd->security_ctx->master_access & SUPER_ACL)) {
    if (opt_spider_transaction_one_shard &&
      ...
      if (table->sql_use_partition_count > 1 && !is_config_table(table)) {
        my_error(ER_ACCESS_DENIED_MULPARTITION_IN_QUERY, MYF(0), "Delete",
                 thd->security_ctx->priv_user, thd->security_ctx->host_or_ip,
  ...
  if (unlikely(error)) DBUG_RETURN(TRUE);
  if ((select && select->check_quick(thd, safe_update, limit)) || !limit) {
    ...
    */
    if (unlikely(thd->is_error())) DBUG_RETURN(TRUE);
    my_ok(thd, 0);
  ...
  if (order) {
    ...
    if (select && select->quick &&
        select->quick
            ->unique_key_range()) {  // Single row select (always "ordered")
      query_plan.using_filesort = FALSE;
      query_plan.index = MAX_KEY;
    } else {
      ha_rows scanned_limit = query_plan.scanned_rows;
      query_plan.index =
          get_index_for_order(order, table, select, limit, &scanned_limit,
                              &query_plan.using_filesort, &reverse);
      if (!query_plan.using_filesort) query_plan.scanned_rows = scanned_limit;
    }
  }
  ...
  */
  ...
  if ((table->file->ha_table_flags() & HA_CAN_DIRECT_UPDATE_AND_DELETE) &&
    ...
    if (!table->check_virtual_columns_marked_for_read()) {
      ...
      if (use_direct_delete && !table->file->direct_delete_rows_init()) {
        ...
        if (!(error = table->file->ha_direct_delete_rows(&deleted))) error = -1;
  ...
  if (query_plan.using_filesort) {
    {
      ...
      */
      ...
      /*
        ...
        a subquery in it)
      */
      if (!with_select) free_underlaid_joins(thd, select_lex);
      select = 0;
    }
  }

  /* If quick select is used, initialize it before retrieving rows. */
  if (select && select->quick && select->quick->reset()) goto got_error;
  ...
  if (unlikely(error)) goto got_error;

  if (unlikely(init_ftfuncs(thd, select_lex, 1))) goto got_error;
  ...
  if (!delete_while_scanning) {
    ...
    */
    ...
    deltempfile =
        new (thd->mem_root) Unique(refpos_order_cmp, table->file,
    ...
    while (!(error = info.read_record()) && !thd->killed && !thd->is_error()) {
      if (record_should_be_deleted(thd, table, select, explain,
        ...
        if (unlikely(
                (error = deltempfile->unique_add((char *)table->file->ref)))) {
          error = 1;
          goto terminate_delete;
        }
        if (!--tmplimit && using_limit) break;
      }
    }
...
terminate_delete:
  ...
  if (will_batch && unlikely((loc_error = table->file->end_bulk_delete()))) {
    if (error != 1) table->file->print_error(loc_error, MYF(0));
    error = 1;
  }
  ...
  */
  ...
  if (likely((error < 0) || thd->transaction.stmt.modified_non_trans_table)) {
    if (WSREP_EMULATE_BINLOG(thd) || mysql_bin_log.is_open()) {
      ...
      */
      int log_result =
          thd->binlog_query(query_type, thd->query(), thd->query_length(),
  ...
  if (likely(error < 0) ||
    ...
    if (thd->lex->analyze_stmt) goto send_nothing_and_leave;
...
/*
  ...
  RETURN VALUE
    ...
    TRUE  error
*/
int mysql_prepare_delete(THD *thd, TABLE_LIST *table_list, uint wild_num,
                         List<Item> &field_list, Item **conds,
  ...
  *delete_while_scanning = true;
  ...
  if ((wild_num && setup_wild(thd, table_list, field_list, NULL, wild_num,
      ...
      setup_fields(thd, Ref_ptr_array(), field_list, MARK_COLUMNS_READ, NULL,
                   NULL, 0) ||
...
int mysql_multi_delete_prepare(THD *thd) {
  ...
  */
  if (setup_tables_and_check_access(
          ...
          lex->select_lex.leaf_tables, FALSE, DELETE_ACL, SELECT_ACL, FALSE))
    DBUG_RETURN(TRUE);
  ...
  */
  ...
  for (target_tbl = (TABLE_LIST *)aux_tables; target_tbl;
    ...
    if (!target_tbl->correspondent_table->single_table_updatable() ||
      ...
      my_error(ER_NON_UPDATABLE_TABLE, MYF(0), target_tbl->table_name.str,
               "DELETE");
      DBUG_RETURN(TRUE);
    }
    /*
...
bool multi_delete::initialize_tables(JOIN *join) {
  ...
  for (JOIN_TAB *tab =
           first_linear_tab(join, WITHOUT_BUSH_ROOTS, WITH_CONST_TABLES);
...
void multi_delete::abort_result_set() {
  ...
  if (deleted) query_cache_invalidate3(thd, delete_tables, 1);
...
int multi_delete::do_deletes() {
  ...
  for (uint counter = 0; table_being_deleted;
    ...
    TABLE *table = table_being_deleted->table;
    int local_error;
    if (unlikely(tempfiles[counter]->get(table))) DBUG_RETURN(1);

    local_error =
        do_table_deletes(table, &tempfiles[counter]->sort, thd->lex->ignore);

    if (unlikely(thd->killed) && likely(!local_error)) DBUG_RETURN(1);

    if (unlikely(local_error == -1))  // End of file
      local_error = 0;

    if (unlikely(local_error)) DBUG_RETURN(local_error);
  }
  DBUG_RETURN(0);
}
...
int multi_delete::do_table_deletes(TABLE *table, SORT_INFO *sort_info,
  ...
  if (unlikely(
          init_read_record(&info, thd, table, NULL, sort_info, 0, 1, FALSE)))
    DBUG_RETURN(1);
  ...
  if (likely((local_error == 0) ||
    ...
    if (WSREP_EMULATE_BINLOG(thd) || mysql_bin_log.is_open()) {
      ...
      if (likely(local_error == 0))
        ...
        errcode = query_error_code(thd, killed_status == NOT_KILLED);
      if (unlikely(thd->binlog_query(THD::ROW_QUERY_TYPE, thd->query(),
                                     thd->query_length(), transactional_tables,
                                     FALSE, FALSE, errcode)) &&
          !normal_tables) {
