...
LEX_CSTRING *make_lex_string(LEX_CSTRING *lex_str, const char *str,
                             size_t length, MEM_ROOT *mem_root) {
...
class Trigger_creation_ctx : public Stored_program_creation_ctx,
                             public Sql_alloc {
 public:
  static Trigger_creation_ctx *create(THD *thd, const char *db_name,
                                      const char *table_name,
                                      const LEX_CSTRING *client_cs_name,
                                      const LEX_CSTRING *connection_cl_name,
                                      const LEX_CSTRING *db_cl_name);

  Trigger_creation_ctx(CHARSET_INFO *client_cs, CHARSET_INFO *connection_cl,
                       CHARSET_INFO *db_cl)
...
Trigger_creation_ctx *Trigger_creation_ctx::create(
  ...
  */
  ...
  if (!db_cl) db_cl = get_default_db_collation(thd, db_name);
...
Trigger *Table_triggers_list::for_all_triggers(Triggers_processor func,
  ...
  for (uint i = 0; i < (uint)TRG_EVENT_MAX; i++) {
    for (uint j = 0; j < (uint)TRG_ACTION_MAX; j++) {
      for (Trigger *trigger = get_trigger(i, j); trigger;
           trigger = trigger->next)
        if ((trigger->*func)(arg)) return trigger;
    }
  }
  return 0;
}
...
bool mysql_create_or_drop_trigger(THD *thd, TABLE_LIST *tables, bool create) {
  ...
  */
  ...
  if (thd->locked_tables_mode) {
    ...
    if (!(tables->table = find_table_for_mdl_upgrade(
              thd, tables->db.str, tables->table_name.str, FALSE)))
      goto end;
  } else {
    tables->table = open_n_lock_single_table(thd, tables, TL_READ_NO_INSERT, 0);
    if (!tables->table) goto end;
  ...
  */
  if (thd->locked_tables_list.reopen_tables(thd, false)) thd->clear_error();
  ...
  */
  ...
  if (!create) thd->lex->restore_backup_query_tables_list(&backup);
  ...
  */
  ...
  if (lex->trg_chistics.ordering_clause == TRG_ORDER_NONE) {
    ...
    */
    ...
    trigger_def->append(
        stmt_definition.str,
        (lex->trg_chistics.ordering_clause_begin - lex->stmt_definition_begin) -
            prefix_trimmed);
    ...
    trigger_def->append(
        stmt_definition.str +
            ...
            prefix_trimmed,
        (lex->stmt_definition_end - lex->trg_chistics.ordering_clause_end) -
            suffix_trimmed);
  }
}
...
bool Table_triggers_list::create_trigger(THD *thd, TABLE_LIST *tables,
  ...
  if (check_for_broken_triggers()) DBUG_RETURN(true);
  ...
  if (sp_process_definer(thd)) DBUG_RETURN(true);
  ...
  */
  ...
  for (trg_field = lex->trg_table_fields.first; trg_field;
  ...
  /*
    ...
    versions
  */
  file.length = build_table_filename(file_buff, FN_REFLEN - 1, tables->db.str,
                                     tables->table_name.str, TRG_EXT, 0);
  file.str = file_buff;
  trigname_file.length =
      build_table_filename(trigname_buff, FN_REFLEN - 1, tables->db.str,
                           lex->spname->m_name.str, TRN_EXT, 0);
  ...
  */
  ...
  trigger->create_time = ((ulonglong)thd->query_start()) * 100 +
                         thd->query_start_sec_part() / 10000;
  ...
  */
  ...
  add_trigger(lex->trg_chistics.event, lex->trg_chistics.action_time,
...
err_without_cleanup:
  ...
  if (trigger_dropped) {
    String drop_trg_query;
    drop_trg_query.append(STRING_WITH_LEN(
        "DROP TRIGGER /* generated by failed CREATE TRIGGER */ "));
...
static bool rm_trigname_file(char *path, const LEX_CSTRING *db,
  ...
  build_table_filename(path, FN_REFLEN - 1, db->str, trigger_name->str, TRN_EXT,
                       0);
...
bool Table_triggers_list::save_trigger_file(THD *thd, const LEX_CSTRING *db,
  ...
  if (create_lists_needed_for_files(thd->mem_root)) return true;

  file.length = build_table_filename(file_buff, FN_REFLEN - 1, db->str,
                                     table_name->str, TRG_EXT, 0);
  file.str = file_buff;
...
Trigger *Table_triggers_list::find_trigger(const LEX_CSTRING *name,
  ...
  for (uint i = 0; i < (uint)TRG_EVENT_MAX; i++) {
    for (uint j = 0; j < (uint)TRG_ACTION_MAX; j++) {
      ...
      for (parent = &triggers[i][j]; (trigger = *parent);
           parent = &trigger->next) {
...
bool Table_triggers_list::drop_trigger(THD *thd, TABLE_LIST *tables,
  ...
  if (!count)  // If no more triggers
  {
    ...
    */
    ...
    if (save_trigger_file(thd, &tables->db, &tables->table_name)) return 1;
  }

  if (rm_trigname_file(path, &tables->db, sp_name)) return 1;
...
bool Table_triggers_list::prepare_record_accessors(TABLE *table) {
...

  {
    ...
    if (!(extra_null_bitmap =
              (uchar *)alloc_root(&table->mem_root, null_bytes)))
      return 1;
    if (!(record0_field = (Field **)alloc_root(
...
bool Table_triggers_list::check_n_load(THD *thd, const LEX_CSTRING *db,
  ...
  path.length = build_table_filename(path_buff, FN_REFLEN - 1, db->str,
                                     table_name->str, TRG_EXT, 0);
  path.str = path_buff;
  ...
  if ((parser = sql_parse_prepare(&path, &table->mem_root, 1))) {
    if (is_equal(&triggers_file_type, parser->type())) {
      ...
      if (unlikely(!trigger_list)) goto error;
      ...
      List_iterator_fast<LEX_CSTRING> it_client_cs_name(
          trigger_list->client_cs_names);
      List_iterator_fast<LEX_CSTRING> it_connection_cl_name(
          trigger_list->connection_cl_names);
      ...
      while ((trg_create_str = it++)) {
        ...
        */
        ...
        if (!trigger_list->client_cs_names.is_empty())
          creation_ctx = Trigger_creation_ctx::create(
              thd, db->str, table_name->str, it_client_cs_name++,
              it_connection_cl_name++, it_db_cl_name++);
        else {
          /* Old file with not stored character sets. Use current */
          creation_ctx =
              new Trigger_creation_ctx(thd->variables.character_set_client,
        ...
        */
        ...
        if (lex.sphead) lex.sphead->m_sql_mode = sql_mode;
        ...
        trigger->on_table_name.length =
            (lex.raw_trg_on_table_name_end - lex.raw_trg_on_table_name_begin);
        ...
        lex_string_set(&trigger->db_cl_name, creation_ctx->get_db_cl()->name);

        /* event can only be TRG_EVENT_MAX in case of fatal parse errors */
        if (lex.trg_chistics.event != TRG_EVENT_MAX)
          trigger_list->add_trigger(
              lex.trg_chistics.event, lex.trg_chistics.action_time,
              TRG_ORDER_NONE, &lex.trg_chistics.anchor_trigger_name, trigger);
        ...
        if (unlikely(parse_error)) {
          ...
          */
          if (!trigger_list->m_has_unparseable_trigger)
            trigger_list->set_parse_error_message(
                error_handler.get_error_message());
          ...
          trigger->definer =
              ((!trg_definer || !trg_definer->length) ? empty_clex_str
                                                      : *trg_definer);
          continue;
        }
        ...
        */
        ...
        DBUG_ASSERT(
            (!my_strcasecmp(table_alias_charset, lex.query_tables->db.str,
                            db->str) ||
        ...
        DBUG_ASSERT((
            !my_strcasecmp(table_alias_charset,
                           lex.query_tables->table_name.str, table_name->str) ||
            (check_n_cut_mysql50_prefix(table_name->str, fname,
                                        sizeof(fname)) &&
             !my_strcasecmp(table_alias_charset,
                            lex.query_tables->table_name.str, fname))));
#endif
        if (names_only) {
          lex_end(&lex);
          continue;
...
void Trigger::get_trigger_info(LEX_CSTRING *trigger_stmt,
                               LEX_CSTRING *trigger_body, LEX_STRING *definer) {
...
bool add_table_for_trigger(THD *thd, const sp_name *trg_name, bool if_exists,
                           TABLE_LIST **table) {
  LEX *lex = thd->lex;
  ...
  *table = sp_add_to_query_tables(thd, lex, &trg_name->m_db, &tbl_name,
...
bool Table_triggers_list::drop_all_triggers(THD *thd, const LEX_CSTRING *db,
  ...
  if (table.triggers) {
    for (uint i = 0; i < (uint)TRG_EVENT_MAX; i++) {
      for (uint j = 0; j < (uint)TRG_ACTION_MAX; j++) {
        Trigger *trigger;
        for (trigger = table.triggers->get_trigger(i, j); trigger;
             trigger = trigger->next) {
          /*
    ...
    if (rm_trigger_file(path, db, name)) result = 1;
...
bool Table_triggers_list::change_table_name_in_triggers(
    THD *thd, const LEX_CSTRING *old_db_name, const LEX_CSTRING *new_db_name,
  ...
  if (unlikely(thd->is_fatal_error)) return TRUE; /* OOM */

  if (save_trigger_file(thd, new_db_name, new_table_name)) return TRUE;
...
Trigger *Table_triggers_list::change_table_name_in_trignames(
    const LEX_CSTRING *old_db_name, const LEX_CSTRING *new_db_name,
...
bool Trigger::change_on_table_name(void *param_arg) {
  ...
  if (param->stopper == this) return 0;  // Stop processing

  trigname_file.length =
      build_table_filename(trigname_buff, FN_REFLEN - 1,
                           param->new_db_name->str, name.str, TRN_EXT, 0);
  trigname_file.str = trigname_buff;

  trigname.trigger_table = *param->new_table_name;

  if (base->create_lists_needed_for_files(current_thd->mem_root)) return true;
...
bool Table_triggers_list::change_table_name(THD *thd, const LEX_CSTRING *db,
  ...
  */
  DBUG_ASSERT(thd->mdl_context.is_lock_owner(MDL_key::TABLE, db->str,
                                             old_table->str, MDL_EXCLUSIVE));

  DBUG_ASSERT(
      my_strcasecmp(table_alias_charset, db->str, new_db->str) ||
  ...
  if (table.triggers) {
    ...
    */
    ...
    if ((err_trigger = table.triggers->change_table_name_in_trignames(
             upgrading50to51 ? db : NULL, new_db, new_table, 0))) {
      /*
      ...
      */
      (void)table.triggers->change_table_name_in_trignames(
          upgrading50to51 ? new_db : NULL, db, old_alias, err_trigger);
...
bool Table_triggers_list::process_triggers(THD *thd, trg_event_type event,
  ...
  if (!(trigger = get_trigger(event, time_type))) return FALSE;
  ...
  */
  ...
  do {
    ...
    err_status = trigger->body->execute_trigger(thd, &trigger_table->s->db,
    ...
    if (table_list->trg_event_map &
      ...
      for (int j = 0; j < (int)TRG_ACTION_MAX; j++) {
        ...
        for (; triggers; triggers = triggers->next) {
          ...
          if (sp_add_used_routine(prelocking_ctx, thd->stmt_arena, &key,
                                  &sp_handler_trigger,
                                  table_list->belong_to_view)) {
            trigger->add_used_tables_to_table_list(
...
bool Table_triggers_list::is_fields_updated_in_trigger(
    MY_BITMAP *used_fields, trg_event_type event_type,
    trg_action_time_type action_time) {
  Item_trigger_field *trg_field;
  DBUG_ASSERT(used_fields->n_bits == trigger_table->s->fields);

  for (Trigger *trigger = get_trigger(event_type, action_time); trigger;
       trigger = trigger->next) {
    for (trg_field = trigger->trigger_fields; trg_field;
         trg_field = trg_field->next_trg_field) {
...
void Table_triggers_list::mark_fields_used(trg_event_type event) {
  ...
  for (action_time = 0; action_time < (int)TRG_ACTION_MAX; action_time++) {
    for (Trigger *trigger = get_trigger(event, action_time); trigger;
         trigger = trigger->next) {
      for (trg_field = trigger->trigger_fields; trg_field;
           trg_field = trg_field->next_trg_field) {
        ...
        if (trg_field->field_idx != (uint)-1) {
          ...
          if (trigger_table->field[trg_field->field_idx]->vcol_info)
            trigger_table->mark_virtual_col(
...
bool Handle_old_incorrect_sql_modes_hook::process_unknown_string(
  ...
  if (unknown_key + INVALID_SQL_MODES_LENGTH + 1 < end &&
    ...
    push_warning_printf(thd, Sql_condition::WARN_LEVEL_NOTE, ER_OLD_FILE_FORMAT,
                        ER_THD(thd, ER_OLD_FILE_FORMAT), (char *)path,
                        "TRIGGER");
...
bool Handle_old_incorrect_trigger_table_hook::process_unknown_string(
  ...
  if (unknown_key + INVALID_TRIGGER_TABLE_LENGTH + 1 < end &&
    ...
    if (!(ptr =
              parse_escaped_string(ptr, end, mem_root, trigger_table_value))) {
...
void build_trn_path(THD *thd, const sp_name *trg_name, LEX_STRING *trn_path) {
  ...
  trn_path->length =
      build_table_filename(trn_path->str, FN_REFLEN - 1, trg_name->m_db.str,
                           trg_name->m_name.str, TRN_EXT, 0);
}
...
bool load_table_name_for_trigger(THD *thd, const sp_name *trg_name,
  ...
  Handle_old_incorrect_trigger_table_hook trigger_table_hook(
      trn_path->str, &trn_data.trigger_table);
  ...
  if (!is_equal(&trigname_file_type, parser->type())) {
    my_error(ER_WRONG_OBJECT, MYF(0), trg_name->m_name.str, TRN_EXT + 1,
             "TRIGGERNAME");

    DBUG_RETURN(TRUE);
  }

  if (parser->parse((uchar *)&trn_data, thd->mem_root, trigname_file_parameters,
                    1, &trigger_table_hook))
    DBUG_RETURN(TRUE);
