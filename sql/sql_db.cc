...
static inline bool cmp_db_names(LEX_CSTRING *db1_name,
                                const LEX_CSTRING *db2_name) {
  return (
      db1_name->length == db2_name->length &&
      (db1_name->length == 0 ||
       my_strcasecmp(table_alias_charset, db1_name->str, db2_name->str) == 0));
}
...
bool my_dboptions_cache_init(void) {
  ...
  if (!dboptions_init) {
    dboptions_init = 1;
    error = my_hash_init(&dboptions, table_alias_charset, 32, 0, 0,
...
void my_dbopt_cleanup(void) {
  ...
  my_hash_free(&dboptions);
  my_hash_init(&dboptions, table_alias_charset, 32, 0, 0,
               (my_hash_get_key)dboptions_get_key, free_dbopt, 0);
...
static bool write_db_opt(THD *thd, const char *path,
  ...
  if ((file = mysql_file_create(key_file_dbopt, path, CREATE_MODE,
    ...
    ulong length;
    length = (ulong)(
        strxnmov(buf, sizeof(buf) - 1, "default-character-set=",
...
bool load_db_opt(THD *thd, const char *path, Schema_specification_st *create) {
  ...
  if ((file = mysql_file_open(key_file_dbopt, path, O_RDONLY | O_SHARE,
                              MYF(0))) < 0)
    goto err1;

  IO_CACHE cache;
  if (init_io_cache(&cache, file, IO_SIZE, READ_CACHE, 0, 0, MYF(0))) goto err2;
...
bool load_db_opt_by_name(THD *thd, const char *db_name,
  ...
  */
  (void)build_table_filename(db_opt_path, sizeof(db_opt_path) - 1, db_name, "",
                             MY_DB_OPT_FILE, 0);
...
static int mysql_create_db_internal(THD *thd, const LEX_CSTRING *db,
  ...
  if (lock_schema_name(thd, dbnorm)) DBUG_RETURN(-1);
  ...
  if (!mysql_file_stat(key_file_misc, path, &stat_info, MYF(0))) {
    ...
    */
    ...
    push_warning_printf(thd, Sql_condition::WARN_LEVEL_NOTE,
                        ...
                        db->str);
    affected_rows = 0;
    if (!thd->is_support_ddl_by_ctl) goto not_silent;
  } else {
    my_error(ER_DB_CREATE_EXISTS, MYF(0), db->str);
    DBUG_RETURN(-1);
  }
...
not_silent:
  if (!silent) {
    ...
    if (mysql_bin_log.is_open()) {
      ...
      */
      if (mysql_bin_log.write(&qinfo)) DBUG_RETURN(-1);
    }
...
static bool mysql_alter_db_internal(THD *thd, const LEX_CSTRING *db,
                                    Schema_specification_st *create_info) {
  char path[FN_REFLEN + 16];
  long result = 1;
  int error = 0;
  ...
  */
  ...
  if (unlikely((error = write_db_opt(thd, path, create_info)))) goto exit;
  ...
  if (mysql_bin_log.is_open()) {
    ...
    */
    ...
    /*
      ...
      metadata lock on the schema.
    */
    if (unlikely((error = mysql_bin_log.write(&qinfo)))) goto exit;
  }
  my_ok(thd, result);
...
static bool mysql_rm_db_internal(THD *thd, const LEX_CSTRING *db,
  ...
  if (lock_schema_name(thd, dbnorm)) DBUG_RETURN(true);
  ...
  if (!(dirp = my_dir(path, MYF(MY_DONT_SORT)))) {
    if (!if_exists) {
      ...
      push_warning_printf(thd, Sql_condition::WARN_LEVEL_NOTE,
                          ...
                          db->str);
      error = false;
      if (!thd->is_support_ddl_by_ctl) goto update_binlog;
    }
  }
  ...
  */
  if ((rm_mysql_schema =
    ...
    for (table = tables; table; table = table->next_local)
      if (check_if_log_table(table, TRUE, "DROP")) goto exit;
  }
  ...
  if (!thd->killed &&
      !(tables && mysql_rm_table_no_locks(thd, tables, true, false, true, false,
                                          true, false))) {
    /*
...
update_binlog:
  if (!silent && likely(!error)) {
    ...
    for (tbl = tables; tbl; tbl = tbl->next_local) {
      ...
      if (ha_table_exists(thd, &tbl->db, &tbl->table_name)) continue;
      ...
      if (query_pos + tbl_name_len + 1 >= query_end) {
        /*
          ...
          metadata lock on the schema.
        */
        if (write_to_binlog(thd, query, (uint)(query_pos - 1 - query), db->str,
                            db->length)) {
          error = true;
          goto exit;
        }
...
static bool find_db_tables_and_rm_known_files(THD *thd, MY_DIR *dirp,
  ...
  if (ha_discover_table_names(thd, &db, dirp, &tl, true)) DBUG_RETURN(1);
  ...
  for (size_t idx = 0; idx < files.elements(); idx++) {
    ...
    if (!table_list) DBUG_RETURN(true);
    table_list->db = db;
    ...
    if (lower_case_table_names)
      table_list->table_name.length =
          my_casedn_str(files_charset_info, (char *)table_list->table_name.str);
  ...
  *tables = tot_list;
  ...
  for (uint idx = 0; idx < (uint)dirp->number_of_files && !thd->killed; idx++) {
...
long mysql_rm_arc_files(THD *thd, MY_DIR *dirp, const char *org_path) {
  ...
  for (uint idx = 0; idx < (uint)dirp->number_of_files && !thd->killed; idx++) {
    ...
    if (mysql_file_delete_with_symlink(key_file_misc, filePath, "",
                                       MYF(MY_WME))) {
      goto err;
    }
    deleted++;
...
bool mysql_change_db(THD *thd, const LEX_CSTRING *new_db_name,
  ...
  */
  ...
  new_db_file_name.str =
      my_strndup(new_db_name->str, new_db_name->length, MYF(MY_WME));
  ...
  */
  ...
  if (test_all_bits(sctx->master_access, DB_ACLS))
    ...
    if (sctx->priv_role[0]) {
      ...
      db_access |=
          acl_get("", "", sctx->priv_role, new_db_file_name.str, FALSE);
    }
  }
  ...
  if (check_db_dir_existence(new_db_file_name.str)) {
    if (force_switch) {
      ...
      push_warning_printf(thd, Sql_condition::WARN_LEVEL_NOTE, ER_BAD_DB_ERROR,
...
bool mysql_upgrade_db(THD *thd, const LEX_CSTRING *old_db) {
  ...
  if ((old_db->length <= MYSQL50_TABLE_NAME_PREFIX_LENGTH) ||
      (strncmp(old_db->str, MYSQL50_TABLE_NAME_PREFIX,
               MYSQL50_TABLE_NAME_PREFIX_LENGTH) != 0)) {
    my_error(ER_WRONG_USAGE, MYF(0),
             "ALTER DATABASE UPGRADE DATA DIRECTORY NAME", "name");
    DBUG_RETURN(1);
  }
  ...
  if (lock_schema_name(thd, old_db->str)) DBUG_RETURN(1);

  /*
    Let's remember if we should do "USE newdb" afterwards.
    thd->db will be cleared in mysql_rename_db()
  */
  if (thd->db.str && !cmp(&thd->db, old_db)) change_to_newdb = 1;
  ...
  if ((dirp = my_dir(path, MYF(MY_DONT_SORT)))) {
    ...
    for (uint idx = 0; idx < nfiles && !thd->killed; idx++) {
      ...
      if (!(extension = (char *)fn_frm_ext(file->name))) continue;

      /* A frm file found, add the table info rename list */
      *extension = '\0';

      table_str.length =
          filename_to_tablename(file->name, tname, sizeof(tname) - 1);
      table_str.str = (char *)thd->memdup(tname, table_str.length + 1);
      Table_ident *old_ident = new Table_ident(thd, old_db, &table_str, 0);
      Table_ident *new_ident = new Table_ident(thd, &new_db, &table_str, 0);
      if (!old_ident || !new_ident ||
          !sl->add_table_to_list(thd, old_ident, NULL, TL_OPTION_UPDATING,
                                 TL_IGNORE, MDL_EXCLUSIVE) ||
          !sl->add_table_to_list(thd, new_ident, NULL, TL_OPTION_UPDATING,
                                 TL_IGNORE, MDL_EXCLUSIVE)) {
        error = 1;
        my_dirend(dirp);
        goto exit;
  ...
  if ((table_list = thd->lex->query_tables) &&
    ...
    */
    ...
    length =
        build_table_filename(path, sizeof(path) - 1, new_db.str, "", "", 0);
    if (length && path[length - 1] == FN_LIBCHAR)
      path[length - 1] = 0;  // remove ending '\'
    rmdir(path);
    goto exit;
  ...
  */
  ...
  if ((dirp = my_dir(path, MYF(MY_DONT_SORT)))) {
    ...
    for (uint idx = 0; idx < nfiles; idx++) {
      ...
      build_table_filename(oldname, sizeof(oldname) - 1, old_db->str, "",
                           file->name, 0);
      build_table_filename(newname, sizeof(newname) - 1, new_db.str, "",
                           file->name, 0);
  ...
  */
  ...
  if (change_to_newdb) error |= mysql_change_db(thd, &new_db, FALSE);
...
const char *normalize_db_name(const char *db, char *buffer,
  ...
  if (!lower_case_table_names) return db;
