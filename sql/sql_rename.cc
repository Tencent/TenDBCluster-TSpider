...
static bool do_rename(THD *thd, TABLE_LIST *ren_table,
                      ...
                      const LEX_CSTRING *new_table_alias, bool skip_error);
...
bool mysql_rename_tables(THD *thd, TABLE_LIST *table_list, bool silent) {
    ...
    for (to_table = 0, ren_table = table_list; ren_table;
      ...
      if ((log_table_rename = check_if_log_table(ren_table, TRUE, NullS))) {
        /*
          ...
          with 0, while QUERY_LOG_SLOW == 1 and QUERY_LOG_GENERAL == 2  and
          QUERY_LOG_ALTER == 3 So, we shift the value to start with 0;
        */
        log_table_rename--;
          ...
          if (to_table)
            ...
            */
            my_error(ER_CANT_RENAME_LOG_TABLE, MYF(0),
                     ren_table->table_name.str, ren_table->table_name.str);
            goto err;
          }
        } else {
          if (to_table) {
  ...
  */
  ...
  if (likely(!error)) query_cache_invalidate3(thd, table_list, 0);
...
static bool do_rename_temporary(THD *thd, TABLE_LIST *ren_table,
                                TABLE_LIST *new_table, bool skip_error) {
  ...
  DBUG_RETURN(
      thd->rename_temporary_table(ren_table->table, &new_table->db, new_alias));
}
...
static bool do_rename(THD *thd, TABLE_LIST *ren_table,
  ...
  if (ha_table_exists(thd, &ren_table->db, &old_alias, &hton) && hton) {
    ...
    tdc_remove_table(thd, TDC_RT_REMOVE_ALL, ren_table->db.str,
                     ren_table->table_name.str, false);

    if (hton != view_pseudo_hton) {
      if (!(rc = mysql_rename_table(hton, &ren_table->db, &old_alias, new_db,
                                    &new_alias, 0))) {
        (void)rename_table_in_stat_tables(
            thd, &ren_table->db, &ren_table->table_name, new_db, &new_alias);
        if ((rc = Table_triggers_list::change_table_name(
                 thd, &ren_table->db, &old_alias, &ren_table->table_name,
                 new_db, &new_alias))) {
          /*
      ...
      */
      if (thd->lex->sql_command != SQLCOM_ALTER_DB_UPGRADE &&
        ...
        my_error(ER_FORBID_SCHEMA_CHANGE, MYF(0), ren_table->db.str,
                 new_db->str);
      else
  ...
  if (unlikely(rc && !skip_error)) DBUG_RETURN(1);
