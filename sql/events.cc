...
static int create_query_string(THD *thd, String *buf) {
  ...
  if (thd->lex->create_info.or_replace()) {
    if (buf->append(STRING_WITH_LEN("CREATE OR REPLACE "))) return 1;
...
bool Events::create_event(THD *thd, Event_parse_data *parse_data) {
  ...
  if (unlikely(check_if_system_tables_error())) DBUG_RETURN(TRUE);
  ...
  if (!(ret = db_repository->create_event(thd, parse_data,
    ...
    if (!event_already_exists) {
      if (!(new_element = new Event_queue_element()))
        ...
        bool created;
        if (event_queue) event_queue->create_event(thd, new_element, &created);
      }
    }
    /*
...
bool Events::drop_event(THD *thd, const LEX_CSTRING *dbname,
...
void Events::drop_schema_events(THD *thd, const char *db) {
  ...
  */
  if (event_queue) event_queue->drop_schema_events(thd, &db_lex);
...
static bool send_show_create_event(THD *thd, Event_timed *et,
  ...
  field_list.push_back(
      new (mem_root) Item_empty_string(thd, "Event", NAME_CHAR_LEN), mem_root);

  if (sql_mode_string_representation(thd, et->sql_mode, &sql_mode))
    DBUG_RETURN(TRUE);

  field_list.push_back(
      new (mem_root) Item_empty_string(thd, "sql_mode", (uint)sql_mode.length),
      mem_root);

  tz_name = et->time_zone->get_name();

  field_list.push_back(
      new (mem_root) Item_empty_string(thd, "time_zone", tz_name->length()),
      mem_root);

  field_list.push_back(
      new (mem_root) Item_empty_string(thd, "Create Event", show_str.length()),
      mem_root);
  ...
  field_list.push_back(new (mem_root) Item_empty_string(
                           thd, "Database Collation", MY_CS_NAME_SIZE),
                       mem_root);
...
bool Events::show_create_event(THD *thd, const LEX_CSTRING *dbname,
  ...
  */
  ...
  if (!ret) ret = send_show_create_event(thd, &et, thd->protocol);
...
int Events::fill_schema_events(THD *thd, TABLE_LIST *tables,
                               COND * /* cond */) {
  const char *db = NULL;
  int ret;
  ...
  /*
    ...
    empty set
  */
  if (opt_noacl) DBUG_RETURN(0);

  if (unlikely(check_if_system_tables_error())) DBUG_RETURN(1);
  ...
  */
  if (thd->lex->sql_command == SQLCOM_SHOW_EVENTS) {
    ...
    if (!is_infoschema_db(
            &thd->lex->select_lex.db) &&  // There is no events in I_S
        check_access(thd, EVENT_ACL, thd->lex->select_lex.db.str, NULL, NULL, 0,
                     0))
      DBUG_RETURN(1);
...
PSI_stage_info stage_waiting_for_next_activation = {
    0, "Waiting for next activation", 0};
PSI_stage_info stage_waiting_for_scheduler_to_stop = {
    0, "Waiting for the scheduler to stop", 0};
...
bool Events::load_events_from_db(THD *thd) {
  ...
  */
  ...
  if (init_read_record(&read_record_info, thd, table, NULL, NULL, 0, 1,
                       FALSE)) {
  ...
  while (!(read_record_info.read_record())) {
    ...
    if (!(et = new Event_queue_element)) goto end;
  ...
  my_printf_error(
      ER_STARTUP, "Event Scheduler: Loaded %d event%s",
      MYF(ME_NOREFRESH | (global_system_variables.log_warnings) ? ME_JUST_INFO
                                                                : 0),
      count, (count == 1) ? "" : "s");
  ret = FALSE;
