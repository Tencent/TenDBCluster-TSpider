...
bool init_read_record(READ_RECORD *info, THD *thd, TABLE *table,
                      SQL_SELECT *select, SORT_INFO *filesort,
  ...
  if (tempfile && !(select && select->quick)) {
    ...
    if (!table->file->inited)
      if (unlikely(table->file->ha_rnd_init_with_error(0))) DBUG_RETURN(1);
    ...
    */
    if (!disable_rr_cache && !addon_field &&
    ...
    info->cache_end =
        (info->cache_pos + filesort->return_rows * info->ref_length);
    info->read_record_func =
    ...
    if (unlikely(table->file->ha_rnd_init_with_error(1))) DBUG_RETURN(1);
  ...
  if ((table->file->ha_table_flags() & HA_CAN_TABLE_CONDITION_PUSHDOWN) &&
      select && select->cond && (select->cond->used_tables() & table->map) &&
...
static int rr_from_tempfile(READ_RECORD *info) {
  ...
  for (;;) {
    ...
    if (tmp == HA_ERR_KEY_NOT_FOUND) continue;
    tmp = rr_handle_error(info, tmp);
    break;
  }
  return tmp;
...
static int rr_unpack_from_buffer(READ_RECORD *info) {
  ...
  (*info->unpack)(info->addon_field, info->cache_pos, info->cache_end);
  info->cache_pos += info->ref_length;
  return 0;
}
...
static int rr_from_cache(READ_RECORD *info) {
  ...
  for (;;) {
    ...
    for (i = 0; i < length; i++) {
      ...
      if (unlikely((error = (int16)info->table->file->ha_rnd_pos(
                        record_pos, info->ref_pos)))) {
